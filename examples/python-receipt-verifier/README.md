...
`sovereign-receipt.json`'s `result` is self-referential (it describes its own verification) — fine for a verifier smoke test, wrong for a product demo, because **a proof is only as compelling as the claim it secures.** A UI that wants to demonstrate proof should sign a receipt for _recognizable work_, so a visitor breaks the proof of _something they'd actually delegate_. Two such vectors, same demo agent, also `sovereign`:

| Fixture                                 | `result` (the act being proven)                                                                    |
| --------------------------------------- | -------------------------------------------------------------------------------------------------- |
| `sovereign-receipt-stripe-audit.json`   | Audited 1,284 Stripe payouts; flagged 3 mismatches over $50; **paused before export** for approval |
| `sovereign-receipt-email-approval.json` | Drafted an outbound email and **queued it for approval — not sent** (outbound send is gated)       |

Both are real (`sovereign: true`, tamper → `§11.2`), but they lead with the _act_ (who/what, the policy pause) and let the cryptography be the quiet backing — `records-vs-acts` doctrine applied to a proof surface. Regenerated by `fixtures/mint-demo-receipts.mjs`. These are **frozen** vectors: consumers byte-match them, so a changed receipt gets a new filename rather than an in-place edit.
