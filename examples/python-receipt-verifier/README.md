...
### Two fixtures, two binding rungs

Both fixtures are real signed receipts on the same suite (`motebit-jcs-ed25519-b64-v1`) and both verify here for **integrity** (signature valid, bytes intact). They differ in **identity binding** — _whose_ key it is:

| Fixture                  | Integrity | Binding rung                                                                                           |
| ------------------------ | --------- | ------------------------------------------------------------------------------------------------------ |
| `example-receipt.json`   | valid     | **integrity-only** — random `motebit_id`; the embedded key signed it, but the id does not bind the key |
| `sovereign-receipt.json` | valid     | **sovereign** — `motebit_id` is the commitment to the signing key, provable offline                    |

This Python reference checks the cryptographic envelope (integrity). The **binding rung** is computed by `@motebit/verifier` (`result.sovereign`) — the integrity check is identical across both, which is exactly why a verifier must never present `valid: true` as proof of identity. See [Verify a Receipt](https://docs.motebit.com/docs/developer/verify-a-receipt) for the full ladder.

`sovereign-receipt.json` is regenerated by `fixtures/mint-sovereign-fixture.mjs` — signed through the canonical `signExecutionReceipt` (never hand-rolled) with a **fixed, public demo key** so the artifact is fully reproducible. There is nothing secret about a demo signer; the point is that anyone can reproduce and verify it.

### Outcome-shaped demo receipts (for proof UIs)

`sovereign-receipt.json`'s `result` is self-referential (it describes its own verification) — fine for a verifier smoke test, wrong for a product demo, because **a proof is only as compelling as the claim it secures.** A UI that wants to demonstrate proof should sign a receipt for _recognizable work_, so a visitor breaks the proof of _something they'd actually delegate_. These vectors, same demo agent, all `sovereign` — and chosen to span the real **governance spectrum** (`status` is a real field), not to manufacture volume:

| Fixture                                    | `status`    | The act being proven (the real governance state)                                                    |
| ------------------------------------------ | ----------- | --------------------------------------------------------------------------------------------------- |
| `sovereign-receipt-research-complete.json` | `completed` | Researched 10 competitors, compiled a sheet — **acted autonomously**, read-only, within bounds      |
| `sovereign-receipt-stripe-audit.json`      | `completed` | Audited 1,284 Stripe payouts; flagged 3 over $50; **paused before export** for approval             |
| `sovereign-receipt-email-approval.json`    | `completed` | Drafted an outbound email and **queued for approval — not sent** (outbound send is gated)           |
| `sovereign-receipt-payment-denied.json`    | `denied`    | Attempted a $1,200 payment; **denied by policy** (payee not allowlisted, over cap) — no funds moved |

Each is real (`sovereign: true`, tamper → `§11.2`) and leads with the _act_ + its governance state — `records-vs-acts` with the cryptography as quiet backing. The spread (**acts / pauses for approval / refuses**) is the honest way to show variety: real `status` values, not fabricated liveness. The strongest is `payment-denied` — the agent's own signed receipt attests it _blocked itself_, which is "constrained enough to trust" made provable. Regenerated by `fixtures/mint-demo-receipts.mjs`. **Frozen** vectors: consumers byte-match them, so a changed receipt gets a new filename, never an in-place edit.
