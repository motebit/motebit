...
- nothing stored about any other connection — phones, browsers, desktop sessions, and sockets that did not prove their device id; sockets the relay believes open are reported live (a count per device and key of host sockets only — device id verified and announcing unattended_runtime; any other open socket whose device id is verified, on a pair with no liveness row, is reported as a count per device and key under live_unenrolled) and never persisted
