 ...
 * (`livenessKeyOf`), and the GET's `host_sockets_open` and the sweep's
 * live-skip count sockets by it too — a live host count and the persisted
 * row never disagree about which sockets are a host. The GET's
 * `sockets_open` is a DIFFERENT quantity (every bound socket on the pair,
 * host or not) and keeps its meaning: it is a session signal, not liveness.
 */
 ...
 * `host_sockets_open`, and the TTL sweep's live-skip (`openHostSockets`).
 * Returns the key the socket counts under, or null.
 *
 ...
 * (surfaces §0), a phone, a tab — is not a host's liveness: counted as one,
 * a dead daemon read "open" while only the desktop was connected, both
 * together read 2 ("two machines may share this id"), and the desktop alone
 * kept a dead daemon's row from ageing out. It still counts in
 * `sockets_open` — something IS attached — which is what a retired
 * machine's "still connected" signal needs.
 */
 ...
 * itself, so the skip and the GET's `host_sockets_open` are one predicate.
 * A row swept this way while a non-host socket is open reappears in the GET
 * as `live_unenrolled` with `host_sockets_open: 0` — a session, not a host.
