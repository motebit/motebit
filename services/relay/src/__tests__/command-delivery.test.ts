 ...
 *
 *  1. Delivery prefers the NEWEST open socket, and never falls through to
 *     another socket of the machine on silence (the envelope is single-use
 *     per machine; see `sendToOne`).
 *  4. A socket that closes — or is retired — after delivery settles the
 *     request at once as `outcome: "closed_after_delivery"`, still a 504
 *     ("delivered, no answer"); a socket that was NOT delivered to settles
 *     nothing.
...
describe("item 4 — a socket that leaves after delivery settles the request at once", () => {
  it("the delivered socket CLOSES ⇒ 504 closed_after_delivery, long before the deadline", async () => {
...
describe("item 6 — an answer settles only the request of the motebit it arrived from", () => {
