...
describe("liveness housekeeping over 3 × 31 days", () => {
  it("an open idle host stays fresh via the flush and is never swept; a closed host is swept after 90 days; a row kept open only by a NON-host socket is swept too", async () => {
    const t0 = Date.now();
    ...
    // the desktop beside a daemon that has since died.
    ...
    // A non-host socket is not a host's liveness: the flush did not refresh
    // the demoted row and the live-skip did not keep it — swept at 90 days.
    expect(rows.has("demoted")).toBe(false);
