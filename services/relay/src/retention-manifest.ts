...
> = {
  stores: [
    // Empty by phase 6a. Each entry below appears here as the matching
    // phase lands the actual enforcement:
    //
    //   - phase 4b-3 — operational ledgers (relay_execution_ledgers,
    //     relay_settlements, relay_credential_anchor_batches, etc.)
    //     register under `append_only_horizon` once the federation
    //     co-witness handshake exists and the relay starts advancing
    //     horizons. Until then, declaring the shape would commit the
    //     operator to behavior the runtime doesn't yet enforce.
    //
    //   - phase 5 — conversation + tool-audit stores register under
    //     `consolidation_flush` once the runtime owns the flush phase
    //     and adapter implementations carry sensitivity.
    //
    // The empty list is the truthful state; it grows additively.
  ],
  pre_classification_default_sensitivity: PRE_CLASSIFICATION_DEFAULT_SENSITIVITY,
  honest_gaps: [
    "operational ledgers (relay_execution_ledgers, relay_settlements, " +
    ...
    "conversation_messages + tool_audit will register under `consolidation_flush` " +
      "when phase 5 lands per-record sensitivity classification and the consolidation " +
      "cycle's flush phase. Today these stores carry no sensitivity field and have " +
      "no retention enforcement.",
    "presence data (agent_registry, relay_identity, pairing_sessions) is TTL-based " +
      "(declared in /.well-known/motebit-transparency.json) and is not " +
      "retention-cert-shaped — TTL expiry is a different mechanism than the signed " +
      "deletion certificates this manifest enumerates. Presence retention is governed " +
      "by the operator-transparency manifest, not this one.",
    "onchain anchor of this manifest is not yet in place; only cached copies " +
      "of the JSON survive operator deletion. Sibling concern to the operator-" +
      "transparency manifest's onchain anchor (also pending).",
  ],
} as const;
