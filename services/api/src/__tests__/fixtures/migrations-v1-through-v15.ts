/**
 * Frozen snapshot of the 15 relay migrations as they existed before
 * the 1.0 squash (2026-04-24). Exists only as the reference for the
 * schema-equivalence test that validates the squashed `v1_initial` in
 * `src/migrations.ts` produces the byte-identical `sqlite_schema`
 * that applying this full chain produces on a fresh DB.
 *
 * NEVER APPLIED IN PRODUCTION. Never imported outside this directory.
 * Never edited after the squash lands. If a future schema change
 * needs a new migration, it lands as `v16`, `v17`, … in
 * `src/migrations.ts` — this file is the historical ceiling of
 * the squash, not an extensible list.
 *
 * The production relay at motebit.com applied versions 1 through 15
 * in sequence before the squash; its `relay_schema_migrations` table
 * still records that history. Fresh installs get `v1_initial` only.
 * This fixture is the equivalence proof that the two paths converge.
 */
...
// ── Frozen pre-1.0 relay migrations (v1 through v15, 2026-03 → 2026-04) ─────

export const preV1SquashMigrations: FixtureMigration[] = [
  {
    version: 1,
    name: "initial_schema",
    up: (db) => {
