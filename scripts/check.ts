...
const GATES: ReadonlyArray<Gate> = [
  ...
  },
  {
    ...
    defends:
      'every package on the safety-critical floor carries property-based / fuzz / mutation test coverage — `fast-check` declared in devDependencies AND at least one `.test.ts` under `src/__tests__/` imports it. The floor is motebit-canonical: money path (`@motebit/virtual-accounts` — conservation invariants), four hardware-attestation verifiers (`@motebit/crypto-{appattest,android-keystore,webauthn,tpm}` — any single mutation in a verified receipt MUST yield `valid: false`), parser boundaries (`@motebit/skills` — `parseSkillFile` typed-error envelope under arbitrary input bytes), semiring algebra (`@motebit/protocol` — laws across arbitrary weighted-digraph inputs). These four boundaries make claims that hand-written tests structurally cannot exhaust; property tests are the empirical backstop. Closed-list-with-reason shape (same as `check-skills-cross-surface` #73 and `check-typed-truth-perception` #80): adding a new safety-critical package = one entry in `PACKAGES_REQUIRING_PROPERTY_TESTS` + one properties.test.ts + one fast-check devDependency. Demoting a package from the floor is a doctrine moment — the commit naming WHY the promise no longer holds. Per `feedback_legibility_ratio` memory: every commitment ships in three artifacts (code + drift gate + doctrine); this gate is the third leg paired with the eight property-suite commits across the seven packages and the `docs/doctrine/evals-as-attestations.md` § "What ships now" testing-discipline arc.',
