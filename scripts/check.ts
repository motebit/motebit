...
const GATES: ReadonlyArray<Gate> = [
  ...
  },
  {
    ...
    defends:
      'every `aud: "<literal>"`, `aud === "<literal>"`, `aud !== "<literal>"`, `createSyncToken("<literal>")`, and `createCallerToken("<literal>")` site in `packages/`, `services/`, `apps/` carries a literal that is a member of the closed `TokenAudience` registry in `@motebit/protocol/src/audience.ts` (17 audiences today: `sync`, `device:auth`, `pair`, `rotate-key`, `push:register`, `task:submit`, `admin:query`, `proposal`, `receipts:read`, `account:{balance,deposit,withdraw,withdrawals,checkout}`, `browser-sandbox-grant`, `browser-sandbox`, `runtime:attach`). Pre-registry, a signing-site typo (`aud: "task:sumbit"`) was a runtime 401 at the verifier — same fail-loud semantics, but the only signal was the wire round-trip; the doctrine in `services/relay/CLAUDE.md` Rule 5 listed six audiences while code used fifteen. Closure pattern, same as `SuiteId` (`check-suite-declared` + `check-suite-dispatch`): closed protocol-level registry + drift gate that asks role questions, not instance questions. Sibling-alignment check verifies `CANONICAL_AUDIENCES` mirrors `ALL_TOKEN_AUDIENCES`; a registry update without a gate update is itself a CI failure. Adding an audience: union entry + named constant + `ALL_TOKEN_AUDIENCES` + `CANONICAL_AUDIENCES` in this gate + doctrine update at `services/relay/CLAUDE.md` Rule 5.',
