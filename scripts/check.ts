...
const GATES: ReadonlyArray<Gate> = [
  ...
  },
  {
    ...
    defends:
      'the slab chrome\'s render is `f(controlState × embodimentMode)` (`docs/doctrine/chrome-as-state-render.md` § "The principle"); every surface that mounts a slab dispatcher MUST dispatch against the full matrix shape. Closed-registry / structural-lock pattern (same shape as #79 `check-universal-slash-coverage`, #10 `check-suite-declared`, #46 `check-audience-canonical`, #85 `check-artifact-type-canonical`, #92 `check-transparency-processors-canonical`, #93 `check-publishable-package-metadata`). `SLAB_SURFACES` is the closed inventory of dispatcher-bearing surfaces (web shipped 2026-05-12, mobile shipped 2026-05-13); `CONTROL_STATES` is the closed `ControlState` union; `DEFERRED_EMBODIMENT_MODES` is the closed list of columns named in the matrix but deferred to PR N. The gate asserts (1) each surface\'s dispatcher file + entry-point exists, (2) every control state appears as a backtick/quote-quoted literal in the dispatcher\'s source (handling, not just declaration — JSDoc-named cells count, matching the doctrine\'s "named in the matrix" contract), (3) every deferred embodiment is referenced. Lifts the doctrine from one-instance-deep (web alone) to generalizable pattern: a future slab surface (desktop, spatial AR-glasses) added to SLAB_SURFACES forces the gate to demand every cell be named in the new dispatcher\'s source. Adversarial probe: rewriting every `paused` literal in `apps/mobile/src/slab-chrome.ts` to `PAUSED_REMOVED` (the structural-erasure drift, where both code and JSDoc forget about the cell) makes the gate flag `control state "paused" not handled in apps/mobile/src/slab-chrome.ts` and exit non-zero; restoring returns the clean `2 surface(s) × 4 control state(s) + 5 deferred embodiment(s)` line. Doctrine: `docs/doctrine/chrome-as-state-render.md` § "Spatial-as-endgame validation" + § "PR 2 scope (mobile)."',
