...
const EXCLUDED_CHECKS: Record<string, string> = {
  ...
  "check-activation-effective":
    "sibling meta-probe for the booted-*-activation suites — reintroduces each suite's severing into the relay source, rebuilds the compiled artifact, and requires the suite to red (a dist rebuild per probe). Far too heavy for the static `pnpm check` pass; runs as the separate activation-effectiveness CI job scoped to relay/* + scripts/* changes.",
...
const GATES: ReadonlyArray<Gate> = [
  ...
  {
    name: "check-tool-guard-parity",
    defends:
      "the desktop Rust tool-guard's destructive-command vocabulary (`apps/desktop/src-tauri/src/tool_guard.rs` — `ALWAYS_DESTRUCTIVE` + the `is_destructive_command` pattern-guarded commands) mirrors the canonical `@motebit/tools` shell-exec source (`packages/tools/src/builtins/shell-exec.ts` — `ALWAYS_DESTRUCTIVE` + `DESTRUCTIVE_PATTERNS`) set-for-set, both directions, no allowlist. The desktop app is the strongest-root surface (the runtime-host coordinator) and runs `shell_exec_tool` on raw webview-supplied input; its TS policy gate is bypassable by a direct IPC call, so the Rust tool-guard is the last-line, defense-in-depth wipe guard — a hand-written Rust mirror of the TS source kept in sync only by a comment until now. Add a destructive command to the TS source and forget the Rust mirror and the most-privileged surface silently loses a catastrophic-wipe guard — the same asymmetric Rust↔TS drift `check-computer-use-dispatcher-parity` forbids for the other mirror. Destructive guards must be identical on every platform (a wipe is catastrophic anywhere), so this is strict set-equality with no allowlist. (Invariant #134, added 2026-06-28.)",
  ...
  {
    name: "check-doctrine-cited-constants",
    defends:
      "a doctrine doc that cites a pinned render constant with a value must cite the value in the code source of truth (`packages/render-engine/src/creature.ts`). Docs narrate the creature's material as physics-derived, but a cited constant is only true if it matches what the body actually renders. Why: the 2026-07-25 verification pass found DROPLET.md citing `transmission 0.98` — with a whole 'the critical consequence of 0.98 transmission' narrative — while the shipped body renders `transmission: 0.94` (creature.ts, pinned by creature.test.ts's `toBeCloseTo(0.94, 5)`); drifted for months, and a maintenance PR nearly 'fixed' it BACKWARDS by trusting the doc table over the code. The rule this makes structural: docs are never ground truth; the code constant + its test is. The canonical value is READ FROM creature.ts at check time (never hardcoded in the gate), so the gate cannot itself drift. v1 pins `transmission` (the single-value constant that drifted, ~6 doctrine sites); `ior` is deliberately reviewer-caught, not gated — DROPLET cites it as a dual value in one cell ('1.45 (physical) / 1.22 (rendered)') and a fragile regex over the physical-vs-rendered split would silently miss drift (worse than none). Same doc↔code drift class as check-doctrine-citations, one level deeper: that gate validates that a cited symbol/path EXISTS; this validates that a cited numeric constant is CORRECT.",
  ...
  {
    name: "check-creature-canon",
    defends:
      "the creature canon's consumption topology (docs/doctrine/creature-canon.md): CANONICAL_CAMERA / CANONICAL_PERFORMANCES exist in packages/render-engine/src/spec.ts (the ONLY home for the creature's camera and performance numbers — the doctrine doc deliberately restates no tunable value); every camera-owning render surface (ThreeJSAdapter, apps/mobile's creature-webview WebView scene) consumes CANONICAL_CAMERA instead of re-encoding camera literals — the hand-copied-scene drift class that already produced mobile rendering the same body under ACESFilmic@1.2 while every other surface had moved to Neutral@1.0; and the golden-frame matrix (apps/web/e2e/golden/golden-matrix.ts) covers every CanonicalCameraName and PerformanceName at least once, so a new pose or performance cannot land without visual-regression coverage. The golden harness itself (Playwright `golden` project, SwiftShader, linux-authoritative committed frames, CI e2e job) is the test-enforced sibling — too heavy for pnpm check; this gate is the cheap static half of the canon's two-layer proof contract (invariant #138, added 2026-07-07 with the creature-endgame arc).",
  ...
  {
    name: "check-eval-kind-canonical",
    defends:
      "the FOUR-way structural lock for the `EvalKind` closed registry (the eleventh registered registry; `docs/doctrine/evals-as-attestations.md`, promoted 2026-07-08 with the Auditor archetype as consumer #1): the `EvalKind` union (`packages/protocol/src/eval-attestation.ts`) × `ALL_EVAL_KINDS` (same file) × `EVAL_KINDS_MIRROR` (`packages/crypto/src/eval-attestation.ts` — crypto keeps zero runtime monorepo deps, so verifyEvalAttestation's fail-closed unknown_eval_kind intake mirrors the registry the way SuiteId values mirror into the dispatch table) × the gate's own `EVAL_KINDS_REFERENCE` MUST agree exactly. PLUS the load-bearing emit-site scan (non-vacuous even at one member): every `eval_kind:` literal in packages/ + services/ production source must be a registry member, so a producer typo fails in CI rather than stranding attestations at a verifier three hops away (__tests__ excluded — negative fixtures legitimately mint non-members). Plus snake_case wire-format compliance. `eval_kind` is the measurement-family discriminator a consumer dispatches on to interpret an EvalAttestation's results[]; unknown kinds fail closed at wire intake. Same closed-registry / structural-lock shape as `check-suite-declared` (#10), `check-settlement-mode-canonical` (#100), `check-memory-source-canonical` (#122).",
    script: "check-eval-kind-canonical",
  },
  {
    name: "check-archetype-slate",
    defends:
      "the three-way structural parity of the archetype slate (docs/doctrine/agent-archetypes.md): scripts/deploy-archetype-slate.ts `SLATE` (what deploys) × scripts/archetype-conformance.ts `ARCHETYPES` (what the daily paid probe checks) × the slate table in apps/docs/content/docs/developer/agent-archetypes.mdx (what the gallery claims) MUST agree on (service, capability, display-name). Drift class: a service added to the deploy ceremony but not the probe is a showcase the conformance run silently stops proving — 'they work' degrades to marketing; a gallery row with no deployed backing is a documented lie; a display-name mismatch breaks the claim-framed Discover rendering; and (fourth check) the declared capability must actually be advertised by services/<name>/src/index.ts — the three declarations agreeing with EACH OTHER is not enough (all three agreed on 'summarize' while the service advertised 'summarize_search', silently un-discovering the deployed slate). Same textual-extraction parity shape as check-computer-use-dispatcher-parity and check-tool-guard-parity. NOT a closed protocol registry (an archetype is curated recipe + self-asserted claim, deliberately never an enum — agent-archetypes.md §2), so no eight-artifact treatment; the three named surfaces are the whole vocabulary.",
    script: "check-archetype-slate",
  },
  {
    name: "check-home-seed-basis",
    defends:
      "the slab home register's derived capability-seed cannot lie (docs/doctrine/motebit-computer.md §home — 'derive the seed; never author it … the N=0 surface cannot lie about what the motebit can do'). Four seams: (1) HOME_CONFIG_KEYS closed-registry lock between apps/web/src/ui/slab-home-model.ts and the gate reference (per-surface UI config vocabulary — deliberately NOT the eight-artifact protocol treatment); (2) live-accessor coupling — buildHomeSeedInputs (apps/web/src/web-app.ts) must answer mind from runtime.isAIReady, relay from the live syncStatus comparison, computer from computerRegistration presence (the check-money-authority textual-anchor style; a key answered by a constant or hand-authored bit is the claimed-vs-enforced hazard); (3) a `recede: <key>` test per key — wired ⇒ its setup affordance is structurally absent (without it a setup chip can rot into a nag or a launchpad into a lie); (4) single-producer scan — deriveHomeSeed is the only HomeTile producer (no `basis:` literals elsewhere in apps/web/src; tests excluded). Drift class: the resting face is the product's first impression and the metabolic principle rendered as UI — a seed that drifts from live state is a brochure wearing a mirror's costume, the exact browser-chrome-costume failure the home-register arc replaced.",
    script: "check-home-seed-basis",
  },
  {
    name: "check-memory-source-canonical",
    defends:
      "the three-way structural lock for the `MemorySource` closed registry — the `MemorySource` union (`packages/protocol/src/memory-source.ts`) × `ALL_MEMORY_SOURCES` (same file) × the gate's own `MEMORY_SOURCES_REFERENCE` mirror MUST agree exactly on the 5-entry provenance vocabulary (`user_stated`, `agent_inferred`, `tool_derived`, `peer_agent`, `consolidation_derived`) — PLUS the two LOAD-BEARING authorship scans (the `check-suite-dispatch` sense, not vacuous registry self-consistency): (a) no `<memory>` tag pattern in `packages/ai-core/src` may carry a `source` attribute — the model can never author its own provenance (the self-escalation channel: an injected turn classifying absorbed web content as user-stated); (b) the MCP server's memory write path may pass ONLY the literal `peer_agent` source and never a caller-derived one — a remote agent can never self-declare a trusted provenance tier. `MemorySource` is the provenance discriminator on every `MemoryFormedPayload` and the input to the `[from:X]` render markers (`MEMORY_SOURCE_MARKERS`, compile-locked via `Record<MemorySource, string>`); it is what makes a web-page-derived memory distinguishable from durable user intent — the persistent-prompt-injection and hallucinated-authority channel the provenance arc closes. Tenth registered registry per `docs/doctrine/registry-pattern-canonical.md`. Wire-format compliance: snake_case identifiers, same convention as `EventType`. Same closed-registry / structural-lock shape as `check-suite-declared` (#10), `check-audience-canonical` (#46), `check-artifact-type-canonical` (#85), `check-sensitivity-canonical` (#97), `check-event-type-canonical` (#99), `check-settlement-mode-canonical` (#100), `check-agent-revocation-reason-canonical`. Doctrine: `docs/doctrine/memory-provenance.md`.",
  ...
  {
    name: "check-ceiling-from-grant",
    defends:
      "the ceiling-provenance invariant \u2014 THE ENFORCER'S CEILING COMES ONLY FROM A VERIFIED GRANT (spec/standing-delegation-v1.md \u00a73.3 rule 2; the produced-not-authored discipline applied to money; sibling of check-money-authority, which locks WHO may mint authority \u2014 this locks WHERE the HOW-MUCH comes from). Three assertions: (1) the sanctioned producer exists \u2014 packages/policy/src/grant-blast-radius.ts exports spendCeilingFromGrant (the only wire\u2192enforcer ceiling mapping) and evaluateBlastRadius; (2) the sanctioned composition uses it \u2014 packages/runtime/src/money-meter.ts (the R4 AND-composition: gate-allow \u2227 meter-allow) derives its ceiling via spendCeilingFromGrant(, so every metered autonomous spend enforces the DELEGATOR'S signed spend_ceiling, never runtime config; (3) no unsanctioned enforcement call sites \u2014 outside the enforcer module, the persistent SqliteGrantSpendStore, the meter, and tests, no production file may call evaluateBlastRadius( or .tryConsume( \u2014 an ad-hoc call site is exactly where a config-sourced or hand-built ceiling would slip in; (4) the RAIL seam is bound only through wrapP2pPaymentWithMeter \u2014 late-bound money tools (ToolDefinition.moneyBinding: 'late', e.g. delegate_to_agent whose spend materializes at quote time) are admitted by the loop on grant+meter PRESENCE, so the runtime must never hand the raw SovereignWalletRail payment builder to the delegation manager: the wrapper meters the delegator's TOTAL outflow (net + every fee leg) and refuses the broadcast on deny. A new reviewed composition extends ALLOWED_CALLERS in the gate with justification.",
    script: "check-ceiling-from-grant",
  },
  {
    name: "check-local-tool-gated",
    defends:
      "the surface-authority keystone — a local frontend is NOT an exception to the policy gate (`docs/doctrine/surface-authority-model.md` § 'The keystone invariant'). `MotebitRuntime.invokeLocalTool` (the explicit user-affordance entry point web/desktop tap to run filesystem/shell tools) once called `this.toolRegistry.execute(name, args)` directly with no policy check — the first symptom of the two-authorities drift the keystone forbids (finding h, surfaced by the 2026-06-10 audit; the call already signed a ToolInvocationReceipt AFTER execution, auditing a side-effect the gate never sanctioned). Scoped to the invokeLocalTool method body, the gate asserts: (1) `this.policy.validate(` runs BEFORE `this.toolRegistry.execute(` (the side-effect never fires ahead of the decision); (2) the decision is honored — a hard-deny guard (`!decision.allowed`, blocks regardless of origin) and an approval guard (`decision.requiresApproval`) sit between validate and execute; (3) the money invariant holds — the approval guard references `RiskLevel.R4_MONEY` (R4 never satisfiable by a bare tap, only a verified standing grant clears it) and the tap-satisfies-approval rule keys on `\"user-tap\"` + `classifyTool(` (a genuine deterministic affordance IS the human approval for R0–R3, per `docs/doctrine/surface-determinism.md`). Same ordered-marker-scan shape as `check-money-authority` (#123) and `check-affordance-routing` (#28).",
    script: "check-local-tool-gated",
  },
  {
    name: "check-runtime-host-election",
    defends:
      "the surface-authority keystone at the PROCESS level — one sovereign runtime per machine (`docs/doctrine/daemon-desktop-unification.md`). Two node processes that each `new MotebitRuntime(...)` against the same `~/.motebit` share one identity key + one SQLite file and become two uncoordinated policy/signing/receipt authorities (finding-h as a process race; the four-uncoordinated-runtimes ground truth the arc was commissioned to end). The election (`electRuntimeHost` + surface wrappers `electCliRuntimeHost`/`electDesktopRuntimeHost`) makes the first process the coordinator and every other an attached frontend, so authority is single by construction. The gate scopes to the shared-machine node hosts (`apps/cli/src` + `apps/desktop/src`; per-device web/mobile/spatial and the deployed molecule-runner are out of scope by construction — own device, own storage, no shared socket) and is a closed registry of construction sites, sibling-aligned both directions: every `new MotebitRuntime(` file must be registered (unregistered ⇒ a new entry point skipped the election — the 'no silent new authority' lock), no stale entries, and each `elected` site's named `electionVia` file must reference an election symbol (construction and election deliberately need not co-locate — the REPL constructs in runtime-factory.ts but elects in index.ts; deleting the election call fails here). `delegate.ts` (one-shot `motebit delegate --plan`) is elected coordinator-role for its lifetime — a transient runtime is still a full signing/receipt authority while it lives, so it binds via electCoordinatorRole before the shared DB opens and releases on completion, or refuses honestly when a coordinator is live (closed 2026-06-12; was the 'one-shot subcommands attach' residual registered exempt at gate-landing). Increment 5 of the daemon-desktop unification arc; the gate the doctrine reserved. Invariant #126, added 2026-06-12",
  ...
  {
    name: "check-surface-controller-adoption",
    defends:
      "surface controllers extracted to `@motebit/surface-kit` (state+actions shared across flat surfaces) are consumed from the package by every adopting surface — never re-forked locally. First controller: `McpManager` (was `MobileMcpManager` / `SpatialMcpManager`, identical lifecycle bar storage + naming). Per-(controller × surface) the gate asserts the surface's controller file imports the canonical class from `@motebit/surface-kit` AND stays a thin adapter (line ceiling) so logic cannot creep back in and re-fork. Closed-registry shape: `ADOPTIONS` is the inventory; adding a controller to the package or a surface to its adopters is one entry. Out of scope by intent (documented, not silent): desktop's stdio-superset MCP manager (reconciled in a follow-up) and CLI (no MCP manager). This is the synchronization-invariant defense for the surface-controller-extraction arc — the third leg (code + drift gate + doctrine) per `feedback_legibility_ratio`. Doctrine: the surface-kit extraction closes the per-surface controller-divergence window the audit named.",
  ...
  {
    name: "check-felt-interior-honesty",
    defends:
      "the two structural honesty invariants of the felt-interior binding (docs/doctrine/felt-interior.md, spec/consolidation-mutation-manifest-v1.md). (1) Coverage is never faked: the felt projection (packages/panels/src/memory/felt-consolidation.ts) never hard-codes `mutationsCoveredBySignature: true` — that field is honest only as the value verifyFeltCoverage computes from a cryptographically-verified ConsolidationMutationManifest (signature + receipt linkage + per-mutation content match); a literal true is a fabricated claim that a signed/anchored glyph covers the displayed sentences when it covers only the counts. (2) The owner-local manifest never leaks via sync: while the ConsolidationMutationManifest type exists, services/relay/src/redaction.ts MUST strip `mutation_manifest` from `consolidation_receipt_signed` events — SyncEngine.pushEvents syncs ALL events, and the manifest carries per-node content digests (dictionary-attackable) + sensitivity tiers that must never persist at or forward through the relay (only the counts-only receipt syncs). The third legibility-ratio artifact for felt-interior; caught a real ingress-leak in PR self-review (feedback_synced_event_payload_redaction). Invariant #127, added 2026-06-13",
    script: "check-felt-interior-honesty",
  },
  {
    name: "check-accrual-basis-canonical",
    defends:
      'the two structural honesty invariants of the felt-accumulation leverage register (docs/doctrine/felt-accumulation.md). (1) Produced-not-authored: an AccrualBasis is minted ONLY in the accrual source (@motebit/memory-graph recalledMemoryBasis), from the retrieval it actually performed — never authored by the model. The model-facing loop (packages/ai-core/src) THREADS a produced basis onto the turn result; it must never CONSTRUCT one, so no ai-core file may name an accrual-kind string literal (the construction fingerprint — same authorship rule as a `<memory>` tag carrying no `source=`). A model that could author `recalled_memory` could fabricate a recall that never happened. (2) No inward aggregate: "more capable over time" is felt as the lived FREQUENCY of leverage moments, never a counter/rate/streak/score (the §2 trend turned inward — the sybil-bait the felt family refuses), so AccrualBasis (packages/protocol/src/accrual.ts) and AccrualAttribution (packages/panels/src/memory/accrual-attribution.ts) declare no count/score/rank/rate/streak/total/aggregate/trend/delta/growth field. Plus the three-way registry lock (AccrualKind union × ALL_ACCRUAL_KINDS × gate reference). The Inc-5 lock of the felt-accumulation arc. Invariant #131, added 2026-06-22',
    script: "check-accrual-basis-canonical",
  },
  {
    name: "check-bond-address-binding",
    defends:
      "the load-bearing anti-sybil invariant of the commitment bond (docs/doctrine/commitment-bond.md, spec/bond-v1.md §2): a BondCommitment is valid ONLY if bonded_address == base58btc(bonded_public_key) — the agent's OWN sovereign identity address. Without it one wallet backs unlimited identities (post one bond, claim it backs thousands of fakes) and the bond is anti-nothing. Three arms: (1) BondCommitment (packages/protocol/src/bond.ts) declares both bonded_address and bonded_public_key; (2) the load-bearing scan — verifyBondCommitment's body (packages/crypto/src/artifacts.ts) compares bonded_address against base58btcEncode(...) and returns false on mismatch, fail-closed (the address derivation is re-checked inside the verifier, never trusted from the wire — the check-suite-dispatch sense, not vacuous self-consistency); (3) the law is specced as foundation law so an independent implementer enforces the same rule. Phase 1 of the commitment-bond arc (Inc 1). Invariant #132, added 2026-06-23",
    script: "check-bond-address-binding",
  },
  {
    name: "check-bond-surface-honesty",
    defends:
      "the cardinal surface rule of the commitment bond (docs/doctrine/commitment-bond.md, spec/bond-v1.md §7): a bond is an anti-sybil SIGNAL, never recourse, so no counterparty/operator-facing surface may frame it as secured funds or guaranteed recourse. Phase-1 honesty is load-bearing — a bond gives a wronged party nothing back, so words like secured/guaranteed/escrow/protected/recourse/covered/'your money back' next to bond status would manufacture a confidence the protocol cannot deliver (sibling of check-public-fee-claims on the fee path). Two arms: (A) surface scan — no curated user-facing surface line (packages/panels/src, apps/web|mobile|desktop|operator|spatial) may conjoin a BOND token with a forbidden framing on the same line (vacuous today — bond surfaces are deferred, so the guardrail is in place BEFORE the first surface); (B) doc anchor — spec §7 surface-honesty law + the doctrine human design-review checkpoint must be present, because the word-list is necessary-not-sufficient (it cannot catch a compliant-but-misleading layout, so a recorded human sign-off on any bond surface is required). Phase 1 of the commitment-bond arc (Inc 3). Invariant #133, added 2026-06-23",
    script: "check-bond-surface-honesty",
  },
  {
    name: "check-sync-token-freshness",
    defends:
      "the rotating-token freshness invariant on the web surface (@motebit/panels rule 3 — web authenticates to the relay with a rotating 5-minute JWT, `createSyncToken()` mints `exp = now + 5min`). Every long-lived HTTP sync adapter constructed in `apps/web/src/**` (`HttpEventStoreAdapter`, `HttpPlanSyncAdapter`, `HttpConversationSyncAdapter`) MUST take `credentialSource` (re-mints per request) — never a static `authToken` string captured once at bootstrap. A polling adapter handed a single 5-min JWT ships an expired `aud:sync` signature on every request past minute 5; the relay reads it as device-not-authorized and 403s, silently wedging cross-device sync for the session. Motivating incident 2026-06-15: HttpConversationSyncAdapter + HttpPlanSyncAdapter were each built with `authToken: token` while their event-store/WS siblings already used a per-request credentialSource — a textbook sibling-boundary miss that landed the fix on two of four sync adapters; the conversation poll 403'd on /sync/:id/conversations after 5 min (confirmed by decoding the live token: iat..exp spanned exactly 300s, the request fired ~60s past exp). WEB-ONLY by construction: desktop/CLI authenticate with a long-lived static `syncMasterToken` (panels rule 3), mobile uses none, so scanning other surfaces would false-positive on their legitimate static-token strategy. WebSocketEventStoreAdapter is excluded — it carries the distinct valid pattern of a 4.5-min refresh timer that reconstructs with a freshly minted token. Invariant #128, added 2026-06-15",
    script: "check-sync-token-freshness",
  },
  {
    name: "check-browser-surface-buffer-polyfill",
    defends:
