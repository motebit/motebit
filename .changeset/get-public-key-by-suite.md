...
`getPublicKeyBySuite(privateKey, suite)` — new MIT primitive for suite-dispatched public-key derivation. Closes a real protocol-primitive-blindness violation in the CLI and plugs the regex hole that let it slip past `check-suite-dispatch`.
...
- **MIT export allowlist updated.** `getPublicKeyBySuite` added to `MIT_ALLOWED_FUNCTIONS["@motebit/crypto"]` in `scripts/check-deps.ts`.
