...
reachable on the one exit path nobody has to ask for. All three paths now consume
this canonical computation instead of re-deriving it. Withdrawal-only by
construction: `debitSpendable` still consults the dispute hold alone, so grant
credit stays spendable on inference.
