...
| **SP 800-63-4**      | Ed25519 cryptographic verification provides strong agent authentication. Key storage in hardware-backed OS keychain (when available) and PBKDF2-protected key files. SP 800-63-4 defines AAL for human authentication; Motebit's agent-to-agent authentication is complementary — an enterprise could bind agent identity to a human AAL2/AAL3 credential via the `owner_id` field and operator mode PIN. |
