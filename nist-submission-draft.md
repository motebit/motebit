...
| **SP 800-63-4**      | Ed25519 cryptographic verification is architecturally aligned with AAL2+ assurance levels (formal conformance not yet assessed). Key storage in hardware-backed OS keychain (when available) provides phishing-resistant authentication. Key rotation via signed succession records preserves identity continuity without centralized revocation. |
