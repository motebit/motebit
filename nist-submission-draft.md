...
- **Revocation:** No centralized revocation authority by design. This is a deliberate tradeoff. **When rotation is possible** (the owner has access to the old key): the succession record cryptographically transfers identity continuity to the new key. The old key is tombstoned. Relays re-register the new public key. **When rotation is not possible** (the old key is lost or compromised without access): the identity is abandoned. A new identity is generated. Trust records, credentials, and memory from the old identity do not transfer — the new identity starts from zero. This is the cost of avoiding centralized revocation infrastructure: there is no authority that can revoke on your behalf, which means there is also no authority that can be compromised, coerced, or experience downtime. In enterprise deployments, organizations can layer their own revocation policy (e.g., OCSP, CRL) on top of the agent identity layer — the two compose without conflict.
...
| **SP 800-63-4**      | Agent authentication uses Ed25519 signature verification with per-session signed tokens (5-minute expiry, explicit scope). Private keys are stored in hardware-backed OS keychain when available (macOS Keychain, Android Keystore). Key rotation preserves identity continuity via dual-signed succession records without centralized revocation. Formal AAL mapping has not been assessed. |
