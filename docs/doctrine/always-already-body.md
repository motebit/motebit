...
- **Partially-instantiated perceptual body** — `peer_viewport`. The contract ships today (driver: peer, source: peer-receipt, consent: signed-delegation, in `EMBODIMENT_MODE_CONTRACTS`); receipt-grade body-shape is wired through slab-controller dissolve transitions and rendered in the desktop slab-items pipeline. Full viewport-grade rendering — a peer's slab visible AS a perceptual body, with the peer's identity-body register transmitting through — is partial. The receipt is body-shaped; the viewport is not yet.
- **Not-yet-instantiated bodies** — mobile's surface analog, spatial's primitive analog. Each will be its own perceptual body when implemented; each inherits the same physics chain.

The principle below applies to **all** bodies in the category. The slab is the worked instance because it's where the principle was first violated and corrected; the identity body already obeyed it (DROPLET.md §VIII: "It is one because it has not yet outgrown its surface" — the body's existence is sufficient unto itself, not contingent on content). Partially-instantiated bodies inherit the rule for the parts that are wired; not-yet-instantiated bodies inherit when they ship.
