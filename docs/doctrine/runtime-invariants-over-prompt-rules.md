...
The drift gate `check-typed-truth-perception` validates wire + prompt + dispatch presence; it does NOT validate that the producer actually populates the field. End-to-end data-flow tracing remains a reviewer-discipline check, not a mechanical one. If a future drift class warrants it, a producer-coverage gate becomes additive (every narration-class field's extractor has a unit test asserting "tag present → field populated"). Until then, the four-part naming is the discipline that prevents the gap from recurring.
