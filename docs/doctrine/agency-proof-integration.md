...
**Closed (was: known gap), surfaced by `check-receipt-conformance`.** `@motebit/state-export-client`'s `verifyReceiptDocument` used to report `integrity-only` offline and never compute the offline `sovereign` rung `@motebit/verifier` does. **Fixed:** `verifyReceiptDocument` now computes the offline (receipt-alone) `sovereign` rung through the same `@motebit/crypto.verifySovereignBinding` primitive the verifier uses — checked after the revocation poison-verdict and before the identity/anchor ladder, so it's the top rung and supplying anchor material can't downgrade it. The two surfaces now agree on integrity **and** the offline sovereign rung, and `check-receipt-conformance` asserts that agreement so it can't silently diverge again. Live-surface effect, shipped deliberately: receipt.computer's offline render now shows `sovereign` (not `integrity-only`) for sovereign receipts — correct, since a sovereign receipt is provable offline.

## 13. The consumed surface is wider than the receipt — and pins are transitive

§2 names the _receipt-verification_ surface. A 2026-07 consumer report corrected our model of what agency actually consumes: it grew beyond receipt verification into the **delegation/authority** primitives of `@motebit/crypto` — `signStandingDelegation` / `verifyStandingDelegation` / `signDelegation` / `isScopeNarrowed` / `computeTokenAuthority` / `verifyDelegationTokenVerdict` / `verifyReceiptVerdict`, plus keys/`did:key` and receipt signing — and the type-only `SeedEscrowPayload` / `SubjectBindingV1` from `@motebit/protocol`. It does **not** parse `TrustCredentialSubject`. `check-api-surface` already covers `@motebit/crypto` and `@motebit/protocol`, so the reciprocal semver guarantee (§2) already protects this wider surface — the gap was in our _communication model_, not the contract. Two invariants, learned from a live consumer's diff:

1. **Name the transitive `@motebit/crypto` delta in a release ping.** A `@motebit/verifier` patch that pins an exact `@motebit/crypto` version forces a consumer holding one-version-per-package to bump crypto too — on their **eager** signing/verifying edge, the riskiest hop of any cascade. For such a consumer the transitive crypto bump _is_ the release; name its delta (`3.15→3.17: additive, zero changed lines in delegation/scope/token paths`), not just the verifier's version string.
2. **Aim non-breakage claims at the surface they actually consume**, not a plausible one. A true statement pointed at a type they don't parse (`capability_stats` leaves `TrustCredentialSubject` byte-identical) reassures no one and reveals a stale model.

The ceiling **tamper/strip/graft** integrity property that consumer pinned as `token-ceiling-*` conformance vectors is now explicit in [`spec/standing-delegation-v1.md`](../../spec/standing-delegation-v1.md) §3.3; `@motebit/crypto`'s `standing-delegation` tests already enforced all three vectors.
