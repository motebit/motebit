...
**Known gap (decided follow-up), surfaced by `check-receipt-conformance`.** `@motebit/state-export-client`'s `verifyReceiptDocument` reports `integrity-only` offline and does **not** compute the offline `sovereign` rung that `@motebit/verifier` does — the two surfaces agree on integrity (what the gate enforces) but diverge on the rung. The principled fix is for `verifyReceiptDocument` to compute the offline `sovereign` rung through the same `@motebit/crypto` primitive (`verifySovereignBinding`) the verifier uses, so the rung never means two different things. **Deferred, not ignored:** it changes a _live_ surface (receipt.computer's offline display would shift `integrity-only → sovereign` for sovereign receipts), so it ships as a focused `state-export-client` change with receipt.computer aware — not bundled into doc work. Until then, for the offline sovereign rung, use `@motebit/verifier`.
