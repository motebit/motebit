...
Hosted agent platforms have converged on five primitives. The same five primitives motebit has been building. The architectures are structurally identical. The difference is which layer owns the identity.
...
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Identity             | Ed25519 keypair plus hardware-rooted attestation chain (`packages/protocol`, `packages/crypto-{appattest,play-integrity,tpm,webauthn}`); user-owned, not account-bound. Promotion path documented in [`promoting-private-to-public.md`](promoting-private-to-public.md).   |
...
- **Sovereign**: identity is a cryptographic keypair the user owns. Memory is local-or-portable. Capabilities are signed when they cross trust boundaries. Scheduled work is a signed delegation receipt. Governance decisions produce auditable artifacts. Revoke nothing — the identity is mathematically the user's, the way an SSH key or a Bitcoin address is.

Both architectures work. They are not in tension on the architecture. They are in tension on **who owns the layer below the agent**. Motebit's thesis is that this layer should be sovereign, the way TCP/IP and SMTP and the file system are sovereign — primitives no vendor can revoke.
