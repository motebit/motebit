...
2. **Time-bounded.** Every delegation has a TTL after which it transitions to the terminal `expired` state (§4). Terminal states are irreversible — once expired, a delegation cannot be revived; the delegator must issue a fresh one. This is the architectural mechanism for "delegations don't live forever." (Two sibling revocation mechanisms exist: at the credential layer, documented in [`spec/credential-v1.md`](../../spec/credential-v1.md) §6, applied when credentials underlying a delegation are revoked; and at the delegation layer for **standing grants** — a `StandingDelegation` ([`standing-delegation@1.0`](../../spec/standing-delegation-v1.md)) is long-lived but revocable via a signed `DelegationRevocation`, the offline-verifiable terminator for grants that mint short-lived per-tick tokens. A plain per-tick `DelegationToken` still relies on its short TTL, not revocation.)
