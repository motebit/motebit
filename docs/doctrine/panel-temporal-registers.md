...
**Soft cap at fire boundary, never mid-LLM-call abort.** The cap is evaluated _between fires_, not during a fire. Once a fire starts, it runs to completion — the in-flight LLM call streams to its natural end and the resulting tool-side-effects, response, and outcome row are written atomically, even if this fire's `tokensUsed` pushes the cumulative total over the cap. After the fire commits, `spent_tokens` rolls up; if cumulative ≥ cap, status flips to `budget_exhausted` and the next tick skips. This is the Apple-grade pattern (Reminders / Focus complete the in-flight act gracefully and defer the next one) and the calm-software pattern: a shattered fire — half-written email, half-made tool call, no outcome row — costs more user trust than a one-fire overshoot bounded by the existing per-run wall-clock + tool-call caps (10 minutes + 50 tool calls in `apps/desktop/src/goal-scheduler.ts`; equivalent in mobile and web). The cap **commits the user**, it doesn't **trip motebit mid-thought**.

**Iteration-boundary wind-down is the future bounding hook.** v1 ships the fire-boundary check only; one fire can overshoot the cap by an amount bounded only by the wall-clock + tool-call ceilings. v1.1 adds a `wind_down: true` signal between agentic-loop iterations in [`packages/ai-core/src/loop.ts`](../../packages/ai-core/src/loop.ts) — when the loop is between tool calls and `(this-fire's running token total) ≥ (remaining budget at fire-start)`, the loop finalizes with whatever it has rather than starting another tool-call iteration. Bounds overshoot to roughly one tool-iteration's worth (a few thousand tokens) instead of unbounded. Doesn't shatter, doesn't abandon work, just stops _extending_ the fire. Not yet implemented; named here so a future contributor reading the doctrine doesn't reach for mid-LLM-call abort as the bounding mechanism — that path is closed.
...
- [`scripts/check-panel-registry-coverage.ts`](../../scripts/check-panel-registry-coverage.ts) — drift gate (#96, landed 2026-05-14) that binds the registry to the surfaces. For each `SIDE_RAIL_PANELS` id × each flat surface (web / desktop / mobile), the gate verifies a concrete mount-site fingerprint exists in a known consumer file. Closes the gap that exposed itself when the panels-doctrine cluster shipped: the registry was a documentation-only artifact until this gate forced every surface to prove it mounts every panel. Adding a 7th panel to the registry without mount-site entries in the gate is itself a CI failure (sibling-alignment). Mobile component naming was canonicalized to the plural registry id (`ConversationsPanel`, not `ConversationPanel`) in the same arc — the gate's first violation on landing.
