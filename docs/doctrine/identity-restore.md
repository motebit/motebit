...
- **`preserveMemories=false` is the default on every surface.** The checkbox is shipped disabled with a "coming soon" label until the re-key migration (preserve_not_implemented) lands. Per design call #3: clear-by-default; the opt-in path is additive, not the spine.
...
- **Restore from recovery seed** — no file picker. User pastes seed → derive public key → **synthesize** minimal `ImportedIdentityMetadata` with a fresh motebit_id (`crypto.randomUUID()`), bornAt = now, empty devices, default governance/memory → type-to-confirm → `restoreIdentity(...)` with `originalContent: undefined`. Desktop clears the stale `_identity_file` from config; bootstrap regenerates one on next launch.
