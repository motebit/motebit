...
- **Catch-up consolidation** — the idle-tick only fires while the process runs and the user idles ≥ the quiet window, so a short-session user who never idles (or closes the app) never consolidates. Add an elapsed-since-`_lastConsolidationCycleAt` catch-up trigger (on startup / first idle) so accumulated state consolidates even without a long idle window, gated by the same quiet-window + abort-on-user-message as the idle path.
