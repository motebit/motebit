...
**Catch-up for short sessions.** The idle-tick only fires while the process runs and the user idles past the quiet window — so a user who never idles, or closes the app before the first tick, would never consolidate. `MotebitRuntime.start()` therefore also fires a one-shot `catchUpConsolidationIfOverdue()`: it runs a single cycle when consolidation is enabled and none has run within the catch-up window (`proactiveCatchUpMaxAgeMs`, default one hour), read from the persistent `ConsolidationCycleRun` event log so "recently" survives process restarts. This is **not** a second loop — `consolidationCycle()` remains the only proactive loop; it is one invocation fired at startup, and the cycle it runs is fully abortable (a user message flips presence to responsive and yields it). On a startup catch-up `_lastUserMessageAt` is still null, so the reflection novelty gate skips reflection — catch-up consolidates accumulated aged episodics and enforces retention without a spurious reflection call.
