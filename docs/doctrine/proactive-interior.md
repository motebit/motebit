...
The unification of `runHousekeeping` is also deferred, but the deprecation is now formalized rather than allowlist-only: both `runHousekeeping` (the function) and `MotebitRuntime.housekeeping()` (the wrapper method) carry four-field `@deprecated` annotations (`since 0.2.0, removed in 1.0.0`) per the deprecation-lifecycle doctrine. Drift-defense #39 (`check-deprecation-discipline`) now tracks the sunset; IDEs show the deprecation at every one of the ~32 call sites across `apps/{cli,web,mobile,spatial}` + runtime tests. The **semantic blocker** for full retirement is curiosity-target computation — `runConsolidationCycle`'s prune phase supersedes housekeeping's prune/retention/decay/episodic work cleanly, but curiosity-target computation (still in `runHousekeeping`'s second pass via `findCuriosityTargets` from `@motebit/memory-graph`, fed to `gradientManager.setCuriosityTargets`) is not yet covered by the cycle. Unifying it is its own design question (does curiosity belong in `gather` alongside reflection + notability ranking, or stay a separate signal the gradient manager subscribes to?) and retiring `housekeeping()` before answering would either bake a wrong choice into a protocol-shape primitive or silently drop a behavior callers depend on. The 1.0.0 horizon gives that question room to land deliberately.
