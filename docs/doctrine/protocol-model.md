...
1. **Protocol** — the open spec anyone can implement. Published in [`spec/`](../../spec/). Consumed via the permissive-floor type packages (Apache-2.0 today: `@motebit/protocol`, `@motebit/sdk`, `@motebit/crypto`, `@motebit/verifier`, and the canonical platform-attestation leaves `@motebit/crypto-appattest`, `@motebit/crypto-android-keystore`, `@motebit/crypto-tpm`, `@motebit/crypto-webauthn`; plus the deprecated `@motebit/crypto-play-integrity` for one minor cycle). A third party reading only the specs and the permissive-floor types can build an interoperating alternative without permission. The architectural role is "permissive floor"; the specific license instance (Apache-2.0) is replaceable.
