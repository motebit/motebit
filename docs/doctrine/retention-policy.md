...
**Sub-note (phase 4b-3, locked 2026-05-01).** The handshake from phase 4 is operationalized under **Path A quorum** — verifier hard floor of `witnessed_by.length ≥ 1` when `federation_graph_anchor.leaf_count ≥ 1` (self-witnessed admitted when `leaf_count = 0`); soft accountability via `WitnessOmissionDispute` filed within **24 hours** of `cert.issued_at`. Solicitation deadline is **10 seconds** per peer (per-request timeout = overall solicitation deadline since `Promise.allSettled` parallel fan-out). Path A composes with the existing `DisputeResolution` adjudicator (`spec/dispute-v1.md` §6.2 + §9.6) — no new adjudication primitive — and preserves cert format stability. The wire format ships under `motebit/relay-federation@1.1` §15; the dispute integration under `motebit/dispute@1.0` §9.6. Higher quorum thresholds are reversible without wire break (verifier-policy tightening only, `witnessed_by[]` already supports any cardinality).
...
**Extension (phase 4b-3, locked 2026-05-01).** From phase 4b-3 onward, certs from federation-aware deployments MUST carry the `federation_graph_anchor` field. The `EMPTY_FEDERATION_GRAPH_ANCHOR` sentinel (`leaf_count: 0`, `merkle_root` = hex SHA-256 of zero bytes) is the canonical self-witnessed encoding for relays with no peers. **Pre-4b-3 certs without the anchor field are grandfathered self-witnessed** — they remain valid through signature alone. The TS field stays `?` optional for grandfathering; verifier policy enforces presence-when-peered, not presence-always. The empty-tree merkle-root sanity check (`@motebit/crypto::verifyDeletionCertificate` rejects `leaf_count = 0` carrying any root other than the empty-tree value) closes the loophole exactly the size of "self-witnessed encoding": without it, decision 9's structural-not-self-declared principle had a hole at the case where it matters most (no witnesses to enforce).
...
`scripts/check-retention-coverage.ts` (drift-defense #67, landed 2026-04-30 — phase 1 reserved slot #52, but post-doctrine renumbering occupied that slot before this gate shipped; the next free invariant number is the durable assignment). Enumerates every runtime-side store with a `sensitivity` column or settlement obligation; asserts each registers a `RetentionShape` in `RUNTIME_RETENTION_REGISTRY` (`packages/protocol/src/retention-policy.ts`); fails CI on omissions. Bidirectional drift check: stale-registry (registered store with no matching CREATE TABLE) and unregistered-table (CREATE TABLE with `sensitivity` column not in the registry) both fail. Same enforcement pattern as `check-consolidation-primitives` (#34) and `check-suite-declared` (cryptosuite agility). Runtime-scoped: the relay's deployment doesn't host these stores; its retention manifest declares `out_of_deployment:` for them by design.

### Sensitivity ceilings as interop law

The 5-tier `SensitivityLevel` enum (`none` / `personal` / `medical` / `financial` / `secret`) is the closed registry every retention-shape ceiling, every egress-write floor, and every cross-implementation gate derives from. The set must be agreed-upon byte-identical across motebit implementations or the cross-device sensitivity contract breaks (device A persisting at `medical` must mean the same thing to device B's session-tier filter). `scripts/check-sensitivity-canonical.ts` (drift-defense #97, landed 2026-05-14) is the four-way structural lock that holds the enum, `ALL_SENSITIVITY_LEVELS` (`packages/protocol/src/sensitivity.ts`), `SENSITIVITY_RANK` (same file), and the gate's own reference mirror in lockstep — a tier insertion is intentional protocol-level work across all four sites. Per-consumer coverage: every load-bearing dispatcher MUST either enumerate every tier as a literal OR route through the canonical algebra primitives (`rankSensitivity`, `maxSensitivity`, `sensitivityPermits`) exported from `@motebit/protocol`. The algebra is the protocol's escape hatch from per-level dispatch chains, sized for code that doesn't need to read what the tiers _are_ — only how they _rank_. The graduation of `packages/policy-invariants/src/computer-sensitivity.ts` from a fourth-shaped local `LEVEL_RANK` + `higherLevel` table to the protocol algebra shipped in the same arc as the gate's landing — closing the un-graduated fourth-instance debt the protocol's `sensitivity.ts` JSDoc had documented as "past trigger." Same closed-registry / structural-lock shape as `check-suite-declared` (#10), `check-audience-canonical` (#46), `check-artifact-type-canonical` (#85), `check-routing-decision-coverage` (#95), `check-panel-registry-coverage` (#96).
...
| 4a + 4b-1 + 4b-2 | Event-log horizon. Horizon certificate format, signed truncation protocol, four storage adapters. **Shipped** at `44d25cd8`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 4b-3             | Federation co-witness solicitation + `WitnessOmissionDispute` accountability layer. Path A quorum (verifier hard floor of 1, soft accountability via 24h dispute window). `EMPTY_FEDERATION_GRAPH_ANCHOR` self-witnessed encoding + empty-tree merkle-root sanity check. `relay_horizon_certs` + `relay_witness_omission_disputes` tables. `advanceRelayHorizon` orchestrator with in-process advisory lock + re-snapshot on retry. Two `POST /federation/v1/horizon/{witness,dispute}` endpoints. Manifest projection of five operational ledgers under `append_only_horizon` with derived `witness_required`. Drift gate #68 (`check-relay-retention-coverage`). `relay-federation-v1.md` 1.0 → 1.1 (additive minor). `dispute-v1.md` §9.6 integration. **Shipped** across `374a9605` (commit 1, protocol) → `25ba9771` (2, crypto sign/verify) → `a2ce0373` (3, wire-schemas) → `9e808870` (4, relay endpoints + flow) → `a7f217c8` (5, manifest projection + #68) → commit 6 (this commit, spec bump + doctrine extension + `@experimental` → `@spec` promotion). |
| 5                | Conversations + tool-audit register under `consolidation_flush`. Per-turn sensitivity inheritance, flush phase added to consolidation cycle, settlement-floor resolver for tool-audit. Operator-panel "what's expiring" affordance — same five-step pattern as the latency-surface ship, this time over flush events. **Shipped** at `0233325e`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 6                | `/.well-known/motebit-retention.json` signed manifest + browser-side re-verifier (6a, shipped at `fda8dd08`). `check-retention-coverage` lands hard at drift-defense #67 (6b, shipped at `79dd6613`). `docs/drift-defenses.md` updated to #67. (4b-3 commit 5 graduated `RETENTION_MANIFEST_CONTENT.stores[]` from empty to enumerating the five operational ledgers; sibling drift gate #68 lands in the same commit.)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |

Phase 4 was the variance source. Phases 2–3 and 5–6 were tightly bounded; phase 4 split into 4a + 4b-1 + 4b-2 (no-peer horizon advance) and 4b-3 (federation co-witness solicitation, six-commit arc closing the doctrinal asymmetry between memory-erase enforcement and append-only-horizon enforcement).
...
- `spec/relay-federation-v1.md` §7.6 — sibling Merkle algorithm reference; same odd-leaf-promotion rule. §15 (added in 1.1) — wire format for the `/horizon/witness` + `/horizon/dispute` endpoints that operationalize Path A quorum from the decision 4 sub-note.
- `spec/dispute-v1.md` §9.6 — `WitnessOmissionDispute` as a dispute class within the existing lifecycle; same 24h/48h/72h ladder, same federation-quorum requirement, two clock gates (wall vs `cert.issued_at` + `filed_at` sanity bounded).
...
- `docs/drift-defenses.md` — `#39 @deprecated four-field contract` (decision 6a migration); `#67 check-retention-coverage` lands with phase 6b (the doctrine reserved #52 in phase 1; the slot was occupied during post-doctrine renumbering, so the gate shipped at the next free invariant number); `#68 check-relay-retention-coverage` is the sibling for relay-side store coverage (phase 4b-3 commit 5).
