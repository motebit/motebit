...
## Worker-to-relay authentication is self-signed

A worker authenticates to its relay with its own identity key — per-audience signed tokens via `RelayAuth` (`@motebit/mcp-server`), introduced by the public `POST /api/v1/agents/bootstrap` — never with the relay operator's master token. `MOTEBIT_API_TOKEN` unlocks the whole relay; a worker that holds it turns any container compromise into an operator compromise. Closed 2026-09-13 across all seven first-party workers; `check-worker-no-master-token` keeps every service outside the relay from reading the variable and every worker-side config from carrying an `apiToken` field. Doctrine: `docs/doctrine/task-admission.md` § "The worker authenticates as itself".
...
- **Desktop key custody.** Docs claimed the desktop key "never touches the filesystem / never plaintext / OS keyring." In fact the desktop key store is the owner-only (`0600` on Unix; Windows is best-effort) **plaintext** file `~/.motebit/dev-keyring.json` on **every** build, and no OS keychain is used: the `keyring` crate was compiled with no platform backend, so it was an in-memory mock whose writes "succeeded" and persisted nothing (key-file durability Finding B-0). Lane B of that work (#763) made the file the declared store and hardened it: R1 strict reads, R3 atomic owner-only writes, and R2 preserve-before-replace ([`docs/proposals/key-file-durability-v1.md`](../proposals/key-file-durability-v1.md)). The docs now state that posture. Mobile (Keychain/Keystore, no fallback) and the CLI (PBKDF2+AES-encrypted file) match their claims.
...
- **The desktop OS keychain: its own arc, [#764](https://github.com/motebit/motebit/issues/764).** Today the desktop key store is file-only, because the keyring crate had no backend (above). Enabling the real keychain is gated on real-device testing against the entry criteria in [`docs/proposals/key-file-durability-v1.md`](../proposals/key-file-durability-v1.md) ("The split"). Three builds found provider behaviours that break a naive design: a read that fails while a write succeeds; a Linux bus with no Secret Service provider; and a locked KeePassXC database answering "not found", so on a real keychain absence requires agreement, never a bare "not found" alone. The old "verified-keyring-only writes on signed builds, `0600` fallback for dev builds" item is **superseded**: it assumed a working keyring backend that never existed. **Trigger:** the arc's entry criteria pass on the founder's signed Mac, Linux (gnome-keyring, KeePassXC) and Windows.
- **Per-device revocation — the cascade half SHIPPED (#702); the kill-switch half still open.** This was deferred on the reasoning that identity-key rotation and per-device keys are distinct axes, and that rotating the identity _shouldn't_ rotate independent device keypairs. That reasoning stands and is preserved — but it had been applied to the whole `devices` table, which conflates two things. A row holding the **identity key** is not an independent device keypair; it is the same key, written down in the place `verifySignedTokenForDevice` reads FIRST. Left there, it shadows the registry key and the rotation ends nothing — the retired key keeps authenticating, which is the one thing a rotation exists to stop. The named trigger (a device-compromise threat scenario) is exactly what rotation is for, so a recorded succession now moves the rows that held the retired identity key, in the same transaction as the registry and the chain, and clears any pairing approval carrying that key (a bearer-less route would otherwise write it back). Rows holding a **different** key — a device linked without key transfer — are untouched, which is the original decision, now enforced narrowly instead of by inaction. Both doors that record a succession (`/rotate-key` and the succession path of `/agents/register`) apply it through one writer (`succession-apply.ts`), so "recorded but not applied" is unrepresentable rather than a state a second route must finish; a retry of the chain HEAD skips freshness and key-on-file (the relay accepted that exact record once) and runs the same idempotent, retired-key-scoped writes, while a rotation back to a previously used key is refused (409 `reuses_key`), as is re-applying any recorded non-head link (`replays_recorded_link`) and departing from a key the chain has already retired (`departs_from_retired_key`): an identity's key history never repeats a key, on either side of a link (#775, `spec/identity-v1.md` §7.5). **Still open:** there is no per-device kill-switch short of a full-identity `/revoke` — revoking ONE machine without rotating the identity remains unbuilt. **Trigger:** a multi-device-revocation requirement (one lost laptop, identity key uncompromised).
...
- **Obvious secrets in an unmarked live conversation — SHIPPED (the credential-class half).** The sensitivity egress gate (`assertSensitivityPermitsAiCall`, every AI-call entry, `check-sensitivity-routing`) blocks a cloud call when the **effective session** sensitivity is medical/financial/secret — but session sensitivity is user-set + sensitive-slab-source, with **no content auto-classification of the user's own live message**. Tool results and memory storage were already deterministically secret-redacted (`packages/policy/src/redaction.ts`); the user's typed input was not, so a credential typed into an _unmarked_ cloud session reached the model. Closed by `SecretRedactingProvider` (`packages/runtime/src/secret-redacting-provider.ts`), wired at `MotebitRuntime.setProvider`: it masks the **credential-class** subset (`RedactionEngine.redactForCloudEgress` — API/AWS keys, bearer tokens, JWTs, PEM keys, seed phrases, connection strings, passwords) from `ContextPack.user_message` + user-role `conversation_history` before they reach a **non-sovereign** provider. No-op on a sovereign (on-device) provider; the local transcript is untouched; the model almost never needs to _see_ a raw credential (agents use keys via the credential/tool path). **Deliberately NOT redacted (user-controlled by design):** SSN + card numbers (financial/PII a user often legitimately uses → set the session tier instead), bare base64 (false-positives), and **medical** (inherently not pattern-detectable — no regex for a diagnosis). The public claim was corrected to match (`apps/docs/content/docs/security.mdx`). The redaction is not silent: each firing emits a content-free `EventType.SecretRedactedFromEgress` audit event (count + credential-class label names + provider mode, never the secret) — the privacy-egress sibling of `SensitivityGateFired`, so the inspector/event-log shows an observable trail. A real-time chat-surface notice that rides this event is a Ring-3 follow-on (surface-determinism — the runtime emits typed truth; surfaces render per-surface).

## Outbound URLs — every caller-influenced fetch is SSRF until proven otherwise

Four seams fetched URLs motebit did not author with nothing more than a `startsWith("http")`: the web proxy's `/v1/fetch`, the read-url atom and the local `read_url` tool (a model-driven fetch, i.e. prompt injection → request forgery), and the relay's persisted agent and federation-peer `endpoint_url`s, which the relay later contacts carrying a bearer. One law now sits in `@motebit/sdk` (`checkOutboundUrl` / `fetchPublic`): http(s) only, no credentials, never loopback / RFC 1918 / CGNAT / link-local incl. cloud metadata / multicast / reserved / `localhost` / `*.local` / `*.internal` / `*.home.arpa`, IPv4 smuggled inside IPv6 refused, a public NAME resolving to a private address refused where a resolver exists (Node), and every redirect hop re-checked (`redirect: "manual"`). The relay applies it at persist time (registration, peer proposal) **and** at connect time (`forwardTaskViaMcp`), because a registry row is old by the time a task arrives. `allowPrivateNetwork` is the local-development allowance (`MOTEBIT_ALLOW_PRIVATE_ENDPOINTS` on the relay, `MOTEBIT_ALLOW_PRIVATE_URLS` on the CLI, `createTestRelay`); it is never set on a deployed service.

Stated limits, so no consumer over-claims: DNS rebinding between resolve and connect needs a connect-time pin in the transport, not a policy check; an edge runtime with no resolver checks literals and names only.
