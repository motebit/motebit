...
- **The reported route is a route that exists.** A record semiring's `add` is component-wise, so the value it assigns to a worker reachable two ways is the best trust of one route beside the best cost of the other — a per-dimension summary, not a hire. Ranking on it can prefer a worker for a combination no route delivers, and a transcript that reports it testifies to a hire that never happened (found 2026-09-14 by an external read of the docs page, confirmed live: the relay's `explainedRankCandidates` receives `relay_delegation_edges` as peer edges). Both rankers (`rankReachableAgents` in `@motebit/semiring`, `graphRankCandidates` / `explainedRankCandidates` in `@motebit/market`) now rank over the **Pareto frontier semiring** (`pareto.ts`: elements are sets of (weight, path) pairs, ⊕ = union-then-prune-dominated, ⊗ = pairwise compose weight AND path) — the policy chooses AMONG real routes, and the chosen route's metrics are the chosen route's metrics. `routing_paths[0]` is that path — read as **evidence** (the recorded-delegation / federation path that justified hiring `selected_agent`), never as a record of execution; who did the work is proven by receipts. Two contract clauses travel with it (`pareto.ts` header): the policy must be monotone in the frontier's dimensions — so any input it reads is lifted into the weight (the market lifts chain hardware attestation as a sixth dimension, because the boost is applied after pruning and a five-dimension prune could discard the route the boost would have made the winner) — and the `maxPaths` bound is an approximation that keeps metrics honest but bounds optimality. The lexicographic composite is exact over keys quantized to 1e-3 (the band is the declared tie tolerance), not over raw scores. Same graph, same traversal; the semiring changed, which is the doctrine's own lever. Proof: the competing-path tests (one expensive trusted route, one cheap less-trusted route to the same worker; the selected route's reported metrics equal its own edges under either policy), the attestation-inversion test, and the brute-force frontier enumeration.
