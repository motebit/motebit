...
- **[liquescentia-as-substrate.md](liquescentia-as-substrate.md)** — the deepest coherence of this doctrine. On AR glasses, the user's real world _becomes_ Liquescentia (the medium that makes the liquescent droplet legible — glass-like optical character against a real chromatic field). The synthetic chromatic gradient (`ENV_LIGHT`) is fallback; reality is the goal. `WebXRThreeJSAdapter` already drops the synthetic env when XR light estimation is available — code crystallized this before the doctrine named it.
