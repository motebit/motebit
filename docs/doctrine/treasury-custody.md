...
7. **Operator-runnable validation surface.** Two probes complement the runtime telemetry above, both backed by the same `/api/v1/admin/treasury-reconciliation` endpoint:
   ...
   - **Paid-flow companion `motebit smoke x402`** (Base Sepolia by default, `--mainnet` for ~$0.0105 USDC against Base mainnet). Bootstraps fresh buyer + worker motebit identities + EVM EOAs, posts a paid listing, drives a real x402 settlement through the relay's gate + facilitator + receipt path, and prints the next reconciliation-cycle observation timestamp. Validates the write side that `smoke reconciliation` later observes — paired together, they exercise the full economic loop end-to-end.

   The two smokes compose: run `motebit smoke x402` to drive a real paid settlement, wait one reconciliation interval, then run `motebit smoke reconciliation` to confirm the cycle observed the new fee. Mainnet operators verify the just-shipped infrastructure with this pair; the buyer EOA is operator-funded once (~$0.05 USDC) and reused across runs.
