...
**Stage 2 — split into two independent closures.** The original conflation was a doctrine drift: "wire-format spec" and "onchain anchor" were both bundled under one trigger ("second operator"), but they answer different questions and clear different gaps. Splitting them:
...
- **Stage 2b — `spec/relay-transparency-v1.md` (still deferred).** A wire format for the signed declaration: field schema, canonical JSON shape, suite registration, anchor format, succession rules. Lands once a second motebit-compatible operator forces the question of _"what fields must we standardize?"_ Codifying it before then would be guessing rather than observing. Stage 2a's anchor shape (`motebit:transparency:v1:{hash}` memo) becomes part of this spec when it lands.
