...
### Proven from the relay's own record: auth events (2026-09-14)

Declared posture said the master token is the operator's and workers never hold it; the retirement arc (#649, #656) made that true. Proving it afterwards failed on evidence: Fly's log window ended before the deploys, so the relay could not show whether anything still presented the token. `relay_auth_events` (`services/relay/src/auth-events.ts`) closes that: every master-token presentation and every refused signed token is recorded by the relay itself — kind, method, path, the token's claimed `motebit_id`, expected audience, rejection reason, correlation id; never token bytes, never IP — for a 30-day rolling window, readable at `GET /api/v1/admin/auth-events` and summarized on `/api/v1/admin/health`. Declared in `DECLARATION_CONTENT.retention.auth_events` and rendered into `PRIVACY.md`, so the record is itself part of the declared posture. The disappearance test holds: remove the table and the claim "only the relay uses the master token" reverts to testimony.
