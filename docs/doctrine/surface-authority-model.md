...
This is a required deliverable of the unification arc, not doctrine poetry: an explicit **host-election rule** (which process becomes coordinator, how a second process detects it and attaches, how a contended election resolves to exactly one, and the desktop-installed-but-not-running case) — with the failure to specify it _being_ the two-coordinators-drift failure mode below. Its specific protocol is grounded at build time against the real keyring-sharing, not frozen here. _Trigger to decide:_ the first machine running both the desktop app and the CLI daemon against one identity.
