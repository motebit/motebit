...
**Status:** in progress. PR1 part 1 (the `MerkleTreeVersion` registry) landed at commit `9cf876a8`; PR1 part 2a (node-tag plumbing) at `ebd3ff56`; PR1 part 2b (leaf-tag primitive + `tree_hash_version?` wire field + dispatch gate `check-merkle-tree-hash-canonical` + registration closure) at `19d1584b` — all three autonomous and zero-behavior-change. PR2 (first real v2 producer — agent-settlement) landed at `63abfe53`; PR3 (second v2 producer — credential-anchor) at `c39ddbf4`. PR4 (third v2 producer — identity-log) returns for review.
...
- Federation-settlement (§2, PR5+) is the natural merge point for the deferred item-4 convergence (`agent-settlement-anchor-v1.md` §9.1) — the federated leaf (`computeSettlementLeaf`, today on the gate's documented-exclusion list) becomes a verbatim-artifact hash under v2 in the same pass.
