...
**Status:** in progress. PR1 part 1 (the `MerkleTreeVersion` registry) landed at commit `9cf876a8`; PR1 part 2a (node-tag plumbing) at `ebd3ff56`; PR1 part 2b (leaf-tag primitive + `tree_hash_version?` wire field + dispatch gate `check-merkle-tree-hash-canonical` + registration closure) at `19d1584b` — all three autonomous and zero-behavior-change. PR2 (first real v2 producer — agent-settlement) landed at `63abfe53`; PR3 (second v2 producer — credential-anchor) at `c39ddbf4`; PR4 (third v2 producer — identity-log) at `aabbecf9`. PR5 (fourth v2 producer — consolidation-anchor) returns for review.
...
- **Next: PR5 — consolidation-anchor is the fourth v2 producer (returns for review).** The first runtime-side producer. `CONSOLIDATION_TREE_HASH_VERSION` in `packages/runtime/src/motebit-runtime.ts` emits `merkle-sha256-rfc6962-v2`; `anchorPendingConsolidationReceipts` routes each leaf through `canonicalLeaf(r, v2)` and builds via `buildMerkleTree(leaves, v2)`; the emitted `ConsolidationAnchor` carries `tree_hash_version` on the wire object itself — **no migration, no DB column** (the anchor object is the persisted event-log record, self-describing and verified standalone). **Gate touches TWO arms** (unlike PR4): the runtime producer joins `LEAF_BUILDERS`, and `consolidation-receipt-v1.md` §4.6 + Option-A frontmatter (`tree_hash_producer: packages/runtime/src/motebit-runtime.ts`) activates a third assertion-4 declaration. Backward-compat automatic: legacy anchors absent ⇒ v1.
...
- Federation-settlement (§2, PR6) is the natural merge point for the deferred item-4 convergence (`agent-settlement-anchor-v1.md` §9.1) — the federated leaf (`computeSettlementLeaf`, today on the gate's documented-exclusion list) becomes a verbatim-artifact hash under v2 in the same pass, then leaves the exclusion list and closes the arc.
