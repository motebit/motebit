...
**Status:** in progress. PR1 part 1 (the `MerkleTreeVersion` registry) landed at commit `9cf876a8`; PR1 part 2a (node-tag plumbing) at `ebd3ff56`; PR1 part 2b (leaf-tag primitive + `tree_hash_version?` wire field + dispatch gate `check-merkle-tree-hash-canonical` + registration closure) landed in this commit — all three autonomous and zero-behavior-change. PR2 (first real v2 producer — agent-settlement) returns for review.
...
- PR1 complete: part 1 (registry, `9cf876a8`), part 2a (node-tag plumbing, `ebd3ff56`), part 2b (leaf-tag + wire field + gate + registration closure, this commit). The agility infrastructure is in place and dormant — v1 is byte-identical everywhere; no producer emits v2 yet.
- Gate-enforcement mechanism resolved at 2b: **Option A** (machine-readable spec frontmatter `tree_hash_version:` + `tree_hash_producer:`). The arm is dormant in `check-merkle-tree-hash-canonical` — no spec declares it until PR2 wires the first v2 producer's spec.
- **Next: PR2 — agent-settlement is the first v2 producer (returns for review).** The producer (`services/relay/src/anchoring.ts`) emits `tree_hash_version: "merkle-sha256-rfc6962-v2"`; deploy-verifier-first ordering; KAT vectors confirm producer/verifier symmetry; the agent-settlement spec gains the §9 "Tree-hash version" subsection + the Option-A frontmatter that activates the gate's assertion (4).
- Federation-settlement (§2, PR3+) is the natural merge point for the deferred item-4 convergence (`agent-settlement-anchor-v1.md` §9.1) — the federated leaf (`computeSettlementLeaf`, today on the gate's documented-exclusion list) becomes a verbatim-artifact hash under v2 in the same pass.
