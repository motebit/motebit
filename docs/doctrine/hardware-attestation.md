...
Scoring is identical across every platform adapter — Secure Enclave, TPM, App Attest, Play Integrity all collapse to the same `1.0 / 0.5 / 0.1 / 0.0` scalar under the bottleneck semiring. The scalar answers "hardware-backed?" yes/no; the platform identifier answers "which hardware?" for audit, not for rank.
...
- **Android Play Integrity.** Play Integrity lands in a subsequent pass behind the same result shape. Android surfaces today inherit `platform: "software"` claims from the CLI-shape path.

- **Chain-of-trust verification for Secure Enclave.** The SE public key is the self-asserted root in v1. App Attest and TPM adapters already verify the platform's own attestation chain (Apple App Attest root CA; Infineon, Nuvoton, STMicro, Intel PTT TPM vendor roots via `@motebit/crypto-tpm`'s `DEFAULT_PINNED_TPM_ROOTS`). The SE chain-of-trust landing is tracked alongside persistent-SE keys.
