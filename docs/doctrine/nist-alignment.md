...
**First non-relay-state-export consumer shipped 2026-05-14.** The goal-results Phase 3 arc landed `goal-result` as the 13th entry in the closed `ContentArtifactType` registry — the first content-artifact category whose producer is the **agent's motebit identity** rather than the relay. Per-fire goal artifacts (the content motebit produced when a goal's cadence fired) sign at fire-time via the runtime's new `signGoalArtifact` helper (`packages/runtime/src/motebit-runtime.ts`); the signed manifest is stored alongside the artifact bytes on the goal record and is offline-verifiable through the same `motebit-verify content-artifact` CLI (`--expect goal-result` resolves automatically via `ALL_CONTENT_ARTIFACT_TYPES`). The registry's stated semantic — "content-artifact category for C2PA-shape provenance" — generalizes from "relay-assembled state-export bundle" to "any motebit-signed standalone artifact," with the producer-identity dimension orthogonal to the category. Doctrine: [`goal-results.md`](goal-results.md) §"Phase 3 — slab annotation + turn-id capture + manifest signing". The drift gate `check-artifact-type-canonical` enforces the registry mirror; the `check-state-export-signed` gate scopes only to relay state-export endpoints and correctly excludes the goal-fire signing path (which lives in the runtime, not the relay).
