...
This is the same move money made — a coin is its transaction history, not a thing. Applied to an agent: the actor is the valid passage from state to state, not the stuff carried between states.

## The pipeline is one object, not seven features

Motebit's seven surfaces are usually read as a sequence:
...
- **Fork:** copying is not a threat to _dissolve_; it is a transition to _declare_. Descendants are lineage-distinct with explicit authority partitions — never silent claimants to one authority. Identity survives substrate change exactly when the change is a validly authorized, witnessed, bounded transition (see [identity-binding-verification](identity-binding-verification.md), [identity-restore](identity-restore.md)).
