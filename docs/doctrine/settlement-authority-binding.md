...
- **Inc 1 — the federated derived+sovereign binding — SHIPPED.** The residual close: at the federated worker leg (`federatedP2pIntent`, `tasks.ts`) the peer-forwarded key must sovereign-bind to the worker's `motebit_id` AND the address must derive from it (`isDerivedSettlementBinding` in `@motebit/wallet-solana` + `verifySovereignBinding`), else the paid federated leg is refused. The local leg is deliberately untouched (write-authorized). Zero new artifact, zero migration; closes the peer-redirect for the sovereign-by-default worker, fail-closed otherwise.
