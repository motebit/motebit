...
`BridgeClient` interface is injected (glucose): `createTransfer()`, `getTransfer()`, `isReachable()`. Configurable poll attempts and interval. Lives in `packages/settlement-rails/src/bridge-rail.ts`. Registered at relay startup when `BRIDGE_API_KEY` + `BRIDGE_CUSTOMER_ID` env vars are set.
