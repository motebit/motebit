...
| **1 holder you control**       | Verify if stripping is _valuable_, not just _cheap_.      | `services/relay/src/migrations.ts` migrations were a 1-holder case. "Squashable" was true (motebit.com at `max(version)=15`, verifying was one SQL call); squashing was still wrong because append-only immutability is a stronger invariant than a shorter file. See the 2026-04-24 squash-and-revert postmortem below.                                                                                                                                                                                                                                                                                                                                                                                                    |
| **N uncontrolled holders**     | Deprecate first, strip at a named sunset date.            | `verifyIdentityFile` + `LegacyVerifyResult` in `@motebit/crypto` — a thin pre-discriminated-union wrapper deprecated under this row's assumption (presumed external npm consumers). A pre-removal holder check before the 3.0.0 crypto major found **zero real holders** — desktop calls its own same-named method (which routes to `verify`), mobile aliases `verify` directly — so it removed cleanly in 3.0.0 (2026-06-06), like `migrateLegacyProvider` in `@motebit/sdk` before it (reclassified zero-holder by an npm-download survey: 2 downloads of the `motebit` CLI, zero external `@motebit/sdk` consumers; stripped in `adc87d19` on 2026-04-23). Replacement: `verify(content, { expectedType: "identity" })`. |
| **Protocol holders (forever)** | **Never strip** unless the protocol itself is superseded. | `spec/migration-v1.md` + `services/relay/src/migration.ts` (cross-relay agent migration). Any third-party implementer reads this spec. Permanent protocol vocabulary.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
The 2026-04-24 squash-and-revert on `services/relay/src/migrations.ts` (15 → 1 → 15 across commits 21a29fc4 → revert) is the canonical example. The doctrine entry that called it out as a cleanup target treated "1 holder, cheap audit" as permission to squash; it should have required an argument for squash being a win on its own architectural terms. At 15 migrations, that argument didn't exist.

## How to apply at motebit (as of 2026-04-23)

- **Relay DB migrations** (`services/relay/src/migrations.ts`): 1 controlled holder. The squash target has been rescinded. Keep the chain append-only; new schema changes land as `v16`, `v17`, … appended below. Revisit only if the chain reaches hundreds of migrations or CI migration-apply time becomes measurable.
- **Config-shape migrators** — `migrateLegacyProvider` in `@motebit/sdk` was this row's canonical example through 2026-04-23. An empirical npm-download survey reclassified it zero-holder (no external `@motebit/sdk` consumers, `motebit` CLI at 2 downloads); stripped in `adc87d19`. The bucket itself is still live in principle; the other former example, `verifyIdentityFile`, was likewise reclassified zero-holder and removed in the 3.0.0 crypto major (2026-06-06).
...
- **Protocol-level cross-relay migration** (`spec/migration-v1.md`, `services/relay/src/migration.ts`): permanent. Never a cleanup target.
