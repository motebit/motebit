...
| **N uncontrolled holders**     | Deprecate first, strip at a named sunset date.            | `verifyIdentityFile` + `LegacyVerifyResult` in `@motebit/crypto` — thin pre-discriminated-union wrapper retained because desktop, CLI scaffold, and external npm consumers still call it. Named replacement (`verify`) exists; `@deprecated since 0.8.0, removed in 2.0.0`. A pre-1.0 candidate for this row, `migrateLegacyProvider` in `@motebit/sdk`, was empirically reclassified zero-holder (npm download survey: 2 downloads of the `motebit` CLI, zero external `@motebit/sdk` consumers) and stripped in `adc87d19` on 2026-04-23. |
