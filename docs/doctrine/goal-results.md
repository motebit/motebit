...
**Deferred from Phase 3, not deferred indefinitely:**

- Desktop + mobile **manifest persistence**. The runtime's `signGoalArtifact` helper is identity-shaped; the schedulers on those surfaces don't yet wire it into their durable goal_outcomes rows. The follow-up commit adds a `signed_manifest TEXT` column to `goal_outcomes` on each surface, plumbs the schedulers, and projects `last_manifest_signed` from the latest outcome row so the receipt-summary row above renders the "signed" indicator identically on web / desktop / mobile. No new doctrine; mechanical cross-surface mirror.
