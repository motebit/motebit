...
- Desktop + mobile **manifest persistence**. The runtime's `signGoalArtifact` helper is identity-shaped; the schedulers on those surfaces don't yet wire it into their durable goal_outcomes rows. The follow-up commit adds a `signed_manifest TEXT` column to `goal_outcomes` on each surface, plumbs the schedulers, and surfaces the same "Signed" indicator on the card. No new doctrine; mechanical cross-surface mirror.
