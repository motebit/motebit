...
**Phase-3 deferral close — SHIPPED 2026-05-14 across the three FLAT surfaces (not the daemon; see the correction below).** Desktop's `tauri-migrations` v4 + mobile's `expo-sqlite-migrations` v24 both added a `signed_manifest TEXT` column to `goal_outcomes`; their schedulers now call `runtime.signGoalArtifact` at fire-time and persist the manifest JSON alongside the artifact bytes. The SQL / projection layer on each surface derives `last_manifest_signed` as `(signed_manifest IS NOT NULL on the latest COMPLETED outcome)`, threading into `ScheduledGoal.last_manifest_signed` on the panels-runner contract. The receipt-summary row reads identically on web / desktop / mobile — calm-software degradation when identity isn't loaded or the signer throws (NULL → no indicator). The `@motebit/protocol` `GOAL_RESULT_ARTIFACT` registry entry that Phase 3 added now has three independent signing consumers per the closed `ContentArtifactType` family.

**Correction — the daemon was never in "all surfaces" (2026-09-16).** "All surfaces" meant web, desktop and mobile; `apps/cli/src/scheduler.ts` was in neither the drift gate's registry nor its allowlist, and the shared `@motebit/persistence` schema never gained the `signed_manifest` column its per-surface siblings added. So the one surface that fires goals with nobody watching signed nothing, kept a 500-character summary, and discarded the artifact it had just produced — leaving the motebit's own word as the only record of unattended work. Migration #47 adds `response_full` + `signed_manifest` to the shared schema, the daemon signs and keeps the whole result, and the gate's registry now names four surfaces with its aperture stated in its own header. The lesson is the gate's, not the scheduler's: a green scanning gate claims only as much as the set it looks at.
