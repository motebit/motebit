...
The relay's transmitter surface for user funds is **zero on the out-flow direction** after the off-ramp arc (Arc 1). Worker earnings under `settlement_mode='p2p'` flow direct delegator→worker with the 5% fee composed as a separate delegator→treasury leg in the same atomic Solana multi-output tx (Arc 2, shipped). What remains open: the in-flow direction for `settlement_mode='relay'` worker earnings — delegator-paid funds still flow through operator-custody virtual accounts on that path. Arc 3 (trust-graduation re-architecture, making P2P the default for all worker earnings) closes the in-flow surface entirely. This doctrine memo is honest about what each commit structurally enforces and what stays open until the next arcs land.
