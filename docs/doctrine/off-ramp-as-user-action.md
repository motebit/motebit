...
The relay's transmitter surface for user funds is **zero on the out-flow direction** after the off-ramp arc (Arc 1). Worker earnings under `settlement_mode='p2p'` flow direct delegator→worker with the 5% fee composed as a separate delegator→treasury leg in the same atomic Solana multi-output tx (Arc 2, shipped). Arc 3 shipped the type-level scaffolding and policy-level disjunctive eligibility gate for the in-flow direction (`WritableSettlementMode = Extract<SettlementMode, "p2p">` + `SettlementEligibility` disjunctive return shape + established-pair OR new-pair-with-acknowledgment eligibility). **What remains open**: the operational submission gate that rejects paid direct delegation without a `payment_proof` is deferred to Arc 3.5 ("migrate test-suite + production delegator clients to P2P-by-default"). Until Arc 3.5 lands, the existing relay-custody path remains active for paid direct delegation under three documented carve-outs (self-delegation, zero-cost delegation, multi-hop sub-receipts) — none of which constitute third-party bank-shape transmission. This doctrine memo is honest about what each arc structurally enforces and what stays open until the next arc lands.
...
2. ~~**Arc 3 — Trust-graduation re-architecture.**~~ **PARTIAL — type-level scaffolding shipped; structural operational enforcement deferred to Arc 3.5.** The disjunctive eligibility gate (established-pair branch via trust + interactions, OR new-pair branch via `delegator_acknowledges_no_history_risk`) shipped — `evaluateSettlementEligibility` now returns the disjunctive `SettlementEligibility` shape (allowed → `mode: "p2p"`; disallowed → no mode field). `WritableSettlementMode = Extract<SettlementMode, "p2p">` is exported from `@motebit/protocol` and consumed at the eligibility result type. The doctrine intent is documented and the type-level scaffolding for "P2P is the only worker-earnings path" is in place. **Carve-outs that remain on the relay-custody path** (documented as deliberate exceptions, not bank-shape transmission): self-delegation (same-party flow), zero-cost direct delegation (no real funds), multi-hop sub-receipts (intra-relay coordination from parent's allocation). **Arc 3.5 — migrate test-suite + production delegator clients to P2P-by-default for paid direct delegation** — the operational submission gate (`TASK_P2P_PROOF_REQUIRED` — reject paid direct delegation without payment_proof) was prototyped during Arc 3 implementation but rolled back: it breaks 32 existing E2E tests that exercise the relay-custody path as legacy contract. Honest scope: migrating the test suite + the existing delegator clients to construct P2P payment_proofs for every paid direct delegation is its own bounded arc. The structural enforcement is type-defined (`WritableSettlementMode`) and policy-defined (disjunctive gate); operational enforcement at the submission boundary lands in Arc 3.5.
