...
- **Inc 3 — wire + provenance (NEXT).** Thread the signed `jti` seed and the stakes→strength mapping through the runtime's `WorkerSelector` closure so live delegations explore reproducibly; surface the decision in the sub-hop provenance ("_explored: sampled 0.82 > incumbent 0.79_") — discharging #326's deferred "surface the why". This is the flip that makes exploration real; like #326's unpin it is observationally inert until a **second provider of a capability** exists on staging.
