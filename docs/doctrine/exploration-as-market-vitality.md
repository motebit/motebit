...
- **Inc 2 — bond lifts ranking.** Thread verified-bond into `RankableWorker`; a bond raises exploration _priority_, distinguished in type and tests from a quality boost.
- **Inc 3 — wire + provenance.** Thread the signed `jti` seed and the stakes→strength mapping through the runtime's `WorkerSelector` closure so live delegations explore reproducibly; surface the decision in the sub-hop provenance ("_explored: sampled 0.82 > incumbent 0.79_") — discharging #326's deferred "surface the why".
- **Inc 4 — the end-game proof.** A deterministic bandit **simulation**: K delegators × T rounds, a hidden best worker + good/sybil newcomers. Assert sublinear regret / convergence to best, every _good_ newcomer promoted, sybil selection-prob → 0, high-stakes rounds never explore, bonded newcomer promoted faster.
