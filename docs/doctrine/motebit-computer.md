...
**Effective sensitivity composes session × tier-bounded modes.** The runtime's `getEffectiveSessionSensitivity` reads slab items in `tier-bounded-by-source` modes (`shared_gaze`, `virtual_browser`, `peer_viewport` — user-fed perception) AND `tier-bounded-by-tool` modes (`tool_result` — tool-fed perception). The thesis claim "medical/financial/secret never reach external AI" structurally covers five egress write boundaries: session-elevated state, drops, classified tool outputs, memory-write candidates, and conversation-message persistence. `all-tiers` modes (`mind`, `desktop_drive`) don't contribute — those are interior or sovereign-by-definition. Items leave their tier behind when the user dismisses them; the gate is state-based. The conversation-message boundary is closed on both sides: the write-side floor stamps each persisted message at `max(default, effective)` so cross-device sync carries true provenance tier, and the read-side filter in `ConversationManager.trimmed()` excludes any message tagged above the current effective tier from the AI-context window. The filter is dynamic — a session whose tier elevates mid-conversation regains access to its own elevated messages; a session at None excludes Secret messages even when load-bearing for the current turn. Untagged legacy messages flow through unchanged for backward compat. Trimmed history is itself an egress shape, and the filter closes the bypass where a Secret-tagged message persisted on device A would surface to device B's None-tier session whose pre-call AI gate sees only `None × None`.
...
**v1 implementation status (2026-05-08):** all six modes have shipped v1 cuts; v1.1 / v1.2 / v1.5 refinements have landed since. `mind` and `tool_result` are universal/automatic via the runtime's built-in slab emission. `desktop_drive` ships on the desktop surface via the Tauri Rust bridge (`apps/desktop/src-tauri/src/computer_use.rs` — `xcap` capture + `enigo` injection + macOS Vision OCR). `shared_gaze` ships across all surfaces via the drag-drop perception substrate (URL/text/image drops in `packages/runtime/src/perception.ts`). **`virtual_browser` v1** is `CloudBrowserDispatcher` in `@motebit/runtime` (the second `ComputerPlatformDispatcher` producer, sibling to the desktop Tauri bridge, reusing the `computer-use-v1` wire format verbatim) talking to a Playwright-driven Chromium executor at `services/browser-sandbox` — deployed to Fly staging at `motebit-browser-sandbox.fly.dev`. The slab renders screenshot observations as image-srcdoc fetch cards (`apps/web/src/ui/slab-items.ts`, mirrored on desktop). Consent is session-scoped for ordinary browser operation + per-action approval for irreversible clicks (Submit / Buy / Pay / File / Send / Permanently delete / I agree / Authorize / Upload — heuristics in `@motebit/policy-invariants`); sensitivity classification runs against the screenshot bytes on every observation, not just the URL. See `spec/computer-use-v1.md` §8.1. **`peer_viewport` v1** stamps the mode explicitly at `delegation_start`, renders the `delegation_receipts[]` chain as per-hop rows on the slab card with signed-checkmark indicators (✓ when a hop returned a verifiable signature, — when absent), and fills the in-flight body with the `Waiting for $peer…` wait-state instead of leaving it empty during the outbound interval. Federation + signed receipts already shipped earlier; the v1 pass is the slab-side rendering refinement that surfaces the chain shape. **Shipped post-v1:** **v1.1** per-dispatcher mode stamping (cloud-browser → `virtual_browser`, desktop → `desktop_drive`) + `normalizeEmbodimentMode` runtime validator + drift gate `check-computer-dispatcher-modes` (#79). **v1.2** `ComputerSessionManager.halt()` / `resume()` / `isHalted()` user-floor primitive per spec §3.3. **v1.2b** two-finger-hold-on-plane gesture on the slab + `/halt` / `/resume` slash commands — three triggers compose the same `user_preempted` boundary. **v1.5** `ComputerSessionReceipt` — every session crystallizes at close into one signed artifact (counts, sensitivity envelope, halt history, JCS-canonicalized SHA-256 over the per-action structural roll-up); sibling of `ToolInvocationReceipt`, signed under `motebit-jcs-ed25519-b64-v1`, self-verifiable with the signer's public key alone. **Still deferred to v1.5+:** promotion of `motebit/computer-use@1.0` from `@alpha` to `@beta` (waits for real-traffic soak on the deployed cloud executor); HA badge / trust-score live composition / bead-arrival animation on `peer_viewport` cards.
...
| Tint (always)         | derived from current soul color, on `attenuationColor`; carried at idle and during work alike | Body coherence: the slab is an organ of the motebit, so it carries the soul tint always — same as the creature's iris keeps its soul color whether the creature is idle or working. An organ that switches identity based on activity reads as a tool the body holds, not a part of it.   |
| Glow (variable)       | soul color on `emissive`; faint baseline at idle, brightening with `activeWarmth`             | Activity is visible as light, not as identity. The slab brightens when the motebit is working and sits at a faint baseline when idle (alive, not extinguished). Peak intentionally gentle so the slab never outshines the creature.                                                       |
