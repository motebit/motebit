...
**v1 implementation status (2026-05-07):** four of six modes ship today. `mind` and `tool_result` are universal/automatic via the runtime's built-in slab emission. `desktop_drive` ships on the desktop surface via the Tauri Rust bridge (`apps/desktop/src-tauri/src/computer_use.rs` — `xcap` capture + `enigo` injection + macOS Vision OCR). `shared_gaze` ships across all surfaces via the drag-drop perception substrate (URL/text/image drops in `packages/runtime/src/perception.ts`). The two partials are `virtual_browser` and `peer_viewport`. **`virtual_browser` v1 begins with `CloudBrowserDispatcher` in `@motebit/runtime`** — the second `ComputerPlatformDispatcher` producer, sibling to the desktop Tauri bridge, reusing the `computer-use-v1` wire format verbatim against an isolated Chromium session (see `spec/computer-use-v1.md` §8.1). The dispatcher seam landed first; the cloud executor (`services/browser-sandbox`, Playwright-driven) is the next slice and the second-producer artifact that promotes `motebit/computer-use@1.0` from `@alpha`. Consent is session-scoped for ordinary browser operation + per-action approval for irreversible clicks (Submit / Buy / Pay / File / Send / Permanently delete / I agree / Authorize / Upload — heuristics in `@motebit/policy-invariants`); sensitivity classification runs against the screenshot bytes on every observation, not just the URL. `peer_viewport` viewport-grade rendering of in-flight delegations is a separate near-term pass; federation + signed receipts already ship.
