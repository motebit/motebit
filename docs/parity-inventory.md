...
| A6  | `transparency-declaration.ts` · `SignedTransparencyDeclaration.content` | `readonly content: unknown` — **required** (`protocol/src/transparency.ts:83`; it is part of the signed hash payload `{spec, declared_at, relay_id, relay_public_key, content}`, so always present) | `content: z.unknown()` → inferred **`content?: unknown`** (optional — zod makes `z.unknown()` keys optional) (`transparency-declaration.ts:71`) | **2**                           | Surfaced by cold review (the parity probe's reverse arm): `{content?}` is not assignable to `{content}`, so the schema accepts an absence the protocol forbids. NOT a suite-pin — it is the **reverse** failure on `transparency-declaration`, distinct from the forward suite-pin (B-suite-2). Fix the schema to require `content` (zod `z.unknown()` optionality is the wart). Sibling audit: other `z.unknown()` sites live inside `.record()` values (`credential-bundle`, `plan-lifecycle`, `dispute`, `skill-manifest`, `witness-omission-dispute`) where key-optionality does not surface — this is the only top-level instance. |
