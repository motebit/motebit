...
## 2. Relay Deployment (`services/relay/`)
...
| `SOLANA_RPC_URL`                                             | Conditional | --             | Master switch for six pipelines: credential / federation-settlement / per-agent-settlement / revocation / transparency-declaration onchain Merkle anchoring + Solana P2P settlement verifier loop + Stripe crypto onramp routes. Set to `https://api.mainnet-beta.solana.com` for free public access. Relay signs memo program submissions with its own Ed25519 identity → Solana address is derivable from the relay pubkey (see Public identifiers below); that address requires SOL for tx fees. |
| `CDP_API_KEY_ID`                                             | Conditional | --             | Coinbase Developer Platform key ID. Required when `X402_TESTNET=false` (mainnet) — relay fails fast at boot with `X402ConfigError` if unset (per `services/relay/CLAUDE.md` rule 15).                                                                                                                                                                                                                                                                                                               |
| `CDP_API_KEY_SECRET`                                         | Conditional | --             | CDP key secret. Pairs with `CDP_API_KEY_ID`. JWT-bearer signed per-request by `@coinbase/x402`'s exported `facilitator` config.                                                                                                                                                                                                                                                                                                                                                                     |
| `BRIDGE_API_KEY`                                             | Conditional | --             | Bridge.xyz orchestration off-ramp API key. **Bridge rail registers in `/health/ready` only when BOTH this AND `BRIDGE_CUSTOMER_ID` are set.**                                                                                                                                                                                                                                                                                                                                                       |
| `BRIDGE_CUSTOMER_ID`                                         | Conditional | --             | Bridge customer ID (operator-scoped). Currently NOT set in production despite `BRIDGE_API_KEY` being staged — Bridge rail not registered until this is added.                                                                                                                                                                                                                                                                                                                                       |
| `RELAY_PROXY_SECRET`                                         | Conditional | --             | Shared secret between relay and `services/proxy` for token-minting endpoint authentication. Required for proxy → relay calls in motebit-cloud routing.                                                                                                                                                                                                                                                                                                                                              |
| `MOTEBIT_FEDERATION_ALLOWED_PEERS`                           | No          | --             | Comma-separated motebit_id allowlist for federation peers.                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `MOTEBIT_FEDERATION_BLOCKED_PEERS`                           | No          | --             | Comma-separated motebit_id blocklist for federation peers.                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `MOTEBIT_FEDERATION_REQUIRE_DISCOVER_SIGNATURE`              | No          | `false`        | Strict per-hop sender signing on inbound `/federation/v1/discover` (relay-federation@1.3 §4.1). Flip to `true` only after EVERY mesh peer runs the signing producer — see issue #188.                                                                                                                                                                                                                                                                                                               |
| `FEATURED_SKILL_SUBMITTERS`                                  | No          | --             | Comma-separated motebit_ids whose skills appear featured in `/api/v1/skills`.                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `STRIPE_CLOUD_PRICE_ID`                                      | Conditional | --             | Stripe price ID for the motebit-cloud subscription product (consumed by `services/relay/src/subscriptions.ts`).                                                                                                                                                                                                                                                                                                                                                                                     |
| `MOTEBIT_TREASURY_RECONCILIATION_INTERVAL_MS`                | No          | `900000` (15m) | Treasury reconciliation cadence. See `SECRETS.md §9` Phase 1 step 6 for the reconciliation primitive's full doctrine.                                                                                                                                                                                                                                                                                                                                                                               |
| `MOTEBIT_TREASURY_RECONCILIATION_CONFIRMATION_LAG_BUFFER_MS` | No          | `300000` (5m)  | Reconciliation confirmation-lag buffer — excludes settlements newer than this from the recorded-fee-sum to respect chain reorg horizon.                                                                                                                                                                                                                                                                                                                                                             |
