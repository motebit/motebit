...
### Signed discover — default flips strict on 2026-07-21

`POST /federation/v1/discover` requests carry a per-hop sender signature since `relay-federation@1.3` (spec §4.1.1). During the rollout the receiver default is **tolerant** (`requireDiscoverSignature: false` — an unsigned discover from an un-upgraded neighbor is accepted with a `federation.discover.unsigned` warning). **On 2026-07-21 that default flips to `true`**: an unsigned inbound discover rejects with `403`. Run `relay-federation@1.3` or later (it always _signs_ its outbound discovers) and your peers will keep interoperating across the flip with no action. If you must keep accepting unsigned discovers — e.g. you peer with a relay still on < 1.3 — set `requireDiscoverSignature: false` explicitly in your federation config; you then own the cold-audit P0-3b fail-open for your own mesh. This is announced, not silent, per the relay's deprecation discipline.
