...
- **`sockets_open` is never persisted.** GET computes it from `connections`: the count of sockets bound as `(device_id, bound_under)` that the relay **believes** open (a half-open socket counts until #691). **`host_sockets_open`** (added after review of #806) is the subset announcing `unattended_runtime` — the one predicate (`livenessKeyOf`) that also writes the row and guards the sweep. Two quantities, never one field: `sockets_open` says something is attached (a retired machine with a desktop open still reads connected); `host_sockets_open` says the host is running. The host count is `motebit doctor`'s hint for a copied `device_id`.
- **Retention.** The TTL sweep on `last_seen_at` (proposed: 90 days) skips any row with an open host socket (a row kept only by a non-host socket is swept; that socket is then served in `live_unenrolled` with `host_sockets_open: 0`). GET serves the retention window and `observing_since`, so a consumer can say "not observed in the last 90 days" rather than "never seen". The doctrine's liveness bullet and spec §8 are amended in the same PR, and the declaration block is rewritten, not reused.
...
{ motebit_id, enrollments, retirements,
  liveness: { observed_by, retention_days, observing_since,
              rows: [{device_id, bound_under, last_seen_at, sockets_open, host_sockets_open}],
              live_unenrolled: [{device_id, bound_under, sockets_open, host_sockets_open}] } }
```
