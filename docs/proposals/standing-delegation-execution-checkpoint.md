...
**Status:** SIGNED OFF 2026-07-04 — all four decisions (D1 relay-coordinated v1 / D2 semantics ratified, USD-micro pinned / D3 `spend_ceiling` @1.2 additive field / D4 settlement-time revocation + 7–30d money-grant lifetime) approved by Daniel Hakim as recommended. Inc 3a (wire: `spend_ceiling` @1.2 + relay delegation-revocation cache) AND Inc 3b (execution wiring: `delegation` presentation ingress → `verifyGrantForTurn` producer path; `VerifiedGrant` carries signed nonce + ceiling; loop-level R4 AND-composition via `meterMoneyAction`; `createMoneyMeter` + `SqliteGrantSpendStore` persistent accumulator; gate `check-ceiling-from-grant` #136; `revoke-then-self-mint-offline` red-team fixture; relay acceptance-time revocation fence `TASK_GRANT_REVOKED`) both shipped same day. Inc 4 (grant tooling: `motebit grant create/list/show/revoke` + `--grant` per-turn presentation, `~/.motebit/grants/` verbatim-artifact store, revocation propagation to the relay cache) shipped 2026-07-05. **The first grant was minted 2026-07-05** — the founder's own motebit, self-delegation, $5.00 lifetime / 7-day / daily 1h ticks — and verified through the live chain (signature, ceiling-in-signed-body, `verifyGrantForTurn` slot-0 acceptance, same-day refusal of a future-dated tick). The arc this checkpoint governs is complete: every axis signed off here is now enforced machinery exercised by a real delegator-signed artifact. First metered spend follows when a `pay`-shaped tool with explicit `amount_micro`/`counterparty` enters the registry.
