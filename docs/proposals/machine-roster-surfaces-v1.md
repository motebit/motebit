...
## 1B. Round-2 amendments (normative; the founder decided B1)

- **B1: legacy identities get a custody flag (founder decision).**
  - **The problem.** For a legacy id whose relay holds no proven key, round 1's `classifyHeldKey` never reaches `identity`, so counts would stay suppressed for good. That is 29 of 50 production identities (2026-09-26).
  - **The rule.** Each surface stores a local **custody flag**, `holds_identity_key: {motebit_id, public_key, set_at, reason}`. It is set only by the two code paths that establish custody:
    1. The surface itself **mints** the identity: it generated the keypair and the id together at first launch.
    2. A Link Device **with key transfer completes**, meaning the transferred seed now sits in the key slot.
  - A pairing without a transfer never sets it; neither does a restore of another identity or a rotation. A rotation **moves** the flag: on `commit`, if the flag names the old key, it is rewritten to the new key.
