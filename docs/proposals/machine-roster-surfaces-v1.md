...
- **Cadence.** N is 10 minutes (`PRESENT_EVERY_MS`). The digest is SHA-256 over the replica's sorted canonical entries. A presentation counts as fully taken when nothing is left to retry (`roster_full` ids are permanent and count as taken, and so, since #802, are entries refused for their own bytes: `too_large`, `malformed`, `wrong_motebit`, `bad_signature`; see clients-v1's C-1 build notes).
...
- **The stored motebit.md gives succession records, and nothing else.** Unlike the browser, the phone keeps an identity file (`@motebit/identity_file`). A file proves possession, not identity (§1A F2): it is self-signed, so anyone who can write it can make it say anything. `localSuccession` therefore uses it only when its signature verifies, it names this motebit_id, **and its current key is the key the phone holds**. A file signed by any other key contributes nothing. Its records are self-verifying; they are evidence for the resolver, never a classification on their own.
  - **Where this helps.** A rotated sovereign identity roots offline through the file's link even when the relay has lost its chain. This holds only for the phone's **first-launch** identity: first launch writes the file, and a rotation re-signs it. Nothing rewrites it on a pairing, and a restore does not store one. So after a pairing the stored file still names the first-launch id and contributes nothing.
- **No guardian pin on the phone (#799 W1).** `pinnedGuardian` is always null. Nothing on the phone writes a guardian from a trusted source. A guardian read from the self-signed file let that file's author sign a guardian recovery from the genesis key onto a device-only key: the chain then rooted, the key classed `identity`, the section showed a false count, and a retire was signed with the device key and posted. **Stated cost: R6's remedy (pin the guardian by restoring the identity file) is not available on the phone.** A guardian-recovered sovereign identity stays `unconfirmed` here: lines are shown, with no count and no acts.
...
- **Disposal.** The roster is keyed by (motebit_id, device_id) and disposed on a pairing that switches identity, on stop and on restore. A disposed roster presents nothing and refuses acts.
  - **Restore.** The roster is disposed, and no new one is created, _before_ the key slot is written, because from then on the slot may hold the restored identity's key while the process still runs as the old one. No roster exists until the app reloads.
  - **Stop.** No roster is created again until `start()`.
...
- **Four Tauri commands, not two** (`src-tauri/src/roster_replica.rs`).
  ...
  - `roster_lease_acquire(ttl)` / `roster_lease_release(token)` are the kit's `exclusive` (R4). The critical section is TypeScript (it signs and presents), so it cannot be one Rust command. The lease is an OS lock on `<file>.lease` held by the Rust process, with an owner token and a timeout: 120 s, clamped to 1–600 s. A process crash releases it through the OS. A lease held by a webview that reloaded is released by its timer, at most 120 s later with the default TTL. Until then the reloaded webview's own act polls for 30 s, then gives up with "locked by another desktop window" and fails closed: nothing is signed or sent (#801 F2).
  - All four commands are `#[tauri::command(async)]`, so they run on Tauri's async-runtime worker threads, never the main thread. The compare-and-swap's lock wait (at most 2 s) and its fsyncs therefore cannot stall the UI. A contended wait can occupy one worker thread for up to 2 s. Acquiring the lease never waits in Rust (`try_lock`); the polling is in TypeScript.
...
- **Retry-After is bounded (#801 F1, on all three surfaces).** Before this fix, one 429 with `Retry-After: 999999999` stored a `retry_until` decades out. The value is re-read at every entry point and kept at its maximum, so a surface would never present again. The rule now lives once, in the kit:
  - `MAX_RETRY_AFTER_MS` is 1 hour. `nextPresentationRecord` stores at most `now + MAX_RETRY_AFTER_MS`, and never carries forward an out-of-bound value it finds in the stored record.
  - `boundedRetryUntil(record, now)` is the read rule. `presentationDue` uses it, and so does each surface's `remember` (web, mobile, desktop). A stored value beyond `now + MAX_RETRY_AFTER_MS` could only have been written before the bound, or planted, so it reads as expired (0).
  - The out-of-bound value is dropped, not clamped to `now + bound`. It is re-read on every entry point, so a clamp relative to the time of reading would slide forward with each read and hold forever.
  - Stated cost: dropping errs toward presenting early. Presenting is idempotent, and the relay re-asserts any wait with a now-bounded `Retry-After`. A clock that jumps back more than an hour can have the same effect on a legitimate value.
...
- **Gate and shared names.** The desktop adapter is in `check-surface-controller-adoption` (346 lines against its 360-line ceiling, by the gate's count, once #800 moved the identity-file rule into the kit). `machine-roster.desktop.json`, `.corrupt-<t>`, `.lock` and `.lease` have a row in `key-file-durability-v1.md`'s shared-names table.
