...
- **R21: when the frozen verdict is taken.** Once the relay records the link, device rows move to the new key, and an old-key GET gets 401. So the frozen verdict is taken either **(a)** before the rotation POST, under the old key, and persisted inside the rotation write-ahead, or **(b)** after the local commit, under the new key, by reducing over the **pre-rotation chain** (the chain minus its new head). Never between the two. The hook also runs on the resume paths (`interrupted-commit-finished`, `already-held`).
