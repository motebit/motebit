...
- **C-1 build notes.**
  ...
  - **A local identity file counts only when the held key signed it** (#800; the CLI half of #799 W1). C1.4 and C1.5 read `motebit.md` from the working directory and every parent, and C-1 took records and the guardian from any self-signed file naming this motebit_id. A self-signed file proves possession of its own key, never that the key speaks for the identity: a file planted in a parent directory, signed by an unrelated key and naming a guardian that signed a recovery from the genesis key onto the held key, rooted that key, showed a count, and had a retire signed and posted. Now a file contributes records, and a guardian, only when it verifies, names this motebit_id **and its current key is exactly the key the CLI holds** (surface-kit's `boundIdentityFile`; the phone and desktop use the same rule through `identityFileRecords`). The rotation hook reads its link the same way, bound to the committed key.
    - **Why the CLI still pins.** A bound file was signed by the very key in hand, so its guardian is this key's own declaration, never a third party's; no file anyone else can write changes what the CLI resolves. The CLI normally holds the identity key (it mints the identity, and rotation re-signs its `motebit.md`), and a guardian-recovered identity roots only through this pin (C1.5), so dropping it would strand that case. The phone and desktop keep no pin because their stored file sits in a slot a restore or pairing writes without re-signing (C-2b, C-2c).
    - **Stated cost (lead decision, #800).** The rule cannot tell a legitimate guardian recovery from a holder of a device-only key D who signs its own file naming a guardian of its own choosing: the bytes are the same shape. That holder can make **its own** CLI view treat D as the identity key. No other reader follows: the phone and desktop never pin a guardian, and another CLI pins only from a file bound to its own held key, so every other reader reduces under its own chain, on which D does not appear. Retirements D signs are therefore unplaceable for everyone else (the relay stores them and never reduces). The stricter rule (pin only when the held key is already on the chain resolved without the guardian) was refused: it would strand real guardian recoveries on the CLI. The CLI is not gated by `classifyHeldKey` (R5).
    - **The kit verifies the file itself.** `boundIdentityFile` calls `verify` from `@motebit/identity-file` (a new surface-kit dependency, layer 2 → 3), never a verifier a surface hands in, so no surface can pass a weaker one.
  - **A refusal for the entry's own bytes is permanent** (#802; amends C5's "a 422 or 413 means not taken"). C5 made only `roster_full` permanent, so an entry the relay refuses `too_large`, `malformed`, `wrong_motebit` or `bad_signature` was re-presented forever, kept the count suppressed as an omission, and was worded "presented again". Each of those is decided from the entry's bytes (or, for `roster_full`, a cap the relay never lowers), so the same bytes are refused every time. The classification lives once, in surface-kit's `machine-roster-refusals.ts` (`RELAY_ENTRY_REFUSALS`, typed so a new reason must be classified).
    - **Permanent:** reported once (`PresentReport.willNotHold`, beside `rosterFull`), kept in the replica's new `relay_refused` list with its reason (absent in an older replica: none), never presented again, and excluded from the set difference, as R20 excludes `roster_full`.
    - **The count is not suppressed by it.** Membership is what the sovereign signed, and the relay only stores. A declared refusal is not the silent omission R27 guards against, and suppressing on it would suppress forever, with no remedy but retiring a real machine. The line stays a member, and the view adds a note: "the relay will not hold N entries this device holds (reasons); kept here and counted, not presented again". Stated cost: another surface that never held the entry does not see it, which is spec §9's residual, as for `roster_full`.
    - **An unknown reason is retryable.** Re-presenting is idempotent, and a retryable entry stays in set-pinning, so the count stays suppressed. Reading a word this surface does not know as permanent would stop replicating the entry for good and drop it from the omission check.
    - **Oversized entries go alone.** An entry past the per-entry bound (4096 bytes of canonical JSON, spec §11) is presented in its own request, so a 413 for the request size never takes its neighbours. A 413 on that single request is read as `too_large`, and only then: the local size check corroborates the relay. A 413 on an entry inside the bound stays retryable (a proxy's limit is not the entry's fault).
    - **Refused at mint.** The kit never mints an enrolment past the bound (`entry-too-large` from `enroll` and `ensureEnrolled`; nothing is kept), so `motebit machines enroll <4KB id> --force` can no longer create one. The bound is the kit's `MAX_ROSTER_ENTRY_BYTES`. The relay's constant cannot be imported into a package, so a CLI test against the in-process relay pins that both sides draw the line at the same byte: 4096 is minted and held, 4097 is refused at mint and by the relay. Retirements carry no free-form field but `motebit_id`, so they are not bounded at mint. If one were refused, the presentation path treats it as permanent.
