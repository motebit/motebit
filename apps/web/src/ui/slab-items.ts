 ...
 *
 * Exported for unit testing; the renderer is the only in-repo
 * consumer.
 */
export function extractScreenshot(result: unknown): ScreenshotPayload | null {
  if (result === null || typeof result !== "object") return null;
  const r = result as Record<string, unknown>;
  // Two action kinds carry inline screenshot bytes today: the
  // explicit `screenshot` action (always returns bytes) and the
  // `navigate` action (v1.3 hardening — returns inline bytes so the
  // slab shows the page right after navigation without depending on
  // the live-screencast deploy). Both produce the same renderable
  // payload; the kind is just the dispatch site that captured it.
...
export interface ReceiptHopSummary {
  ...
  /** First 12 chars of the hex signature, or `null` if absent (peer didn't sign). */
  readonly signaturePrefix: string | null;
  /** Duration in ms, or `null` when absent (older receipts predate the field). */
  readonly durationMs: number | null;
  /** Reported status — one of "completed", "failed", "denied", "returned" fallback. */
  readonly status: string;
}

export interface ReceiptSummary {
  /** Outer receipt — what the peer this motebit asked signed back. */
  readonly outer: ReceiptHopSummary & { readonly taskId: string };
  /** Sub-delegations the peer made, flattened depth-first. */
 ...
 * Pure / synchronous; exported for unit tests.
 *
 * Returns `null` for missing / non-object inputs so the caller falls
 * through to the "outbound — nothing perceived yet" path.
 */
