...
function planStatusBadge(status: PlanEntry["status"]): { text: string; className: string } {
  switch (status) {
    case "active":
      return { text: "active", className: "plan-status active" };
    case "completed":
      return { text: "completed", className: "plan-status completed" };
    case "failed":
      return { text: "failed", className: "plan-status failed" };
    case "paused":
      return { text: "paused", className: "plan-status paused" };
    default:
      return { text: status, className: "plan-status" };
  }
}
...
function stepStatusBadge(status: PlanStepEntry["status"]): { text: string; className: string } {
  switch (status) {
    case "pending":
      return { text: "pending", className: "step-status pending" };
    case "running":
      return { text: "running", className: "step-status running" };
    case "completed":
      return { text: "completed", className: "step-status completed" };
    case "failed":
      return { text: "failed", className: "step-status failed" };
    case "skipped":
      return { text: "skipped", className: "step-status skipped" };
    default:
      return { text: status, className: "step-status" };
  }
}
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      step.retry_count > 0
        ? React.createElement(
            "span",
            { className: "step-retries" },
            `${step.retry_count} retr${step.retry_count !== 1 ? "ies" : "y"}`,
          )
        : null,
      duration ? React.createElement("span", { className: "step-duration" }, duration) : null,
    ),
    step.result_summary != null && step.result_summary !== ""
      ? React.createElement("div", { className: "step-result" }, truncate(step.result_summary, 200))
      : null,
    step.error_message != null && step.error_message !== ""
      ? React.createElement("div", { className: "step-error" }, truncate(step.error_message, 200))
      : null,
  );
}
...
export function PlansPanel({ plans }: { plans: PlanEntry[] }): React.ReactElement {
  ...
  return React.createElement(
    ...
    ...sorted.map((plan) => {
      ...
      return React.createElement(
        "div",
        { key: plan.plan_id, className: "plan-entry" },
        React.createElement(
          "div",
          {
