...
The CLI is a Node.js REPL for talking to your motebit — streaming responses, tool approvals, slash commands, and a daemon mode for scheduled goals. Same code that ships via `npm install -g motebit`; running `pnpm --filter motebit start` from a monorepo clone invokes it with the package source instead of the published bundle.

## Running the CLI

```bash
pnpm --filter motebit start
```
...
| Flag | Description |
|---|---|
| `--provider <name>` | `anthropic`, `openai`, `google`, `groq`, `deepseek`, `local-server`, or `proxy` (`ollama` is a deprecated alias for `local-server`) |
...
Type `y` to approve; anything else denies. The agent continues with the result (or denial) and completes the turn.
...
| `motebit halt [goal <id>] [--reason "..."] [--remote]` | Withdraw the permission to act unattended; `--remote` signs a command to the running runtime and returns its acknowledgement |
| `motebit resume [<halt_id>\|all] [--remote]` | Give that permission back |
| `motebit halt-status [--remote]` | What is stopped, and whether the motebit has acknowledged it |
| `motebit runs [list\|show <run_id>\|ack <run_id> --allow-fresh-run]` | Goal-run ledger: runs holding their goal (paused on you, or interrupted with side effects) and recent runs; `show` opens one run — what it produced and whether that result is signed, what its tools reported, and the evidence pointers a stranger could re-check against the original source; `ack --allow-fresh-run` releases a reviewed interrupted run, accepting that its goal's next run may repeat effects |
| `motebit rotate [--reason "..."]` | Rotate Ed25519 keypair with signed succession record |
| `motebit machines [--json]` | The machines this motebit runs unattended work on — reduced on this machine under a key chain it verified, never by the relay; a count only when nothing suppressed it, citing the chain head |
| `motebit machines retire <device_id>` | Sign a retirement for every standing entry of a machine (undo with `machines enroll`) |
| `motebit machines enroll <device_id> [--force]` | Rejoin a retired line, or enrol an id explicitly; refuses an id that could never answer unless `--force` |
...
In HTTP mode, the server registers with the sync relay (if configured) and sends a heartbeat every 5 minutes. Authentication is fail-closed: callers present a motebit signed token (`Authorization: Bearer motebit:<signed-token>`, e.g. the relay's per-task `task:dispatch` token) and everything else except `/health` gets 401. There is no static `authToken` setting in `~/.motebit/config.json`.
