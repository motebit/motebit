...
2. Store the private key in `~/.motebit/dev-keyring.json` (owner-only, `0600`)
...
2. First time: set a 4-6 digit PIN (stored in the keyring as a salted PBKDF2-SHA256 digest, never plaintext)
