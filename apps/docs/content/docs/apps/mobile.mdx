...
In the **Capabilities** panel (not the settings modal):
...
| **Billing** | Motebit Cloud balance and funding for this motebit (`BillingPanel`) |

MCP servers (server list with trust toggles, add/remove, motebit-pinned public keys) live in the **Capabilities** panel, not in the settings modal.
