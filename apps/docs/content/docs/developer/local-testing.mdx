...
The relay lives in `services/relay/`. One environment variable is required (`X402_PAY_TO_ADDRESS`); `MOTEBIT_API_TOKEN` is optional but the `Bearer` examples below use it:
...
> The self-declared deposit route (`POST /api/v1/agents/:id/deposit`) no longer exists — it was removed as a treasury-drain vector, and balance is credited only by verified server-side funding (Stripe checkout via `POST /api/v1/agents/:id/checkout`, or on-chain USDC picked up by the deposit detector). For a local loop, register an unpriced listing (`unit_cost: 0`) so no balance is needed.
...
`npm run self-test` from a scaffolded agent (`npm create motebit -- --agent`) exercises the self-delegation probe against the live relay (`dev` and `start` do not pass `--self-test`), so sybil defense regressions are caught by every new developer who runs it during onboarding.

## Reading the inspector dashboard

The inspector dashboard gives you a visual window into one motebit's interior — agent trust graphs, task queues, credentials, and event history.
...
- Fund the account through a verified path (Stripe checkout, or the on-chain deposit detector) — the self-declared `/deposit` route was removed as a treasury-drain vector; for local testing, use an unpriced listing
