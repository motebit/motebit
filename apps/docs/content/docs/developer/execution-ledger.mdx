...
> **This hash-then-sign recipe is specific to the ledger.** An `ExecutionReceipt` is signed differently: Ed25519 over the JCS-canonical UTF-8 **body bytes directly**, with no intermediate SHA-256. Do not apply the ledger's "sign the 32-byte content hash" recipe to receipts — use the published `signExecutionReceipt` / `verifyExecutionReceipt`, which encode the correct recipe for you.
...
Returns the execution ledger the agent submitted for a specific goal — signed by the agent. If none was submitted, this route returns 404. For a relay-reconstructed ledger assembled from synced events (unsigned), use `GET /api/v1/execution/:motebitId/:goalId` (the state-export surface, which wraps the body in a relay-signed `ContentArtifactManifest`).
...
### Reconstructed ledger (inspector)
...
1. Check `spec` is `"motebit/execution-ledger@1.0"` or `"motebit/execution-ledger@1.1"` (1.1 is what the relay emits when it archives the inner `signed_receipts`; agent-produced ledgers are 1.0).
...
5. **Do not try to Ed25519-verify the entries in a v1.0 ledger's `delegation_receipts[]`** — they are `DelegationReceiptSummary` objects (§4.2) whose `signature_prefix` is the _first 16 characters_ of the signature, for linkage/display only (not a verifiable signature, and there is no embedded public key). To independently verify a delegated hop, retrieve the **full signed `ExecutionReceipt`** as a separate edge artifact (from the relay or the delegating agent) and verify _that_ with `verifyExecutionReceipt` / `verifyArtifact`. The ledger proves _its own_ timeline is authentic; verifying the delegated work is a separate step against the full receipts.
