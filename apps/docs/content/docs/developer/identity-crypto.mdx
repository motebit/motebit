...
2. **Store the private key** in the platform's secure storage. On desktop, `~/.motebit/dev-keyring.json` — an owner-only (mode `0600` on Unix; Windows is best-effort) file, access-controlled rather than application-encrypted; the OS keychain is not used yet (its enablement is a separate arc gated on real-device testing, because keychain providers differ in ways that can lose a key). On CLI, PBKDF2-encrypted as `cli_encrypted_key` in `~/.motebit/config.json`. On mobile, `expo-secure-store` (iOS Keychain / Android Keystore, no fallback). The private key never appears in the non-secret `config.json` and never crosses the network; on desktop the key file is access-controlled but not application-encrypted (moving it into the OS keychain is tracked as its own arc — see [security boundaries](https://github.com/motebit/motebit/blob/main/docs/doctrine/security-boundaries.md)).
...
| **Desktop**         | Private key stored in the owner-only (`0600`) `~/.motebit/dev-keyring.json` — access-controlled, not application-encrypted. The OS keychain is not used yet: enabling it is its own arc, gated on real-device testing. |
...
| Concern               | Approach                                                                                                                                                                                                                                                                                                                                                   |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Key storage**       | Mobile: iOS Keychain / Android Keystore. Desktop: `~/.motebit/dev-keyring.json`, owner-only (`0600`), access-controlled rather than application-encrypted; the OS keychain is not used yet. CLI: PBKDF2-encrypted file. Never in the non-secret `config.json`.                 |
