...
2. **Store the private key** in the platform's secure storage. On desktop, the Tauri keyring (the OS Keychain / Credential Manager / Secret Service) — with an owner-only (mode `0600`) local fallback file, because some OS keyrings silently drop writes on certain build signatures and the failure cannot be detected in-process. On CLI, PBKDF2-encrypted as `cli_encrypted_key` in `~/.motebit/config.json`. On mobile, `expo-secure-store` (iOS Keychain / Android Keystore, no fallback). The private key never appears in the non-secret `config.json` and never crosses the network; on desktop the keyring fallback is access-controlled but not application-encrypted (verified-keyring-only writes on signed builds are tracked hardening — see [security boundaries](https://github.com/motebit/motebit/blob/main/docs/doctrine/security-boundaries.md)).
...
All fields below are **required** — a verifier MUST reject a token missing any of them (spec `auth-token@1.0` §3). `createSignedToken` stamps only `suite`; every other field is the caller's to supply, so mint with `mintAudienceToken` (which fills `jti`, `iat` and `exp` and binds `aud`) rather than hand-assembling a `{mid,did,iat,exp}` payload — that would be rejected for missing `jti`/`aud`.

| Field   | Required | Description                                                                                                    |
| ------- | -------- | -------------------------------------------------------------------------------------------------------------- |
| `mid`   | yes      | The `motebit_id`                                                                                               |
| `did`   | yes      | The `device_id`                                                                                                |
| `iat`   | yes      | Issued-at timestamp (milliseconds since epoch)                                                                 |
| `exp`   | yes      | Expiration timestamp (default: 5 minutes from `iat`)                                                           |
| `jti`   | yes      | Unique token nonce (UUID v4 recommended) — prevents replay. Must be unique per token.                          |
| `aud`   | yes      | Audience — the endpoint/operation the token authorizes (e.g. `"task:submit"`). Prevents cross-endpoint replay. |
| `suite` | yes      | Cryptosuite, always `"motebit-jwt-ed25519-v1"` today. Signed; missing/unknown is rejected fail-closed.         |
...
| **Mobile**          | Private key stored in `expo-secure-store` (iOS Keychain / Android Keystore, hardware-backed where available). No fallback.                                                                                                                |
| **Desktop**         | Private key stored in the OS keyring, with an owner-only (`0600`) local fallback file when a keyring write cannot be confirmed (some build signatures silently drop keyring writes). The fallback is access-controlled, not application-encrypted; verified-keyring-only writes on signed builds are tracked hardening. |
...
| Concern               | Approach                                                                                                                                                                                                                                                                                                                                                   |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Key storage**       | Mobile: iOS Keychain / Android Keystore. Desktop: OS keyring, with an owner-only (`0600`) local fallback file when a keyring write cannot be confirmed (access-controlled, not application-encrypted — verified-keyring-only writes on signed builds are tracked hardening). CLI: PBKDF2-encrypted file. Never in the non-secret `config.json`.                 |
