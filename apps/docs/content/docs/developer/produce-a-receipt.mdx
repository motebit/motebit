...
> New here? The [Quickstart](/docs/developer/quickstart) is the shortest runnable mint→verify in one file. This page is the depth behind it.
...
`@motebit/crypto` mints; `@motebit/verifier` is used in the self-check step below. `strictHashBinding` requires `@motebit/crypto@3.3.0+` / `@motebit/verifier@1.4.0+`.
...
From the spec — the two string hashes are §11.4 "Content Hashing"; `args_hash` is defined in §3.1 (tool-invocation fields):
...
You can narrow this boundary — not erase it — with **evidence provenance**: attach an `EvidenceProvenance` to a citation and a consumer can re-fetch the primary source and re-check that the cited span is actually present in it, offline and in any language. That makes input _presence_ independently re-verifiable; it still proves presence, never truth (no oracle). See [Verify a citation's evidence](/docs/developer/verify-evidence).
