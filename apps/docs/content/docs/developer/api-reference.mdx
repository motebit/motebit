...
The other narrow-surface published packages: `@motebit/crypto-{appattest,android-keystore,tpm,webauthn,play-integrity}` (the five hardware-attestation leaves bundled by `@motebit/verify` — `crypto-play-integrity` is deprecated and removed at 2.0.0; `crypto-android-keystore` is the canonical sovereign-verifiable Android primitive), `create-motebit` (scaffold), and `motebit` (the BSL-1.1 reference runtime CLI).
