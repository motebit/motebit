...
```
my-agent/
  motebit.md          Signed agent identity (Ed25519 public key, governance, metadata)
  src/tools.ts         Tool definitions — this is the file you edit
  package.json         Scripts wired to `motebit serve`
...
Open `src/tools.ts`. The scaffold starts with a minimal echo tool:
...
This runs `tsc` to compile TypeScript, then starts the MCP server:

```bash
tsc && npx motebit serve --identity ./motebit.md --tools ./dist/tools.js --serve-transport http --direct
```
...
| Flag | Purpose |
|------|---------|
| `--identity ./motebit.md` | Load the signed identity file |
| `--tools ./dist/tools.js` | Load your compiled tool definitions |
| `--serve-transport http` | Use HTTP (StreamableHTTP) transport instead of stdio |
| `--direct` | Direct tool execution — no LLM, maps prompt to tool input |
...
The production `start` script includes `--self-test`:
...
{
  "scripts": {
    "dev": "tsc && npx motebit serve --identity ./motebit.md --tools ./dist/tools.js --serve-transport http",
    "start": "npx motebit serve --identity ./motebit.md --tools ./dist/tools.js --serve-transport http --self-test"
  }
}
```
