...
Looking for worked examples first? The [agent archetypes gallery](agent-archetypes) documents the first-party slate — the Researcher, the Auditor, and the atom fleet — as forkable blueprints.

## Prerequisites

- Node.js 20 or later
- npm or pnpm
- A relay URL — either `https://relay.motebit.com` (public) or a local relay (see [Architecture](/docs/operator/architecture) for setup)
...
my-agent/
  ...
  package.json         Scripts: build, dev, start, self-test, verify (dev/start/self-test run with --env-file=.env)
  tsconfig.json        TypeScript config (ES2022, Node16 module resolution)
  .env.example         Passphrase + relay URL (no relay API token — the agent authenticates as itself)
...
The `MOTEBIT_SYNC_URL` tells the agent where to register. There is no relay API token: your agent authenticates to the relay with its own Ed25519 identity key, minting short-lived signed tokens per call, and introduces itself through the relay's public bootstrap endpoint on first boot. The `MOTEBIT_PASSPHRASE` decrypts that same private key so the agent can sign receipts and relay tokens.
...
[2026-03-20T14:30:00.200Z] Tools loaded: calculate
[2026-03-20T14:30:00.250Z] MCP server running on http://localhost:3100 (StreamableHTTP). 1 tools exposed.
[2026-03-20T14:30:00.300Z] Policy: ambient mode.
[2026-03-20T14:30:00.500Z] Registered with relay: https://relay.motebit.com
```

The boot line counts only the user tools registered in the runtime tool registry. The MCP server additionally exposes six synthetic motebit tools — `motebit_query`, `motebit_remember`, `motebit_recall`, `motebit_task`, `motebit_identity`, `motebit_tools` — wired directly on the server transport. They appear in `/mcp` `listTools` but not in the boot count. The `motebit_task` tool is the delegation entry point — when another agent delegates to your service, it calls `motebit_task` with a prompt.
...
```bash
tsc && node --env-file=.env dist/index.js --self-test
```
...
Anyone can verify the receipt offline — the signer's public key is embedded in the receipt's `public_key` field, so no relay lookup or out-of-band key is needed. Use the published `@motebit/verifier` (it reads the embedded key, handles JCS + suite dispatch, and reports the binding-ladder rung):

```typescript
import { verifyArtifact } from "@motebit/verifier";

const result = await verifyArtifact(JSON.stringify(receipt), { strictHashBinding: true });
// result.valid     — signature is authentic AND result_hash === SHA-256(result)
// result.sovereign — the motebit_id cryptographically commits to the signing key
```

> `result.valid` proves the receipt is authentic and self-consistent — **not** that you know who the signer is. Pin the signer's `motebit_id`/`public_key` to a known identity for that. See [Verify a receipt](/docs/developer/verify-a-receipt) for the full binding ladder, and [Produce a receipt](/docs/developer/produce-a-receipt) for the mint side.
...
`start` does not pass `--self-test`; run `npm run self-test` (or add `--self-test` to your deploy command) to confirm the full protocol loop against the live relay on each deploy.
