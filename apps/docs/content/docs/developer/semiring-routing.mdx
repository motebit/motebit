...
Agent network routing is algebraic, not ad-hoc. The `@motebit/semiring` package provides a generic `Semiring<T>` interface, concrete semirings for different routing concerns, a `WeightedDigraph<T>`, and two traversal algorithms that answer every routing query the system needs. Most trusted path, cheapest pipeline, reachability, and all of the above simultaneously — by swapping the semiring.
...
```typescript
import { productSemiring, TrustSemiring, CostSemiring } from "@motebit/semiring";
...
```typescript
import { mappedSemiring, CostSemiring } from "@motebit/semiring";
...
```typescript
import { WeightedDigraph, TrustSemiring } from "@motebit/semiring";
...
```typescript
import { optimalPaths } from "@motebit/semiring";
...
```typescript
import { transitiveClosure } from "@motebit/semiring";
...
```typescript
import { optimalPathTrace } from "@motebit/semiring";
...
```typescript
import { projectGraph, TrustSemiring } from "@motebit/semiring";
...
```typescript
import type { Semiring } from "@motebit/semiring";
