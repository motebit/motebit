...
With an identity file and bearer auth:

```bash
motebit --serve \
  --serve-transport http \
  --identity motebit.md \
  --serve-auth-token my-secret-token
```
...
Returns the motebit's identity. If the server was started with `--identity motebit.md`, returns the full signed identity file. Otherwise returns a JSON summary with the motebit ID and public key.
...
- `/sse` — requires auth (SSE connection for MCP transport)
- `/messages` — requires auth (JSON-RPC messages)
