...
The `--self-test` flag submits a task to itself via the relay after registration, proving the full loop works (submit → route → execute → receipt → settle). This is the pattern used by `npm create motebit --agent` scaffolds.
