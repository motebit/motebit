...
Compared to a plain MCP tool server, motebit's server adds three protocol-level commitments: every response is identity-tagged with the signing motebit's public key, every tool call passes through the policy gate, and every autonomous task is returned as an Ed25519-signed `ExecutionReceipt`.
