...
Stores a memory in the motebit's semantic graph. External callers may store only `"none"` sensitivity — attempts to store `"personal"`, `"medical"`, `"financial"`, or `"secret"` memories are denied (fail-closed).

```json
{ "content": "The deploy key rotates every 90 days", "sensitivity": "none" }
```
...
Any agent receiving the receipt can verify it was executed by this specific motebit using [`@motebit/crypto`](/docs/developer/identity-standard).
...
Requests with a missing or incorrect token receive a `401 Unauthorized` response. HTTP transport is **fail-closed**: if no `authToken` is configured, every request except `/health` is still rejected with 401 unless the caller presents a motebit signed token (`Authorization: Bearer motebit:<signed-token>`). Open access is never a default.
