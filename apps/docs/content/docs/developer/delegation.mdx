...
4. **Receipt** — The service returns a signed `ExecutionReceipt` — an Ed25519 signature over the task ID, result hash, tools used, and timestamps. The receipt embeds the signer's public key, making it self-verifiable without contacting any relay or lookup service.
...
Returns a list of registered agents with their endpoint URLs, capabilities, public keys, and W3C `did:key` identifiers.
...
2. **Identity call** — Calls the `motebit_identity` synthetic tool. The server returns its `motebit_id`, `public_key`, and `did` (W3C `did:key`) — either as JSON or a full `motebit.md` identity file.
...
```typescript
{
  ...
  public_key?: string;    // signer's Ed25519 public key, HEX (embed it → offline sovereign verify)
  ...
  suite: "motebit-jcs-ed25519-b64-v1"; // REQUIRED, signed — omit it and verification fails
  signature: string;      // Ed25519 signature (base64url)
}
```

> **`suite` is required and signed.** `signExecutionReceipt` stamps it for you; if you hand-build a receipt you MUST include `suite: "motebit-jcs-ed25519-b64-v1"` in the body before signing, or the verifier reconstructs a different body and the signature won't match. Use the published `signExecutionReceipt` rather than assembling the body yourself — see [Produce a receipt](/docs/developer/produce-a-receipt).
...
```typescript
import { signDelegation, bytesToHex } from "@motebit/crypto";

const token = await signDelegation(
  {
    delegator_id: "motebit-abc",
    // Embedded public keys are HEX (64 chars) — like a receipt's `public_key`
    // field. base64url is used only for signatures, never for embedded keys.
    delegator_public_key: bytesToHex(delegatorKeypair.publicKey),
    delegate_id: "motebit-xyz",
    delegate_public_key: bytesToHex(delegateKeypair.publicKey),
    scope: "web_search",
    issued_at: Date.now(),
...
The signature covers all fields except `signature` itself: the body is JCS-canonicalized (RFC 8785) to UTF-8 bytes and Ed25519-signed directly — the same recipe as execution receipts (sign the canonical bytes, not a hash of them).
...
Verifies the Ed25519 signature using the `delegator_public_key` (hex) embedded in the token, **and checks expiration by default** — it returns `false` when `token.expires_at < Date.now()`. Pass `verifyDelegation(token, { checkExpiry: false })` to verify the signature only (e.g. auditing a historical chain), and `{ now }` to pin the clock.
...
<ReferenceExample
  ...
  note="Service authors who want to ship a paid agent without forking motebit run the published `motebit` CLI; the binding shown here is what `motebit serve` calls under the hood."
>
...
External callers cannot store high-sensitivity memories. The `motebit_remember` tool rejects any request with sensitivity level `personal`, `medical`, `financial`, or `secret` — only `none` is accepted from an external caller. Memory queries via `motebit_recall` filter out these sensitivity levels before returning results.
