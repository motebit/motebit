...
6. Call `motebit_task` on Charlie with the URL as the prompt, passing `relay_task_id` for economic binding and the relay's `dispatch_token` (returned by the task submission) as proof of admission — a worker configured for task admission refuses the hop without it.
...
interface ExecutionReceipt {
  ...
  relay_task_id: string; // binds receipt to the relay's economic identity — required; the relay rejects a receipt without it
  delegation_receipts?: ExecutionReceipt[]; // nested sub-delegation proofs
  suite: "motebit-jcs-ed25519-b64-v1"; // REQUIRED, signed — stamped by signExecutionReceipt
  signature: string; // Ed25519 over the JCS-canonical UTF-8 bytes of all other fields (not a hash of them)
}
```
...
After enough successful delegations, the relay may auto-promote trust levels through the `AgentTrustLevel` enum: `Unknown` / `FirstContact` → `Verified` → `Trusted`. The reference defaults live in `REFERENCE_TRUST_THRESHOLDS` (`packages/protocol/src/trust-algebra.ts`): promote to `Verified` at 5 successful tasks with ≥ 80% success rate, promote to `Trusted` at 20 successful tasks with ≥ 90% rate, demote when success rate drops below 50% with at least 3 tasks observed. `Blocked` is the terminal demotion state. The state machine is evaluated on every receipt via `evaluateTrustTransition`; an implementer can override the thresholds by passing a different `TrustTransitionThresholds` config.
