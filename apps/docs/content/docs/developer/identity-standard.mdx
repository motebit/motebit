...
| Private key theft                  | Encrypted at rest (PBKDF2 + AES-256-GCM) in config on the CLI; the OS keychain on mobile; an owner-only (`0600` on Unix; Windows is best-effort) key file on desktop (access-controlled, not encrypted); never in the identity file |
