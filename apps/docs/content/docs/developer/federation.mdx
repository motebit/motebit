...
| Storage    | AES-256-GCM encrypted at rest (PBKDF2, 600k iterations) when `MOTEBIT_RELAY_KEY_PASSPHRASE` is set. Plaintext hex for dev mode. |
...
Relay A (initiator)                         Relay B (responder)
    ...
    |  { relay_id, public_key, endpoint_url,     |
    |    display_name, nonce,                    |
    |    challenge: Sign(relay_id:nonce:suite, key_b) } |
    | <----------------------------------------- |
    |                                            |
    |  3. POST /federation/v1/peer/confirm       |
    |  { relay_id,                               |
    |    challenge_response: Sign(relay_id:nonce:suite, key_a)|
    |  }                                         |
    | -----------------------------------------> |
    |                                            |
...
Step 1: Relay A sends its identity and a 32-byte random `nonce`. Step 2: Relay B signs `${relay_id}:${nonce}:${suite}` (the wire field is `nonce`; the cryptosuite id is part of the signed message) with its private key, proving key possession. Step 3: Relay A verifies B's signature, then signs the same shape over B's `nonce` with its own key. Relay B verifies. Both transition to `active`.
...
The signature covers `relay_id || timestamp` to prevent replay. A suspended peer is excluded from discovery and routing. Recovery has hysteresis: each successful heartbeat decrements the missed count by one, so a peer suspended at 3 misses needs 3 consecutive successes to return to `active`. After 5 misses, the peer is removed and requires a full handshake to re-establish.
...
```json
{
  ...
  "budget": { "amount": 0.1, "currency": "USD" },
...
| Parameter    | Value                                                                                   |
| ------------ | --------------------------------------------------------------------------------------- |
| Max attempts | `max_attempts` on the retry row (default 5); the policy ceiling is `maxRetries: 8`       |
| Backoff      | Exponential from 5 s (5 s, 10 s, 20 s, 40 s, …) with ±20% jitter, capped at 1 hour       |
| Status       | `pending` -> `completed` or `failed`                                                    |

When attempts are exhausted the relay auto-refunds the delegator; nothing waits for manual intervention. Constants live in `services/relay/src/retry-policy.ts` (`DEFAULT_RETRY_POLICY`).

## Trust model

Cross-relay trust uses the same `evaluateTrustTransition` state machine that governs agent trust. Relay peers are trust subjects with their own trust records and follow the same `AgentTrustLevel` progression as agents: `Unknown` → `FirstContact` → `Verified` → `Trusted`, with `Blocked` as the terminal demotion state.
...
| Body field     | Value                                                                                                             |
| -------------- | ----------------------------------------------------------------------------------------------------------------- |
| `sender_relay` | The sending relay's `motebit_id` (some routes name it `relay_id`)                                                 |
| `timestamp`    | Unix epoch milliseconds; requests more than 5 minutes from the receiver's clock are rejected                        |
| `signature`    | Hex Ed25519 signature over the RFC 8785 canonical JSON of the request body with `signature` removed                |

Authentication is carried **in the JSON body**, not in HTTP headers: the receiver looks up the sender's registered public key, drops `signature`, canonicalizes the rest and verifies.
...
| ------------------------------- | -------- | ------- | ------------------------------------- |
| `federation.enabled`            | boolean  | on when `endpoint_url` is set | Whether federation is active |
| `federation.max_peers`          | number   | `50`    | Maximum active peer relationships     |
...
When `federation.enabled` is `false`, all `/federation/v1/` endpoints return HTTP 403 (`Federation is disabled on this relay`). When `enabled` is left unset, federation is on whenever `endpoint_url` is configured.
