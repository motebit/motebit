...
- **At rest.** On mobile the private key lives in the iOS Keychain / Android Keystore. On desktop it lives in `~/.motebit/dev-keyring.json`, an owner-only (`0600` on Unix; Windows is best-effort) file — access-controlled, not application-encrypted (the same protection as an SSH key). The OS keychain is not used yet; enabling it is its own arc, gated on real-device testing (`docs/proposals/key-file-durability-v1.md`). On the CLI it is encrypted with your passphrase using PBKDF2 (600,000 iterations, SHA-256) and AES-256-GCM. It never crosses the network and never enters the non-secret config metadata.
