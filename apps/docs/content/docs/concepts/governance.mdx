...
The PIN is hashed and stored in the device's key store (the iOS Keychain / Android Keystore on mobile; the owner-only (`0600`) `~/.motebit/dev-keyring.json` file on desktop). It's never stored in plaintext and never sent over the network.
