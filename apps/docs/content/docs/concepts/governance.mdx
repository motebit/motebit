...
| Draft (`R1_DRAFT`) | Creates metadata, no external effect | Browse events, fetch a URL |
...
Read and draft tools flow through automatically. Write and execute tools require operator mode. Financial tools always route to approval unless a signed standing-delegation grant covers the call — they are gated, never auto-run, and no configuration can change that. The risk level is inferred from the tool's name and description, so even tools from external sources get classified.
...
The PIN is hashed and stored in the device's key store (the iOS Keychain / Android Keystore on mobile; the owner-only (`0600` on Unix; Windows is best-effort) `~/.motebit/dev-keyring.json` file on desktop). It's never stored in plaintext and never sent over the network.
