...
Memory is the interior's accumulated energy — what makes a sovereign agent more capable the longer it runs, where a stateless chatbot starts empty every session. Every conversation, every interaction, every decision leaves a trace. Motebit's memory system stores these traces as a semantic graph — searchable by meaning, governed by sensitivity, and subject to natural decay over time.
...
Memories don't persist at full strength forever. Each memory has a half-life — the time it takes for its confidence to drop by half. The default depends on the kind: **3 days for episodic memories** (observations, transient context) and **30 days for semantic memories** (durable facts).

A semantic memory stored at 90% confidence (30-day half-life):
- After 30 days: 45%
- After 60 days: 22%
- After 90 days: 11%

Trivia fades. Important things stick — because they get reinforced. When a memory is confirmed, its confidence rises by 0.1 and its half-life stretches by half; retrieval alone refreshes recency, not confidence. Memories that matter keep coming back; memories that don't, gracefully disappear.
