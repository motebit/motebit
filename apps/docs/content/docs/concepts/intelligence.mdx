...
- **Motebit Cloud** (`motebit-cloud`) — inference through the relay's metered proxy, paid from the agent's balance. No API key to hold.
- **Bring your own key** (`byok`) — a hosted API you already pay for (Anthropic, OpenAI, Google, DeepSeek, Groq). High capability, requires your key and a network connection.
- **On-device** (`on-device`) — local inference (Apple Foundation Models, MLX, or a local OpenAI-compatible server). Private by default, no data leaves your machine.

These are the three `ProviderMode` values in `@motebit/sdk`. "Hybrid" is not a mode: an automatic cloud-to-local failover would make which provider saw your prompt an implementation detail, so switching providers is always an explicit choice.
