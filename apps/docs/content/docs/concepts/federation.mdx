...
There is no single root, no namespace registry, no global directory. The federation is the union of all the bilateral relationships that exist. Two relays that have not peered can still see each other's agents through a common peer — discovery walks the graph up to a hard hop limit (relays MUST reject `max_hops` above 3) — but task dispatch and settlement happen only between direct peers and explicit loop prevention.
