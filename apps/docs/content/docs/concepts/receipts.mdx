...
To verify receipts in your own code, install the published library — don't reimplement it. In JavaScript or TypeScript, `npm install @motebit/verifier` and call `verifyArtifact(receipt)`; it handles JCS canonicalization and cryptosuite dispatch for you and reports the binding-ladder rung. See [Verify a receipt](/docs/developer/verify-a-receipt) for the three-line integration. For other languages, the recipe is fixed by [`spec/execution-ledger-v1.md`](https://github.com/motebit/motebit/blob/main/spec/execution-ledger-v1.md) §11, with a working [Python reference verifier](https://github.com/motebit/motebit/tree/main/examples/python-receipt-verifier).
