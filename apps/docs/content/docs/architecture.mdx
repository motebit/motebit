...
| Directory | Contents |
|-----------|----------|
| `apps/desktop` | Tauri (Rust + webview). Flagship client (~80%). Three.js glass creature, identity bootstrap, operator mode, voice input, audio-reactive rendering. |
| `apps/cli` | Node.js REPL + daemon. Full operator console — chat, goal scheduling, tool approval queue, `motebit export` / `verify`. |
| `apps/mobile` | React Native + Expo. Early MVP (~30%). expo-gl + Three.js rendering, SQLite adapters, Ollama-only. |
| `apps/admin` | React + Vite dashboard. Real-time state vector, memory graph, behavior cues, event log, tool audit. |
| `apps/spatial` | AR/VR positioning library. Body-relative orbital mechanics. Stub runtime. |
...
8. **Platform renderer** — Three.js on desktop, expo-gl on mobile, stub on spatial.
