...
1. **Identity** — The app generates an Ed25519 keypair and stores the private key in the owner-only (`0600`) `~/.motebit/dev-keyring.json` (the OS keychain is not used on desktop yet). This is your agent's identity. It never leaves your device.
