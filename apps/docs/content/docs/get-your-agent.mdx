...
The `--agent` scaffold creates a project with `src/tools.ts` (an echo tool to start), `motebit.md` (signed service identity), and `package.json` wired to `motebit serve --direct`. Edit `src/tools.ts`, run `npm run dev`, and your agent registers with the relay and is ready to receive delegated tasks. When deployed, `npm run start` includes a self-test that verifies the full receipt loop.
