...
Three install paths share one identity: a signed identity file you can verify and share, a service agent that runs as a worker, and the full operator console with REPL, daemon, and MCP server. Pick by what you want to ship.
...
The identity-only path is lightweight — no runtime, no database, just a cryptographically signed file. The service agent path scaffolds a complete project with `src/tools.ts`, identity, and scripts wired to `motebit serve --direct` — a few commands from install to your first paid task. The full agent gives you the complete operator console with streaming chat, MCP tool connections, goal scheduling, and everything described below.

## Desktop app

The desktop app is the primary way to interact with your motebit. A liquescent droplet floats on your screen — it breathes, glows when thinking, responds to your voice, and remembers your conversations.

Desktop releases are available from the [GitHub releases page](https://github.com/motebit/motebit/releases).

### First launch

1. **Identity** — The app generates an Ed25519 keypair and stores the private key in the owner-only (`0600` on Unix; Windows is best-effort) `~/.motebit/dev-keyring.json` (the OS keychain is not used on desktop yet). This is your agent's identity. It never leaves your device.
...
Everything is local. Your memories, your identity, your conversation history — all stored on your machine in a local SQLite database protected by filesystem permissions; your private key is kept apart: encrypted on the CLI (PBKDF2 + AES-256-GCM), in the iOS Keychain / Android Keystore on mobile, and in an owner-only (`0600`) key file on desktop.
...
| **Anthropic** | Enter your API key. Stored in the owner-only (`0600`) `~/.motebit/dev-keyring.json`, never in `config.json`. |
...
Your PIN hash is stored in the owner-only (`0600`) `~/.motebit/dev-keyring.json`. Every tool call is logged to an audit trail.

## Command line

Install the CLI globally — the same agent, no creature, full operator console:
...
Once registered, your memories, conversations, and goals sync across devices. Pairing is currently driven from the desktop and mobile apps' settings panels — they call the relay's pairing endpoints (`POST /api/v1/pair/initiate` and `POST /api/v1/pair/complete`) to exchange a 6-character code, then sync through the relay with encrypted payloads. Each device generates its own Ed25519 keypair under the same `motebit_id`; compromise of one doesn't compromise the other. A standalone CLI pairing subcommand is not currently exposed.
...
```bash
# Docker (recommended)
docker build -t motebit-relay -f services/relay/Dockerfile .
...
See [Relay Federation](/docs/developer/federation) for the full protocol specification.

## What happens over time

Your motebit accumulates:

- **Memories** — Conversations become semantic nodes with confidence scores. Important things strengthen. Episodic memories decay with a 3-day half-life and semantic ones with a 30-day half-life. You can inspect, export, or delete any memory.
...
The old identity is abandoned. Memories do not migrate between identities — each identity has its own isolated memory graph. Hardware-backed storage on mobile (iOS Keychain / Android Keystore) and owner-only files on desktop and CLI make compromise unlikely in practice.
