...
All three accumulate locally. Switching LLM providers does not reset memory, trust, or the audit trail. Multi-device sync ships your accumulated state between devices through the relay; with encrypted sync enabled, the relay sees only opaque ciphertext.
