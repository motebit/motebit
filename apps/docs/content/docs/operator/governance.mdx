...
In practice with the defaults: read and draft tools (≤ `R1_DRAFT`) flow automatically; write, execute, and financial tools (`R2`–`R4_MONEY`) require approval. Because `deny_above` sits at the top of the scale (`R4_MONEY`), nothing is hard-denied by default — set `deny_above` lower to unconditionally block a risk class. (An `R4_MONEY` call can only ever *auto*-execute when the turn carries a cryptographically verified standing-delegation grant; absent that, it always routes to approval — see the governance doctrine.)
...
| Aspect | Detail |
|--------|--------|
| **PIN** | 4-6 digits. Stored in the device key store (the owner-only `~/.motebit/dev-keyring.json` on desktop) as a salted PBKDF2-SHA256 digest (100 000 iterations) — never plaintext, never sent over the network. |
...
- `maxCallsPerTurn` — maximum tool calls per turn (default 10; configurable per-device)
...
The audit log is queryable via the inspector dashboard described in [Architecture](/docs/operator/architecture), the CLI (`motebit approvals list`), and programmatically through the `AuditLogger` API. Mode changes (operator mode enable/disable) are also logged as audit entries.
