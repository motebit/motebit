...
The audit log is queryable via the admin dashboard described in [Architecture](/docs/operator/architecture), the CLI (`motebit approvals list`), and programmatically through the `AuditLogger` API. Mode changes (operator mode enable/disable) are also logged as audit entries.
