...
The shape is **53 packages · 7 architectural layers · 5 surfaces + 6 supporting apps · 11 services · 33 open specs**. Layer membership is enforced by `pnpm check-deps` in CI — violations break the build.
...
All 33 specs (listed in the tree above) are Apache-2.0, published in `spec/`, and have working reference implementations in this repo. A third party can stand up an interoperating implementation today using only the published specs and the permissive-floor type packages — no permission required.
