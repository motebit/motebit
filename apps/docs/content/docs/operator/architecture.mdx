...
│   ├── crypto-play-integrity/ [L2]          Google Play Integrity JWT verifier — pinned JWKS, ES256/RS256 dispatch.
│   ├── crypto-tpm/            [L2]          Windows / Linux TPM 2.0 EK chain verifier — pinned vendor roots (Infineon, Nuvoton, STMicro, Intel PTT).
│   ├── crypto-webauthn/       [L2]          WebAuthn platform-authenticator packed-attestation verifier — pinned FIDO root set.
...
│   ├── verify/                [L6]          Canonical `motebit-verify` CLI — the BSL aggregator that bundles the four platform adapter leaves (App Attest, TPM, Play Integrity, WebAuthn) for one-install offline verification.
│   │
