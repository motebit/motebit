...
The shape is **33 packages · 7 architectural layers · 5 surfaces + 3 supporting apps · 8 services · 12 open specs**. Layer membership is enforced by `pnpm check-deps` in CI — violations break the build.
...
The remaining 28 packages are BSL-1.1 — the reference implementation that answers *how the system decides, adapts, routes, and governs*. See `LICENSING.md` for the full boundary test and commercial terms.
