...
The shape is **53 packages · 7 architectural layers · 5 surfaces + 6 supporting apps · 9 services · 33 open specs**. Layer membership is enforced by `pnpm check-deps` in CI — violations break the build.
...
Nine services in four roles. All are reference implementations of the protocol specs — swappable by anyone running a compliant alternative.
