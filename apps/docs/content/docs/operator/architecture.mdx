...
├── packages/
│   │
│   │   # Layer 0 — Foundation (MIT floor; zero internal deps)
...
│   ├── wire-schemas/          [L1]          Zod schemas for wire-format types. Generates committed JSON Schema artifacts into `spec/schemas/` — the protocol-owned MIT tree third parties implement against.
│   │
│   │   # Layer 2 — Engines
│   ├── crypto-appattest/      [L2 · MIT]    Apple App Attest chain verifier — pinned Apple root, deterministic from Apple's public spec.
│   ├── crypto-play-integrity/ [L2 · MIT]    Google Play Integrity JWT verifier — pinned JWKS, ES256/RS256 dispatch.
│   ├── crypto-tpm/            [L2 · MIT]    Windows / Linux TPM 2.0 EK chain verifier — pinned vendor roots (Infineon, Nuvoton, STMicro, Intel PTT).
│   ├── crypto-webauthn/       [L2 · MIT]    WebAuthn platform-authenticator packed-attestation verifier — pinned FIDO root set (Apple, Yubico, Microsoft).
...
│   ├── verifier/              [L6 · MIT]    Library — verifyFile, verifyArtifact, formatHuman helpers over @motebit/crypto. MIT-pure; no bin.
│   ├── verify/                [L6]          Canonical `motebit-verify` CLI — the BSL aggregator that bundles the four MIT platform leaves (App Attest, TPM, Play Integrity, WebAuthn) with motebit-canonical defaults (bundle IDs, RP ID, integrity floor) for one-install offline verification.
│   │
...
Nine packages are MIT (the open protocol). `check-deps.ts` enforces that MIT packages export only types, enums, branded casts, allowlisted utilities, and deterministic verifiers — never algorithms that would bind a competing implementation to this codebase.
...
| `@motebit/crypto-play-integrity` | How is a Google Play Integrity token verified? | L2 |
...
The four `crypto-*` platform leaves verify against each platform's published public trust anchor (Apple root, Google JWKS, vendor TPM roots, FIDO roots) — no motebit-specific judgment. Motebit-canonical aggregation (default bundle IDs, RP ID, CLI shape) lives in `@motebit/verify` (BSL) one layer up.

The remaining 26 packages are BSL-1.1 — the reference implementation that answers *how the system decides, adapts, routes, and governs*. See `LICENSING.md` for the full boundary test and commercial terms.
