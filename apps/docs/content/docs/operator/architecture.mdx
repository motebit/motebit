...
The shape is **53 packages · 7 architectural layers · 5 surfaces + 6 supporting apps · 11 services · 35 open specs**. Layer membership is enforced by `pnpm check-deps` in CI — violations break the build.
...
│   ├── operator/              [supporting]  React + Vite relay-operator console. Withdrawals, federation peers, transparency, disputes, fees, anchoring.
│   ├── identity/              [supporting]  Static browser tool for inspecting motebit.md files.
│   ├── verify/                [supporting]  Static browser receipt verifier. Paste a signed receipt, verified in-tab; separates signature integrity from identity binding. No login, no relay.
...
│   │
│   │   # Layer 1 — Primitives
│   ├── create-motebit/        [L1 · Apache-2.0]    Public scaffolder. npm create motebit.
│   ├── verifier/              [L1 · Apache-2.0]    Library — verifyFile, verifyArtifact, formatHuman helpers over @motebit/crypto. Permissive-floor-pure; no bin.
│   ├── state-export-client/   [L1 · Apache-2.0]    Browser-safe verifier for `X-Motebit-Content-Manifest` HTTP headers on relay state exports. Trust-on-first-use bootstrap from `/.well-known/motebit-transparency.json` produces a `TransparencyAnchor`; per-call `verifiedStateExportFetch` wraps `fetch` with manifest-against-bytes verification + optional producer-key pin; `verifyInnerSignedReceipts` recursively audits v1.1 execution-ledger `signed_receipts[]` entries against each delegate motebit's own embedded key. Sibling of `motebit/verify` (CLI for files on disk); same crypto primitives, different consumer surface. Drift-locked by `check-state-export-consumer-verifies` (#87) + `check-execution-ledger-inner-receipt-verified` (#90).
...
│   ├── runtime-host/          [L1]          Runtime-host election + signed `runtime:attach` handshake. One coordinator per machine; frontends attach over the local socket.
│   ├── sqlite-migrations/     [L1]          Schema-version-tracked migrations across mobile / desktop / persistence. One Migration shape, sync + async runners.
...
│   ├── treasury-reconciliation/ [L1]         Operator-treasury observability: recorded-fee-sum vs onchain `balanceOf`. Sibling of deposit-detector but watches the operator's x402 fee-collection address, never an agent wallet.
│   ├── self-knowledge/        [L1]          Committed BM25 corpus over motebit's public docs. Interior tier of the answer engine.
│   ├── skills/                [L2]          User-installable procedural-knowledge runtime — registry, selector, trust gate, fs adapter. agentskills.io-compatible with motebit-signed extensions.
...
│   ├── relay-client/          [L2]          Typed relay HTTP transport — wire-schema-validated bodies, audience-bound auth, one error shape.
│   ├── identity-file/         [L2]          Generate/parse/verify motebit.md.
│   │
│   │   # Layer 3 — Lower composites
│   ├── verify/                [L3 · Apache-2.0]    Canonical `motebit-verify` CLI — aggregates the canonical Apache-2.0 platform leaves (App Attest, Android Keystore, TPM, WebAuthn; plus deprecated Play Integrity for one minor cycle) with motebit-canonical defaults (bundle IDs, RP ID, integrity floor) for one-install offline verification. The defaults are overridable flags, not proprietary judgment — the permissive-floor line stays here.
...
│   ├── surface-kit/           [L3]          Surface-agnostic controllers (MCP manager); surfaces inject platform ports.
│   │
...
│   │
│   │   # Layer 6 — Application-tier libraries (above the orchestrator; apps/services sit above this)
...
│   ├── code-review/           [molecule]    $0.20/review. Claude-powered, signed receipts.
│   ├── auditor/               [molecule]    $0.01/audit. LLM-free — measures an agent's public verification surface, signs an eval attestation.
│   ├── clerk/                 [molecule]    $0.01/task. Money-execution pole — metered R4 spend under a self-issued signed grant, fail-closed, dry-run-first.
│   ├── web-search/            [atom]        $0.003/request. Priced + p2p now the multi-hop settlement arc landed; single-hop + multi-hop delegation proof.
...
│   ├── browser-sandbox/       [atom]        Playwright-driven Chromium for the virtual_browser embodiment — second computer-use-v1 producer.
...
│   ├── relay-transparency-v1.md             Signed operator-transparency declaration — the trust-anchor primitive every motebit verifier pins.
...
│   ├── settlement-invoice-v1.md             The bill that extends the receipt chain to the money — issuer-signed CostAttestation + Invoice, offline-verifiable; motebit owns the format, the issuer runs the rails.
│   ├── auth-token-v1.md                     Signed bearer tokens, audience binding, replay prevention.
│   ├── signed-request-envelope-v1.md        Stateless per-request identity auth: registered-key, detached payload digest, audience binding.
...
│   ├── standing-delegation-v1.md            Standing (cadence-scoped, revocable) grants that mint short-lived per-tick delegation tokens; delegation revocation.
│   ├── discovery-v1.md                      .well-known, DNS SRV, agent resolution, relay metadata.
│   ├── agent-revocation-v1.md               Operator de-listing of agents from Discover — signed, reasoned, public feed. De-list, not de-identify; hygiene, not curation.
│   ├── migration-v1.md                      Departure attestation, credential export, trust bootstrap.
│   ├── seed-escrow-v1.md                     Durability without custody: authenticator-wrapped seed, opaque to its custodian, relay-optional restore.
...
│   ├── consolidation-mutation-manifest-v1.md  Signed owner-facing commitment to a cycle's exact formed/refined mutations — the felt-interior binding.
...
│   ├── skills-v1.md                         agentskills.io-compatible procedural-knowledge files with motebit-namespaced extensions for cryptographic provenance, sensitivity-tiered loading, and HA gating.
│   ├── skills-registry-v1.md                Relay-hosted index of submitted skill envelopes. Permissive submit, curated discovery, content-addressed identity, consumer-side re-verification.
│   ├── bond-v1.md                           Agent's own sovereign capital as an anti-sybil staked signal — self-signed proof-of-funds, RPC-verified never custodied; address bound to identity. Phase 1: signal, not recourse.
│   ├── evidence-provenance-v1.md            Re-verifiable provenance on a verdict's EvidenceRef — a content-addressed verbatim span in a primary record; the byte-determinism conformance requirement for projection recipes. Verifiable-locality from signatures to evidence.
│   ├── eval-attestation-v1.md               Signed third-party measurement (subject ≠ signer) — per-axis verdicts embedded whole, closed EvalKind intake, envelope law only; the Auditor archetype's deliverable.
│   ├── routing-transcript-v1.md             Signed routing-decision transcript (subject = signer, receipt-family) — the frozen candidate set, posterior reads and draws, and decision parameters behind a paid hire; integrity + recomputable faithfulness, two rungs.
│   └── machine-roster-v1.md                 Which machines a motebit runs unattended work on — a sovereign-signed SET (enrolment + retirement, id = hash, merge = union, remove wins), relay-transported verbatim. Quantify over membership, annotate with liveness; never a lease, never an admission gate.
│
└── scripts/                   # Drift-defense CI gates
    ...
    └── check-gates-effective.ts             Meta-gate: probes every gate into failure to prove it bites and emits a repair instruction.
```

## Layer rule

Layer N may depend on layers `0..N-1` in production and `0..N` in devDependencies. Apps and services sit in the **application tier above** the seven package layers (`L0`–`L6`) and may depend on any layer — deliberately unenforced, because an app IS the top of the DAG. That tier is deliberately not one of the seven: while it shared the number `6`, every package explicitly mapped to L6 silently inherited the app tier's exemption and got no layer enforcement at all (#544). The canonical source of truth is the `LAYER` map in `scripts/check-deps.ts`; the tree above mirrors it. `github-action` is a standalone build artifact for the GitHub Actions runtime and sits outside the DAG.
...
| `@motebit/verifier`                             | How does a third party compose offline verification?               | L1    |
| `@motebit/verify`                               | How does a one-install CLI verify any motebit artifact end-to-end? | L3    |
| `@motebit/state-export-client`                  | How does a browser consumer verify a relay's signed state export?  | L1    |
| `create-motebit`                                | How does a third party scaffold against the protocol?              | L1    |

The four canonical `crypto-*` platform leaves (`crypto-appattest`, `crypto-android-keystore`, `crypto-tpm`, `crypto-webauthn`) verify against each platform's published public trust anchor (Apple App Attest root, Google Hardware Attestation roots, TPM vendor roots, FIDO roots) — no motebit-specific judgment. `@motebit/verify` aggregates them with motebit-canonical defaults (bundle IDs, RP ID) into one install, but the defaults are overridable flags, not proprietary judgment — the permissive-floor line stays at the aggregator. The BSL line holds at `motebit` (the operator console) and everything below it: daemon, MCP server, delegation routing, market integration, federation wiring — the actual reference-implementation judgment.

`motebit-verify` covers six artifact categories under one Apache-2.0 CLI: identity files, execution receipts, credentials (with hardware-attestation channels when present), presentations, skill envelopes, and content-artifact manifests on relay-assembled state exports. The last category — `motebit-verify content-artifact <body> --manifest <header>` — verifies the relay-asserted `X-Motebit-Content-Manifest` HTTP header emitted on every endpoint registered in `services/relay/src/state-export.ts`. Trust anchor: `relay_public_key` from `/.well-known/motebit-transparency.json` (which is itself self-signed and self-attesting). The 12 canonical content-artifact categories live in the closed `ContentArtifactType` registry in `@motebit/protocol`, drift-locked by `check-artifact-type-canonical`; the per-endpoint wiring requirement is drift-locked by `check-state-export-signed`. See [`docs/doctrine/nist-alignment.md`](https://github.com/motebit/motebit/blob/main/docs/doctrine/nist-alignment.md) §8.

The remaining packages are BSL-1.1 — the reference implementation that answers _how the system decides, adapts, routes, and governs_. Both license families converge to Apache-2.0 at each version's Change Date (four years after first public release), eliminating the two-license end state. See `LICENSING.md` for the full boundary test, convergence story, and commercial terms.
...
| `desktop`  | Surface    | Live     | Tauri liquescent droplet. Three.js creature, identity, operator mode, voice.                                   |
...
| `operator`  | Supporting | Internal | React + Vite relay-operator console. 10 tabs: health (default), withdrawals, federation peers, transparency, disputes, fees, credential anchoring, treasury reconciliation, receipts, freeze. Master-token gated; fleet-shaped. |
...
Eleven services in four roles. All are reference implementations of the protocol specs — swappable by anyone running a compliant alternative.

| Role          | Services                                                       | Pattern                                                                                                                                                            |
| ------------- | -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Relay**     | `relay`                                                        | The only function with a legitimate centralization premium — multi-device sync. Also: settlement, federation, discovery, 5-tier rate limiting, dispute resolution. |
| **Molecules** | `research` ($0.25/report), `code-review` ($0.20/review), `auditor` ($0.01/audit), `clerk` ($0.01/task) | Agents that reason and compose. `research`/`code-review` are Claude-powered and emit signed delegation receipt chains; `auditor` is LLM-free — it measures another agent's public verification surface and signs an eval attestation (subject ≠ signer); `clerk` is the money-execution pole — it runs a metered R4 spend under a self-issued signed grant, fail-closed, dry-run-first.          |
| **Atoms**     | `web-search` ($0.003/request), `read-url` ($0.002/request), `summarize`, `embed`, `browser-sandbox` — priced + p2p now the multi-hop settlement arc landed | Capability providers. Single-hop reference implementations of the delegation spec; `browser-sandbox` is per-session rather than stateless.      |
...
All 35 specs (listed in the tree above) are Apache-2.0, published in `spec/`, and have working reference implementations in this repo. A third party can stand up an interoperating implementation today using only the published specs and the permissive-floor type packages — no permission required.
...
2. **Store** the private key on the device — an owner-only (`0600` on Unix; Windows is best-effort) key file on desktop (the OS keychain is not used yet), PBKDF2-encrypted file (600K iterations) on CLI, expo-secure-store on mobile, IndexedDB with a derived-key envelope on web. The key never leaves the device.
