...
The remaining 25 packages are BSL-1.1 — the reference implementation that answers _how the system decides, adapts, routes, and governs_. Both license families converge to Apache-2.0 at each version's Change Date (four years after first public release), eliminating the two-license end state. See `LICENSING.md` for the full boundary test, convergence story, and commercial terms.
