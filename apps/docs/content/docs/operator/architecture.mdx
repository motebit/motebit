...
│   ├── auditor/               [molecule]    $0.05/audit. LLM-free — measures an agent's public verification surface, signs an eval attestation.
│   ├── clerk/                 [molecule]    $0.05/task. Money-execution pole — metered R4 spend under a self-issued signed grant, fail-closed, dry-run-first.
...
| **Molecules** | `research` ($0.25/report), `code-review` ($0.50/review), `auditor` ($0.05/audit), `clerk` ($0.05/task) | Agents that reason and compose. `research`/`code-review` are Claude-powered and emit signed delegation receipt chains; `auditor` is LLM-free — it measures another agent's public verification surface and signs an eval attestation (subject ≠ signer); `clerk` is the money-execution pole — it runs a metered R4 spend under a self-issued signed grant, fail-closed, dry-run-first.          |
| **Atoms**     | `web-search` ($0.05/request), `read-url`, `summarize`, `embed`, `browser-sandbox` | Capability providers. Single-hop reference implementations of the delegation spec; `browser-sandbox` is per-session rather than stateless.      |
