...
| `packages/` | 31 shared TypeScript packages — the interior of the system. |
| `services/` | Sync relay (`api`), reference paid agents (`research`, `code-review`), capability providers (`web-search`, `read-url`, `summarize`, `embed`), and the Vercel edge proxy (`proxy`). |
| `spec/` | 12 open specifications — `identity-v1.md`, `execution-ledger-v1.md`, `relay-federation-v1.md`, `market-v1.md`, `credential-v1.md`, `settlement-v1.md`, `auth-token-v1.md`, `credential-anchor-v1.md`, `delegation-v1.md`, `discovery-v1.md`, `migration-v1.md`, `dispute-v1.md`. |
...
| Package | Role |
|---------|------|
| `protocol` | Network protocol — identity, receipts, credentials, delegation, settlement, trust algebra. Types, semirings, routing primitives. MIT, zero deps. |
| `crypto` | Sign and verify every Motebit artifact — identity files, execution receipts, credentials, delegations, succession records, credential anchors. Cryptosuite-agile for post-quantum. MIT, zero monorepo deps. |
| `sdk` | Developer contract — stable types, adapter interfaces, governance config; product vocabulary the runtime consumes (`MotebitState`, `BehaviorCues`, `RenderSpec`, etc.). Re-exports `protocol`. MIT. |
...
| `mcp-server` | Expose a motebit as an MCP server — bearer auth, synthetic tools, signed receipt construction. |
| `sync-engine` | Multi-device sync — HTTP/WebSocket adapters, conflict detection, retry backoff. |
| `market` | Budget allocation, settlement, reputation aggregation, graph-based routing. |
| `semiring` | Trust algebra — agent network graph construction, multi-objective ranking, provenance tracking, trust state transitions. |
| `wallet-solana` | Sovereign Solana USDC rail — Ed25519 identity key IS the Solana address. RPC adapter, ATA management, SPL transfers. |
...
| `identity-file` | Generate, parse, verify `motebit.md` — signed agent identity files. |
| `create-motebit` | Public CLI — `npm create motebit` scaffolds a signed identity (`motebit.md`) or a runnable agent service (`--agent`). MIT. |
| `planner` | PlanEngine — decomposes goals into multi-step plans with dependency tracking, plan-level reflection. |
