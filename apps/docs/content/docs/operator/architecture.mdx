...
│   ├── web-search/            [atom]        Zero-cost until the multi-hop settlement arc (then ~$0.003/request). Single-hop + multi-hop delegation proof.
...
| **Atoms**     | `web-search`, `read-url`, `summarize`, `embed`, `browser-sandbox` (zero-cost until the multi-hop settlement arc) | Capability providers. Single-hop reference implementations of the delegation spec; `browser-sandbox` is per-session rather than stateless.      |
