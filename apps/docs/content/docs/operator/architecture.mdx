...
The shape is **53 packages · 7 architectural layers · 5 surfaces + 6 supporting apps · 10 services · 33 open specs**. Layer membership is enforced by `pnpm check-deps` in CI — violations break the build.
...
Ten services in four roles. All are reference implementations of the protocol specs — swappable by anyone running a compliant alternative.
...
| **Molecules** | `research` ($0.25/report), `code-review` ($0.50/review), `auditor` ($0.05/audit) | Agents that reason and compose. `research`/`code-review` are Claude-powered and emit signed delegation receipt chains; `auditor` is LLM-free — it measures another agent's public verification surface and signs an eval attestation (subject ≠ signer).          |
