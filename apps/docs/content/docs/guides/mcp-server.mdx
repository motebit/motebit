...
Any agent receiving the receipt can verify it was executed by this specific motebit using [`@motebit/verify`](/docs/guides/identity-standard).
...
In addition to the six synthetic tools, the server exposes all tools from the motebit's [tool registry](/docs/guides/tools) — built-in tools and connected MCP server tools. Every proxied tool call passes through the [policy gate](/docs/concepts/governance):
