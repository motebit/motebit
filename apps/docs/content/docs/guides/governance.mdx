...
For the conceptual foundation, see [Governance](/docs/concepts/governance). For where policy sits in the package architecture, see [Architecture](/docs/architecture).
...
The audit log is queryable via the [admin dashboard](/docs/architecture), the CLI (`motebit approvals list`), and programmatically through the `AuditLogger` API. Mode changes (operator mode enable/disable) are also logged as audit entries.
