...
SQLite in WAL mode. Stores events, memories, identities, devices, audit logs, and state snapshots. Each table uses the standard adapter pattern — see [Event Sourcing](/docs/architecture/event-sourcing) for schema details.
