...
export function MemoryGraphPanel({ memories, edges, onDelete }: MemoryGraphPanelProps): React.ReactElement {
  ...
  const tooltip = hoveredNode && hoveredNode.x != null && hoveredNode.y != null
    ? h("g", { key: "tooltip", pointerEvents: "none" },
        h("rect", {
          x: hoveredNode.x + 12,
          y: hoveredNode.y - 30,
        ...
        h("text", {
          ...
          fill: "#e0e0e0",
          fontSize: 11,
        }, hoveredNode.content.length > 50 ? hoveredNode.content.slice(0, 50) + "..." : hoveredNode.content),
        h("text", {
          x: hoveredNode.x + 20,
          y: hoveredNode.y + 2,
          fill: "#8888aa",
  ...
  const memoryList = memories.slice(0, 20).map((m) =>
    h("div", { key: m.node_id, className: "memory-node" },
