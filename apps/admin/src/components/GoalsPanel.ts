...
function statusBadge(goal: GoalEntry): { text: string; className: string } {
  ...
  if (goal.status === "paused" || !goal.enabled) return { text: "paused", className: "goal-status paused" };
...
export function GoalsPanel({ goals }: { goals: GoalEntry[] }): React.ReactElement {
  ...
  return React.createElement("div", { className: "panel" },
    ...
    ...sorted.map((g) => {
      ...
      return React.createElement("div", { key: g.goal_id, className: "event-entry goal-entry" },
        React.createElement("div", { className: "goal-header" },
          ...
          React.createElement("span", { className: "goal-interval" }, formatInterval(g.interval_ms)),
          g.consecutive_failures > 0
            ? React.createElement("span", { className: "goal-failures" }, `${g.consecutive_failures} failures`)
            : null,
        ),
