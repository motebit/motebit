...
export const DESKTOP_MIGRATIONS: readonly Migration[] = [
  ...
  },
  {
    ...
    statements: [
      // Sibling of persistence migration v38 — `@motebit/protocol::SettlementRecord`
      // gained a required `motebit_id` (payee) so a settlement receipt names who
      // was paid in its signed body, not only the relay-internal allocation_id.
      // Nullable for backward-compat reads: rowToSettlement surfaces legacy rows
      // as "" so wire-schema validation rejects them — the intended fail-closed
      // signal. New writes persist the payee explicitly.
