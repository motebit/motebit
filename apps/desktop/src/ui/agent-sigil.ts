 ...
 * surface's. This file is the per-surface sibling of `apps/web/src/identity-sigil-svg.ts`;
 * the code region below `SigilSvgOptions` is byte-identical to it and locked by
 * `check-sigil-renderer-parity` — renderers live per-surface (params-not-pixels:
 * the shared `@motebit/sdk` emits sigil PARAMS, surfaces emit pixels), and the
 * gate keeps the two DOM siblings from drifting.
 *
