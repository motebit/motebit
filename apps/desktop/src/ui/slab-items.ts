...
function applyDelegationPayload(
  ...
  if (!summary) {
    // Outbound — packet has left, no receipt yet. Sibling-mirrored
    // to apps/web/src/ui/slab-items.ts. Doctrine: motebit-computer.md
    // §Hand "a packet leaves the slab toward a peer, returns as a
    // bead with a signed receipt." Body names the wait-state so the
    // card isn't a silent void during the outbound interval.
 ...
 * (packet sent, receipt not yet returned). Sibling-mirrored to
 * apps/web/src/ui/slab-items.ts. Strips the truncation ellipsis so
 * it doesn't collide with the wait-message's own trailing ellipsis.
 * Exported for tests.
 */
