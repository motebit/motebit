...
| **Leaf content**       | Whole signed `FederationSettlementRecord`           | Whole signed `SettlementRecord`             | Full W3C VC 2.0 with proof                        |
...
### 9.1 Converged: federation-anchor (the RFC 6962 arc-closer)

The per-agent stream (this spec) and the credential stream are **self-verifiable offline** — a holder reconstructs the leaf from the exact signed artifact it holds, the batch payload binds `suite`, and a portable `verify*` in `@motebit/crypto` closes the loop (`verifyAgentSettlementAnchor`, `verifyCredentialAnchor`). The **federation settlement stream now is too**, as of the RFC 6962 tree-hash arc's closing PR (PR6 — `docs/doctrine/merkle-tree-hash-versioning.md` §8). The three counts it closed:

1. **Leaf was a column projection, not a whole-artifact hash.** The old `computeSettlementLeaf` hashed a hand-typed subset of `relay_federation_settlements` fields — a re-typed field list a holder could not reproduce. **Resolved:** verified against the bytes, no signed federation-settlement artifact existed to hash, so the convergence MINTS a relay-signed `FederationSettlementRecord` (`@motebit/protocol`), persists `canonicalJson(record)` verbatim (`relay_federation_settlements.record_json`), and the leaf became `SHA-256(canonicalJson(the signed record))` — `computeFederationSettlementLeaf` (`@motebit/crypto`) / `federationSettlementLeaf` (relay producer). The projection function was deleted.
2. **Batch payload omitted `suite`.** **Resolved:** the federation `cutBatch` `anchorPayload` now binds `suite` (signature-bound), like §4.2.
3. **No portable verifier, and untracked.** **Resolved:** `FederationSettlementAnchorProof` lands in `@motebit/protocol` and `verifyFederationSettlementAnchor` in `@motebit/crypto` (the fourth Merkle consumer); `check-signed-artifact-verifiers` (#107) tracks it as a `verifier`, and a peer self-verifies a federation inclusion proof with `@motebit/crypto` alone.

Safe at the clean window the trigger named — no production federation-settlement-anchor data existed when the bytes changed. See `spec/relay-federation-v1.md` §7.6 for the converged wire format and §7.6.9 for the tree-hash version.
