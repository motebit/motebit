...
| Skill `scripts/` execution requested at runtime                                | v1 does NOT auto-execute scripts. Operator-mode + per-script approval deferred to phase 2.                                                                                                                                                                                      |
...
| 2                 | `SkillSelector` wired into the runtime context-injection path; per-turn relevance ranking. Script execution governance: `scripts/` extracted to a quarantine path on install; first execution requires per-script operator approval (sibling pattern to existing tool-approval flow). |
