...
A relay that stores and serves succession records carries three obligations, because the signed payload (§3.8.1) names two keys and no `motebit_id` — a record cannot say whose history it belongs to, so the route must.
...
3. **An identity's key history never repeats a key, on either side of a link.** A relay MUST refuse a record whose `new_public_key` equals any key (compared as a verifier decodes it: the key's bytes, not its spelling) named on either side of any succession it has recorded for that identity, or the key it holds for the identity. It MUST likewise refuse a record whose `old_public_key` is named anywhere in that recorded history other than as the latest link's `new_public_key`: a key the history has retired is never departed from again, whatever the key on file says, or the history forks (K1→K2, then K1→K3). A rotation back to an earlier key (A→B→A) is therefore refused: a history that repeats a key cannot be read as an ordered succession of distinct keys (the machine-roster law refuses it as `duplicate_key`, `machine-roster-v1.md` §6), and returning to a key re-opens every record that departs from it. A record whose `(old_public_key, new_public_key)` pair is already recorded is a retry only when it is the latest recorded link: the relay MUST then change nothing and append nothing. Any other already-recorded pair — for example a guardian recovery (§3.8.3) re-presented after the identity has rotated on — MUST be refused and never applied again. A refusal under this obligation is a conflict with recorded state, not a malformed request.
