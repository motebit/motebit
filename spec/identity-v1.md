...
A relay that accepts bearer tokens verified against per-device key records carries five obligations on these routes, because at such a relay a key record is a credential. They turn on one definition. A device may be registered under its OWN key (linked without key transfer); it is a first-person caller, and its key is not the identity's. **The identity key, as far as a relay can prove it**, is, in order: the key the relay holds for the identity; else the head of the succession it has recorded; else a key the `motebit_id` itself commits to; else, for an id that commits to no key, the one key every keyed device record of the identity agrees on. Where none decides, the relay MUST NOT guess.

1. **A succession is the identity's own act.** `rotate-key` MUST refuse a record presented under a token for any other `motebit_id`: the signed payload (§3.8.1) names two keys and no identity, so the record alone cannot say whose history it belongs to. The record's `old_public_key` MUST be the provable identity key.
2. **A rotation ends the old key.** In one transaction with recording a succession, the relay MUST replace the old key in every device record of that identity that held it, and MUST end the identity's open sessions, which were authenticated under it. A device registered under its own key MUST be left unchanged. Rotation is the remedy for a lost or stolen machine (§8.2); a relay that goes on verifying tokens against the rotated-away key has recorded the remedy without applying it. A machine that has not yet received the new key is refused until it does. Two successions departing from one key MUST NOT both be recorded.
3. **A guardian is installed by the identity key.** The guardian `attestation` (§3.3) proves the guardian's consent, not the identity's. A relay MUST accept a guardian only from a caller whose token was verified under the identity key, and MUST NOT replace an installed guardian as a side effect of registration (§3.3.1: the identity key holder must cooperate). A guardian can recover an identity to any key (§3.8.3), so who may install one is who may take the identity.
4. **A first registration establishes only a proven key.** Where the relay holds no key for the identity, a first-person caller MAY establish one only if it is the provable identity key and the key that caller's own token was verified under. No other path — a receipt, in particular — may set or change the key a relay holds for an identity.
5. **A caller is judged by the key it proved, when it proved it.** A relay MUST judge a caller by the key its token was verified under at verification, not by re-reading a key record later, and MUST refuse a caller whose proven key is no longer a credential at the moment of the write: a request can be held open across a rotation. A key-transfer route that takes no bearer MUST write nothing but the provable identity key.
