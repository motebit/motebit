...
| `device:auth`           | Ad-hoc authenticated relay reads; `POST /api/v1/agents/{id}/roster`, `GET /api/v1/agents/{id}/roster` (machine roster, `spec/machine-roster-v1.md` §11)                                                    | Per-device auth headers on relay calls. The roster routes also require the token to verify under a registered DEVICE row's key (never the agent-registry fallback) and name the path's motebit                                  |
