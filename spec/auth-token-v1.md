...
| `device:auth`           | Ad-hoc authenticated relay calls — reads, and the machine roster's writes and reads: `POST /api/v1/agents/{id}/roster`, `GET /api/v1/agents/{id}/roster` (machine roster, `spec/machine-roster-v1.md` §11) | Per-device auth headers on relay calls. The roster routes also require the token to verify under a registered DEVICE row's key (never the agent-registry fallback) and name the path's motebit                                  |
...
| `admin:query`           | Agent-registry routes: `POST /api/v1/agents/register`, `/heartbeat`, `DELETE /deregister`, `GET /api/v1/agents/{id}`; the default audience for `/api/v1/agents/*` routes not named elsewhere in this table | A service's self-signed registration family (the worker authenticates to its relay as itself — no operator secret) plus agent-registry reads. `/api/v1/admin/*` is operator-only and takes the master bearer, not this audience |
