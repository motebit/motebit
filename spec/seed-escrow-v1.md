...
**Identity substitution.** A malicious custodian could return a _different_ valid payload (an attacker's seed) hoping the client adopts it. `identity_pubkey_check` defeats this only if the client compares against its own expected key when it has one. A fresh-device restore has no prior key, so the binding anchor is NOT the payload's self-asserted check field — it is the **identity registry / key-transparency log** (`identity-binding-verification`): the restoring client MUST confirm the restored seed re-derives to the `motebit_id` it intends to restore, per the registry, before first use. For sovereign seed-derived ids the seed→`motebit_id` derivation IS that binding.
