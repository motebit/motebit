...
SignedRequestEnvelope {
  motebit_id:      string      // Requesting identity — UUIDv7 string
...
with the payload as the raw request body. The verifier digests the body as received (after transfer decoding, before any parsing-normalization) — the digest is over `canonicalJson` of the parsed payload, so JSON whitespace differences do not break verification.
...
- Signing: `@motebit/crypto` — `signRequestEnvelope(payload, identity, aud)` *(to be published with this spec)*
- Verification: `@motebit/crypto` — `verifyRequestEnvelope(envelope, payload, registeredKey, expectedAud)`; re-exported by `@motebit/verifier`
