...
- `read_page` — ax-tier observation companion. Surfaces with an open browser session (`virtual_browser` embodiment) SHOULD register this tool alongside `computer` so the AI's reasoning loop has a structured-text path that does NOT require a screenshot. `mode: "ax"` per `@motebit/protocol`'s `ToolMode`; the registry sort lands it ahead of `computer({kind:"screenshot"})` (`mode: "pixels"`) so the AI defaults to the cheaper structured tier when the question is "what's on the page." Input is `{ session_id?: string }`; output is a `ReadPageResult` carrying `url`, `title`, body `text` (bounded; `text_truncated: bool` set when the cap fired), heading hierarchy in document order, and visible links with absolute hrefs. **No pixel transmission** — the privacy invariant `screenshot bytes never reach external AI` is unchanged. Page text crosses the AI boundary subject to the same sensitivity gate that governs `read_url` / `web_search` (`outbound: true`).
...
- **back** / **forward** / **reload** (Slice 2e) — browser history navigation. Parameter-less; the kind alone discriminates. Server-side: `page.goBack` / `page.goForward` / `page.reload`, all with the same `{ waitUntil: "domcontentloaded", timeout: 15_000 }` shape as navigate. Empty-history semantics: `back` / `forward` against a session with no matching history MUST be a no-op (Playwright returns null; the wire treats null as 204 success). Matches real-browser UX: the user clicks the button and nothing changes.
...
- **back** / **forward** / **reload** — `{ kind: "back" }` / `{ kind: "forward" }` / `{ kind: "reload" }`. Parameter-less; the kind alone is the audit shape. Nothing to redact — history navigation carries no user-supplied data.
