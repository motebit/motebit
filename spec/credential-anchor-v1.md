...
**Tree-hash domain separation.** Under the `merkle-sha256-rfc6962-v2` tree-hash version (§5.4) the leaf hash gains the RFC 6962 §2.1 leaf-domain tag: `credential_leaf = SHA-256(0x00 ‖ canonicalJson(credential))`. The `canonicalJson(...)` entry bytes are identical to the `merkle-sha256-plain-v1` (untagged) form above — only the `0x00` prefix is added — so producer and verifier derive the same leaf as long as both dispatch on the proof's declared version.
...
CredentialAnchorProof {
  ...
  tree_hash_version?: string    // MerkleTreeVersion (§5.4); absent ⇒ "merkle-sha256-plain-v1"
}
```
...
`tree_hash_version` is OPTIONAL — **absent ⇒ `merkle-sha256-plain-v1`** (every proof minted before this axis existed still verifies). It is NOT part of the signed batch payload (§4.2): the version is bound transitively through `merkle_root` (a given leaf set under a given version produces exactly one root, and the relay signs that root), so a verifier that flips the version reconstructs a different root and fails step 2 (Merkle) or step 3 (signature). See §5.4.

### 5.2 Verification Algorithm

Given a credential `vc` and its `CredentialAnchorProof`:

1. **Hash verification:** `credential_leaf(vc) === proof.credential_hash`, computing the leaf under `proof.tree_hash_version` (absent ⇒ `merkle-sha256-plain-v1`; an unknown value MUST be rejected fail-closed, never downgraded — §5.4)
2. **Merkle inclusion:** Verify `(leaf=credential_hash, index=leaf_index, siblings, layer_sizes)` reconstructs to `merkle_root`, dispatching the leaf/node domain tags on the same resolved `proof.tree_hash_version`
...
The reference relay stores `merkle_root`, `leaf_count`, `first_issued_at`, `last_issued_at`, `signature`, and `tree_hash_version` on the `relay_credential_anchor_batches` row; `siblings` and `layer_sizes` are reconstructed from the ordered `credential_ids` at proof-serve time, hashed under the row's stored `tree_hash_version` (NULL ⇒ `merkle-sha256-plain-v1`). Alternative implementations MAY precompute and persist every proof, or compute them lazily. The wire format above is what crosses the boundary.
...
### 5.4 Tree-hash version (RFC 6962 §2.1 domain separation)
...
- `merkle-sha256-rfc6962-v2` — `status: preferred`. RFC 6962 §2.1 (`leaf = SHA-256(0x00 ‖ entry)`, `node = SHA-256(0x01 ‖ left ‖ right)`), giving the leaf-vs-node second-preimage resistance the §3 leaf-hash construction needs to make a leaf indistinguishable from an interior node.
...
**Deploy-verifier-first.** Every verifier surface MUST accept v2 _before_ any producer emits it — otherwise v2 proofs strand at a lagging verifier. The portable verifier (`@motebit/crypto`'s `verifyCredentialAnchor`) accepted v2 from PR1 part 2b; this spec's reference producer flips to v2 in PR3. Already-anchored v1 batches keep their committed v1 root: the reference relay persists `tree_hash_version` per batch and reconstructs each under its own version (a pre-PR3 batch reads NULL ⇒ v1).
...
tree_hash_producer: services/relay/src/credential-anchoring.ts
```
