...
The `routing_choice` field is present when the relay performed scored routing and returned provenance data. It records which agent was selected, the composite score that justified the selection, the planned execution route whose execution metrics that score reflects (with the planned-route alternatives it beat), and the vouching path behind the trust value. The two paths answer two different questions and MUST NOT be conflated: a recorded delegation is **evidence** (it can raise trust in an agent) but the task does not traverse it, so its cost, latency, reliability and custody never enter this hire's metrics; only hops the task takes (the direct hire, a federation relay leg) do. The record is exact by construction — the ranking runs over the Pareto frontier of real planned execution routes per candidate, with trust taken from the best evidence path — so a verifier can recompute both from the same edges. Neither path asserts who performed the task; participation is proven by receipts (§4.1, §6).
...
| `routing_paths`           | string[][] | PLANNED EXECUTION routes to the selected agent, each an ordered list of node MotebitIds after the delegator — the hops the task takes (`[selected_agent]` for a direct hire; `[peer_relay, selected_agent]` for a federated one). `routing_paths[0]` MUST be the route whose composed execution metrics (cost, latency, reliability, risk, custody) the score reflects; further entries are the non-dominated planned-route alternatives the policy weighed, best first. Every entry is a route that exists; a component-wise optimum no single route attains MUST NOT be reported. A pinned hire reports `[[selected_agent]]`. |
| `alternatives_considered` | number     | Number of non-dominated planned execution routes to the selected agent the policy chose among (`routing_paths.length` when unbounded); `0` for a pinned hire.                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `trust_evidence_path`     | string[]   | OPTIONAL (additive, 2026-09-14). The path through EVERY edge of the routing graph — recorded delegations included — whose composed trust is `sub_scores.trust`: a chain of vouching, never a record of who executed. It may differ from `routing_paths[0]` (an intermediary can vouch for an agent the delegator then hires directly). Absent for a pinned hire.                                                                                                                                                                                                                                                                |
