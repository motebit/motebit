...
| `routing_paths`           | string[][] | EXECUTED routes to the selected agent, each an ordered list of node MotebitIds after the delegator — the hops the task takes (`[selected_agent]` for a direct hire; `[peer_relay, selected_agent]` for a federated one). `routing_paths[0]` MUST be the route whose composed execution metrics (cost, latency, reliability, risk, custody) the score reflects; further entries are the non-dominated executed alternatives the policy weighed, best first. Every entry is a route that exists; a component-wise optimum no single route attains MUST NOT be reported. A pinned hire reports `[[selected_agent]]`. |
| `alternatives_considered` | number     | Number of non-dominated executed routes to the selected agent the policy chose among (`routing_paths.length` when unbounded); `0` for a pinned hire.                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
