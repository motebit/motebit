...
The `routing_choice` field is present when the relay performed scored routing and returned provenance data. It records which agent was selected, the composite score that justified the selection, and the path through the routing graph whose composed metrics that score reflects (with the alternatives it beat). The record is exact by construction: the ranking runs over the Pareto frontier of real paths per candidate (path-preserving), so the reported metrics are one existing path's own and a verifier can recompute them from the same edges. The path is **evidence, not execution**: the graph's edges are recorded delegation history, discovery and federation peers, so `routing_paths[0]` justifies the choice of `selected_agent`; it does not assert that each intermediary took part in performing the task. Participation is proven by receipts (§4.1, §6).
...
| `routing_paths`           | string[][] | Routes to the selected agent, each an ordered list of node MotebitIds after the delegator. `routing_paths[0]` MUST be the path whose composed edge metrics `sub_scores` reflects — the evidence that justified selecting `selected_agent`. Any further entries are the non-dominated alternative paths the policy weighed, best first. Every entry is a path that exists in the routing graph; a component-wise optimum that no single path attains MUST NOT be reported. A path is a trust justification, not a record of who executed the task. A pinned hire reports `[[selected_agent]]`. |
| `alternatives_considered` | number     | Number of non-dominated viable routes to the selected agent the policy chose among (`routing_paths.length` when unbounded); `0` for a pinned hire.                                                                                                                                                                                                                                                                                                                                                                                                                                            |
