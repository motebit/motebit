...
The `routing_choice` field is present when the relay performed scored routing and returned provenance data. It records which agent was selected, the composite score that justified the selection, and the route through the routing graph that was actually hired (with the alternatives it beat). The record is exact by construction: the ranking runs over the Pareto frontier of real routes per candidate (path-preserving), so the reported metrics are the chosen route's own and a verifier can recompute them from the same edges.
...
| `routing_paths`           | string[][] | Routes to the selected agent, each an ordered list of node MotebitIds after the delegator. `routing_paths[0]` MUST be the route that was chosen — the route whose composed edge metrics `sub_scores` reflects. Any further entries are the non-dominated alternative routes the policy weighed, best first. Every entry is a route that exists in the routing graph; a component-wise optimum that no single route attains MUST NOT be reported. A pinned hire reports `[[selected_agent]]`. |
