...
- f460df1: Teach the sovereign binding check `did:key` delegators — close an identity-binding parity gap caught by agency.computer (second verify-family consumer).
...
- f6d820a: Close fail-open gaps in the credential verifier and make `suite-dispatch` fail-closed on an unknown suite (cold-audit remediation).
...
- f15fbc2: `verifyRequestEnvelope` now checks freshness **by default** — closing a fail-open replay window (agency-reported, same-day post-publish).
...
- c8a5819: Add the self-signed motebit-announcement primitive — `signMotebitAnnouncement` / `verifyMotebitAnnouncement`, the `SignableMotebitAnnouncement` / `MotebitAnnouncementVerifyResult` / `AnnouncementSurface` types, and the `MOTEBIT_ANNOUNCEMENT_SUITE` / `MOTEBIT_ANNOUNCEMENT_MAX_AGE_MS` constants.
...
- 26f4040: Cite the spec section in `verifyReceipt` failure messages, matching the Python reference verifier byte-for-byte.
