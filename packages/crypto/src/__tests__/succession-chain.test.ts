...
async function createSuccessionRecordWithTimestamp(
  ...
  // We need to import ed25519Sign and canonicalJson to create records with specific timestamps
  const { ed25519Sign, canonicalJson, bytesToHex } = await import("../index.js");
  ...
  const oldSig = await ed25519Sign(message, oldKeyPair.privateKey);
  const newSig = await ed25519Sign(message, newKeyPair.privateKey);
