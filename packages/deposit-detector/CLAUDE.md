...
6. **Confirmation depth is required, not optional.** Every `detectDeposits` call MUST declare `confirmations: number`. The cycle never crosses `currentBlock - confirmations` (the safe horizon), so a chain reorg shallower than `confirmations` cannot roll back a credit — the credit hadn't been issued at that depth yet. The cursor likewise advances only over confirmed blocks. Testnets at `0` are explicit (fast feedback in dev); mainnet chains MUST declare a real depth (12 for Base / Optimism / Arbitrum / Ethereum, 64 for Polygon — mirror Coinbase's published depths). The relay's `CONFIRMATIONS_BY_CHAIN` map (`services/relay/src/deposit-detector.ts`) is the canonical per-chain registry; the drift gate `check-deposit-detector-confirmations` enforces every chain in `USDC_CONTRACTS` has a non-zero entry on mainnet.
