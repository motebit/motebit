...
- **Provider-mode resolver** (`provider-mode.ts`, `provider-resolver.ts`) — the rules for choosing between sovereign / BYOK / subscription provider modes. A third party integrator needs this logic to match motebit's runtime; the underlying types (`ProviderMode`, `SensitivityLevel`) belong to the protocol, but the resolver is the developer's copy-paste-stable starting point. The resolver is a pure dispatcher; the **sensitivity-aware routing** that keeps medical / financial / secret payloads away from external AI is enforced one layer up by `MotebitRuntime` (`assertSensitivityPermitsAiCall`) at every AI-call entry — `runTurn`, `runTurnStreaming`, `generateCompletion`, and the wrapped outbound-tool registry — fail-closed before any bytes leave the device. The drift gate `check-sensitivity-routing` (#65) locks both sides: enforcement at the runtime boundary AND a `/sensitivity` affordance on every chat surface so users can actually elevate the tier.
...
Both are on the permissive floor (Apache-2.0), both at Layer 0. The protocol is zero-dep; the sdk's sole dependency is the protocol itself (never BSL code). The line is intentional:
