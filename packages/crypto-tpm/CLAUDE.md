...
Step 3 is the reason this lives in BSL, not MIT. **Which vendor roots to pin** is a policy judgment — motebit today accepts Infineon, Nuvoton, STMicroelectronics, and Intel PTT. **How to handle chain-path validation, revocation, and clock-skew** is a policy judgment. `@motebit/crypto` stays dep-thin and MIT-pure; this package metabolizes `@peculiar/x509` and hand-rolls a minimal TPM binary parser to produce a yes/no answer the sovereign verifier consumes.
