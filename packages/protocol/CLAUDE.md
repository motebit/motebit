...
5. **Interfaces at this layer are the interoperable contract.** `SettlementRail`, `GuestRail`, `SovereignRail`, `ChainAnchorSubmitter`, `CredentialSource`, `ServerVerifier` — a third party building an alternative implementation binds to these, not to BSL. Closed literal-union registries (`SuiteId`, `TokenAudience`, `ToolMode`, `ComputerActionKind`, `ContentArtifactType`) are the same closure pattern applied to wire-vocabulary surfaces; each one gets a named-constant set, a frozen `ALL_*` iteration array, an `is*` type guard, and a sibling drift gate (`check-suite-declared`, `check-audience-canonical`, `check-tool-modes`, `check-artifact-type-canonical`, etc.) scanning every literal site against the registry. Adding a member is intentional protocol-level work: union entry + named constant + iteration entry + gate update + doctrine.
