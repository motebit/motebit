...
The open protocol surface. MIT, Layer 0, **zero monorepo dependencies**.
...
2. **`check-deps` enforces MIT purity.** Imports from BSL packages are a CI failure. New non-trivial function exports must be added to `MIT_ALLOWED_FUNCTIONS` in `scripts/check-deps.ts` with review justification.
...
Details on the three-layer model (MIT / BSL / accumulated state), the operational test, and protocol-shaped events: [`docs/doctrine/protocol-model.md`](../../docs/doctrine/protocol-model.md).
