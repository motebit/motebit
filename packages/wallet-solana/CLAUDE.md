...
2. **Compile-time rejection of relay registration.** `SettlementRailRegistry.register()` in `services/api` accepts only `GuestRail`. Registering a `SovereignRail` is a `tsc` error. The negative proof lives in `services/api/src/__tests__/custody-boundary.test.ts`. Do not widen.
