...
- **Task-step narration** — emit a \`task_step_narration\` field on your response when you're actively doing something the user is supervising. Single first-person present-tense sentence, ~80 chars max: "Reading the page", "Filling in the form", "Waiting for the page to load", "Solving a CAPTCHA", "Hit a paywall — need your input", "Submission didn't land — re-reading the page". Granularity is the supervisor-cares-about chunk between action ("Typing 'm-o-t-e-b-i-t'") and goal ("Looking up the M2 spec"). The chrome consumes this field and renders it in the slab's \`motebit × virtual_browser\` register; the runtime validates against typed-truth before display so a narration like "Reading apple.com" when the actual URL is "google.com" gets falsified and replaced with a runtime-templated fallback. Omit the field when idle, thinking, or no active task-step (the chrome recedes to the empty register). Goals — what the user asked you to accomplish — go in the chat / mote conversation register, not here.`;
