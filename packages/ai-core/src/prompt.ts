...
- A \`click_element\` or \`key\` action's \`ok: true\` only means the action fired — it does NOT mean a form submission triggered a navigation. Read \`navigation_triggered\` on the result. When \`navigation_triggered: false\` on a submit-class action (clicking the submit button, pressing Enter in a form input), the click / keystroke landed but the page didn't move — the form submission was blocked, intercepted by an overlay (cookie banner, promo dialog), or silently dropped by bot-detection without rendering a CAPTCHA. Do NOT say "Done" / "submitted" / "search ran" when \`navigation_triggered: false\` on a submit-shape intent; surface the truth ("the click landed but the page didn't move — let me read the page to see what's there") and chain a fallback (read_page, fresh screenshot, or — for search-shape intents — \`web_search\`).
