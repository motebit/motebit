/**
 * Build the dogfood skill at `skills/motebit-spec-writer/`.
 *
 * Sibling to `build-reference-skill.ts` — same shape, different
 * content. Generates a fresh ephemeral keypair on each run, signs the
 * manifest + envelope per `spec/skills-v1.md`, writes the signed
 * artifacts to disk. The signing key is NOT persisted; signatures
 * verify offline against the embedded `public_key`.
 *
 * Re-run after editing the body or any frontmatter field:
 *
 *   pnpm --filter @motebit/skills build-spec-writer-skill
 */
...
const SKILL_DIR = join(REPO_ROOT, "skills", "motebit-spec-writer");

const BODY = `# Motebit Spec Writer
...
async function main(): Promise<void> {
  ...
  const unsignedManifest = {
    ...
    description:
      "Draft motebit/<name>@<version> specifications with the foundation-law markers, wire-format triple-sync, and drift-gate discipline the repo already uses. Read existing specs, mirror the template, ship the protocol type + zod schema + JSON Schema in one pass.",
    version: "1.0.0",
    platforms: ["macos", "linux", "windows"] as const,
    metadata: {
      author: "motebit dogfood",
      category: "protocol-engineering",
      tags: ["spec", "protocol", "motebit", "drift-defense"],
    },
    motebit: {
