...
8. **Query/act kinds are opaque here; the registries live with the record owner.** The `query` (records) and `act` (typed panel affordances) frames carry kind strings this package never interprets — `@motebit/runtime`'s `resolveAttachedRead`/`resolveAttachedAct` closed registries are the single authority, exactly as the invoke seam carries opaque capability names. Distinct frames keep records-vs-acts typed end-to-end; `invoke` stays relay delegation, never a local act. Absent seams answer honest `query_error`, never a hang.
9. **Bridged organs surface as tools only through `bridged-tools.ts`, definitions injected.** The module owns mechanics (live bridged-set view, execution over the bridge, fail-closed classification); the hosting surface injects the canonical `ToolDefinition`s at the registration site (this package cannot import `@motebit/tools`, and the embodiment stamp belongs to the registration site per `docs/doctrine/motebit-computer.md`). `AI_LOOP_EXCLUDED_ORGANS` (`se_attestation`) throws at wire time — deterministic user affordances never become model-chosen tools (`docs/doctrine/surface-determinism.md`). Policy stays in the hosting runtime's gate: bridged tools enter the SAME registry and the SAME policy gate as local tools, never a side door.
...
- `apps/cli` (increment 2, shipped) — the daemon binds (refusing honestly when a coordinator is live); the REPL attaches as a rendering frontend (chat / invoke / approval proxying via `apps/cli/src/runtime-host.ts`, which owns the authority-field strip at the wire boundary); `motebit serve` attaches-or-coordinates (attached: an MCP frontend over the coordinator's interior via the query/act frames — no DB handle, no worker mode, no signing).
