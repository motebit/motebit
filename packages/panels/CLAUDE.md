...
Drift gate `check-panel-controllers` enforces the cross-surface contract for every registered panel family — sovereign, agents, memory, goals, skills, **activity** + **retention** + **self-test** (the sovereignty-visible trifecta shipped 2026-05-06: signed-action timeline + browser-verified operator retention manifest + on-demand adversarial-onboarding probe). Any file under `apps/<app>/src/{ui,components}/` whose name matches a family pattern AND hits the family's I/O signatures (relay endpoints, runtime accessors, `cmdSelfTest` / `runSelfTestNow`) must import from `@motebit/panels`. Surfaces that ship the panel UI but bypass the controller silently drift the per-family contract; the gate catches that in CI rather than at user encounter.
