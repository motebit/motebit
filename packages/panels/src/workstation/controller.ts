...
/**
 * Activity event — the raw args/result bytes the workstation consumes
 * to render *what the motebit is doing right now* (when the tool's
 * output is something worth showing — e.g. a screenshot frame for
 * computer-use). Matches the runtime's `ToolActivityEvent`; duplicated
 * here for the same Layer-5 self-contained reason as
 ...
 *
 * The interface is kept in the public surface so Phase 2's computer-use
 * viewport state can subscribe to it without reshaping the adapter; for
 * the receipt-log-only MVP today the adapter's
 * `subscribeToolActivity` is optional and unused.
 */
