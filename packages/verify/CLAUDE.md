...
The canonical `motebit-verify` command-line tool. Apache-2.0 (permissive floor), Layer 6 (Applications). Ships the `motebit-verify` binary that handles every signed motebit artifact — identity files, execution receipts, credentials, presentations, skills, and content-artifact manifests on relay-assembled state exports — including credentials carrying hardware-attestation claims under any of the four canonical sovereign-verifiable platforms (Apple App Attest, Android Hardware-Backed Keystore Attestation, TPM 2.0, WebAuthn). The deprecated Google Play Integrity adapter was removed 2026-05-03 — credentials carrying `platform: "play_integrity"` now hit the canonical dispatcher's fail-closed "verifier not wired" branch. See `docs/doctrine/hardware-attestation.md` § "Three architectural categories" for the structural reason.

The `content-artifact` subcommand (`motebit-verify content-artifact <body-file> --manifest <header-or-path>`) verifies the relay-asserted `X-Motebit-Content-Manifest` HTTP header emitted on every state-export endpoint (`docs/doctrine/nist-alignment.md` §8). `--producer-key <hex>` pins the expected signer offline; `--expect <type>` narrows to a member of the closed `ContentArtifactType` registry in `@motebit/protocol`. Trust-anchor chain: fetch `/.well-known/motebit-transparency.json`, verify its self-signature against the embedded `relay_public_key`, then use that key as `--producer-key` for any subsequent state-export verification — same Ed25519 identity signs both the transparency declaration and every content-artifact manifest.

The `approval-decision` subcommand (`motebit-verify approval-decision <decision-file>`) verifies a signed human-consent `ApprovalDecision` — the "approve" band of the governance triad (`docs/doctrine/receipts-unified.md` § "the verifiable governance triad"), proof of permission before a gated act. The decision carries the approver's embedded `public_key`, so verification is self-contained offline; `--producer-key <hex>` pins which approver, `--expect-verdict approved|denied` asserts the outcome. Pure wiring around `@motebit/crypto`'s `verifyApprovalDecision` (Rule 1). A subcommand rather than auto-detection so the artifact stays contained here without expanding `@motebit/crypto`'s core detector union (the deny band — an `ExecutionReceipt{status:"denied"}` — and the auto band — a `ToolInvocationReceipt` — verify through the existing receipt path / `@motebit/crypto` library).
...
2. **Defaults match motebit's canonical identifiers.** `com.motebit.mobile` for iOS + Android bundles, `motebit.com` for WebAuthn Relying Party ID. Operators verifying credentials from a fork or federation peer override via CLI flags (`--bundle-id`, `--android-attestation-application-id`, `--rp-id`) or the `HardwareVerifierBundleConfig` object. The defaults are conveniences, not doctrine.
3. **No global state, no implicit fetches, no network.** Every adapter pins its own trust anchor at package-bundle time.
