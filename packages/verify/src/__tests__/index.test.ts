...
describe("cross-compatibility", () => {
  it("verifies files generated by @motebit/identity-file", async () => {
    // This test imports from the monorepo package to ensure the standalone
    // verifier is compatible with the identity-file generator
    const { generate, toHex } = await import("@motebit/identity-file");
    ...
    const result = await verify(content);
    expect(result.valid).toBe(true);
    expect(result.identity!.motebit_id).toBe("cross-compat-test");
    expect(result.identity!.devices).toHaveLength(1);
    expect(result.identity!.devices[0]!.name).toBe("Test Device");
  });
});
