...
- bfa04db: settlement-invoice@1.0 Increment 2 — wire-schemas + verifier re-export.
...
- e1f0826: Expose `verifyEvidenceProvenance` (and its `EvidenceProvenance` / `EvidenceProvenanceResult` / `DigestRef` / `DigestAlgorithm` types) through `@motebit/verifier`.
