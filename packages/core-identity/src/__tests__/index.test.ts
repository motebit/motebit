...
describe("InMemoryIdentityStorage", () => {
  ...
  it("returns null for unknown mote_id", async () => {
  ...
  it("saves and loads by mote_id", async () => {
    await storage.save({
      mote_id: "m1",
      created_at: 1000,
      owner_id: "owner-1",
...
describe("IdentityManager", () => {
  ...
  it("create() generates a valid identity with a UUID-like mote_id", async () => {
    const identity = await manager.create("owner-1");
    expect(identity.mote_id).toBeTruthy();
    // UUID v7 format: 8-4-4-4-12 hex chars with dashes
    expect(identity.mote_id).toMatch(
  ...
  it("create() logs an IdentityCreated event", async () => {
    const identity = await manager.create("owner-1");
    const events = await eventStore.query({ mote_id: identity.mote_id });
  ...
  it("load() returns the created identity", async () => {
    ...
    const loaded = await manager.load(created.mote_id);
    expect(loaded).not.toBeNull();
    expect(loaded!.mote_id).toBe(created.mote_id);
  });

  it("load() returns null for unknown mote_id", async () => {
  ...
  it("incrementClock() increments and persists the version_clock", async () => {
    const identity = await manager.create("owner-1");
    const clock1 = await manager.incrementClock(identity.mote_id);
    expect(clock1).toBe(1);

    const clock2 = await manager.incrementClock(identity.mote_id);
    expect(clock2).toBe(2);

    const loaded = await manager.load(identity.mote_id);
  ...
  it("export() returns the identity data", async () => {
    const created = await manager.create("owner-1");
    const exported = await manager.export(created.mote_id);
    expect(exported).not.toBeNull();
    expect(exported!.mote_id).toBe(created.mote_id);
    expect(exported!.owner_id).toBe("owner-1");
  });

  it("export() returns null for unknown mote_id", async () => {
