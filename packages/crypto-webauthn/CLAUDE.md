...
3. For `fmt: "packed"` without `x5c` (self attestation): verifying `attStmt.sig` over `authData || clientDataHash` using the public key carried IN `authData` (the credential public key itself). Self-attestation proves only that the credential's own key signed the challenge — not that any specific vendor minted it. The result reports `attestation_kind: "self"` so consumers can distinguish it from a vendor-rooted `"full"` chain; the routing semiring's per-platform scalar deliberately does not read the field (#446, `docs/doctrine/hardware-attestation.md` § sub-tier granularity — informational today, a deferred wire-format arc if a consumer ever needs to rank on it).
