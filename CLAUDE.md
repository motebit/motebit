...
**Agent delegation flow.** Discovery (DNS `_motebit.{domain}` or relay `/api/v1/agents/discover`) → identity verification (Ed25519 signed JWT + public key pinning on first contact) → budget allocation (estimateCost → allocateBudget, HTTP 402 if insufficient) → task submission (`motebit_task` synthetic tool through MCP) → execution (full agentic loop or direct tool execution for service motebits, PlanEngine records timeline events into execution ledger) → receipt (SHA-256 hashed prompt/result, canonical JSON, Ed25519 signed, nested delegation receipts for chain-of-custody) → relay receipt verification (Ed25519 signature check on ingestion) → trust record update (relay increments successful_tasks/failed_tasks, runs evaluateTrustTransition for auto-promotion/demotion, emits trust_level_changed event) → budget settlement (settleOnReceipt deducts actual cost) → credential issuance (relay issues AgentReputationCredential on verified receipt) → trust accumulation (verified receipts compound the semiring computation graph — graphRankCandidates uses trust records for algebraic routing) → ledger signing (runtime builds + signs GoalExecutionManifest per execution-ledger-v1.0 spec). The web-search service (`services/web-search/`) is the reference implementation proving the full loop.
