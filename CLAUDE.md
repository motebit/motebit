...
**Server verification boundary.** Third-party MCP servers are verified via `ServerVerifier` adapter (`verify(config, tools) → VerificationResult`), run automatically during `connect()` after tool discovery. Fail-closed: `ok:false` or thrown errors disconnect. Two built-in implementations: `ManifestPinningVerifier` (fail-closed, rejects on manifest change) and `AdvisoryManifestVerifier` (always accepts, revokes trust on change — used by desktop/web/mobile/spatial for user-facing re-approval flows). `configUpdates` in the result carry new hash/toolNames/trusted for persistence by callers. All 5 surface apps now use ServerVerifier instead of manual `checkManifest()` boilerplate.
