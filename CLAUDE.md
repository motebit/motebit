...
- **The body is always-already there.** Surfaces precede content. The body exists before content arrives, persists when content departs, never depends on content for its existence. Content embeds into the body's typed slots, never adjacent — adjacency violates body coherence. Empty states are READY states: when the body has no content, it shows a sympathetic-breathing affordance announcing readiness, inheriting the substrate's quiescence rhythm so the empty register is quietly alive, not silent. Lazy-render-on-first-content is wrong by default; a body whose existence is contingent on content is two surfaces stitched together, not one body in two registers. Compounds with `liquescentia-as-substrate` (the medium is always present), `motebit-computer` (slab is the body's first-person perceptual field), and `records-vs-acts` (the body persists between acts, not only during them). See [`docs/doctrine/always-already-body.md`](docs/doctrine/always-already-body.md).
