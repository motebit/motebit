...
- **The body is always-already there.** Surfaces precede content. A body whose existence is contingent on content is two surfaces stitched together, not one body in two registers. The slab on /computer is alive the moment the user lands — `WebApp.bootstrapComputer` eagerly calls `registration.ensureDefaultSession()` at app boot, and within 1-2s the live_browser slab item mounts via `onSessionLive`, chrome lands inside its proper slot. Content embeds INTO the body's typed slots — chrome inside `controlBandSlot`, screencast inside the screen mesh, ghost-ready affordance inside `stageEl` — never adjacent (no `setSlabControlBand` viewport-top fallback, no off-slab overlays). Empty states are READY states: a sympathetic-breathing pulsing mark + caption (`type a URL · or ask motebit`) fills the body during cold-start and post-session-close, breathing at the same 0.3 Hz the creature does, body-coherent. Compounds with `liquescentia-as-substrate` (the medium is always present), `motebit-computer` (slab is the body's first-person perceptual field), and `records-vs-acts` (acts pass through the body, records sit alongside; the body persists between acts). See [`docs/doctrine/always-already-body.md`](docs/doctrine/always-already-body.md).
