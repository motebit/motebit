...
**What's proven vs. what's untested.** Proven: identity bootstrap, memory compounding over conversations, policy governance, multi-device sync (localhost + test), agent delegation with signed receipts (integration tests), trust accumulation from verified receipts. Untested in production: two personal motebits delegating across the internet, multi-device sync under real network conditions (cellular → WiFi → relay), the intelligence gradient trending upward over weeks of real usage, and external developers completing the `create-motebit` → CLI → delegation flow without guidance.
