...
**Server verification boundary.** Third-party MCP servers are verified via `ServerVerifier` adapter (`verify(config, tools) → VerificationResult`), run automatically during `connect()` after tool discovery. Fail-closed: `ok:false` or thrown errors disconnect. `ManifestPinningVerifier` is the built-in implementation: pins tool manifest SHA-256 hash on first connect, rejects on subsequent connects if manifest changes, returns diff diagnostics (tools added/removed). `configUpdates` in the result carry new hash/toolNames for persistence by callers. This is integrity-of-surface verification, not cryptographic server identity — deeper attestation (certificate pinning, server identity) is the next `ServerVerifier` implementation.
