...
services/
  api/             Sync relay server: REST + WebSocket, device auth, receipt verification, budget/settlement, credential issuance, rate limiting, graph query endpoints (trust-closure, path-to, graph, routing-explanation with provenance), multi-hop delegation edge caching (relay_delegation_edges), persistent relay identity, federation endpoints (peering, discovery, cross-relay routing, settlement). Organized into modules: index.ts, federation.ts, task-routing.ts, credentials.ts, pairing.ts, data-sync.ts
