...
- [`docs/doctrine/always-already-body.md`](docs/doctrine/always-already-body.md) — surfaces precede content; content embeds into the body, never adjacent; empty states are READY, not absent — the slab announces itself before any session opens
...
- **The body is always-already there.** "Body" is a category — the _identity body_ is the creature (canonically derived in [`DROPLET.md`](DROPLET.md): sphere by variational calculus, 0.3 Hz breathing by Rayleigh's eigenmode, glass-transmissive by material derivation; sufficiency proven in §VIII); the _perceptual body_ is a workspace surface where motebit acts and the user perceives (the slab on /computer is the worked instance per [`motebit-computer.md`](docs/doctrine/motebit-computer.md), itself a body geometrically — glass volume, meniscus silhouette, typed slots, sympathetic-breathing substrate inheriting DROPLET's 0.3 Hz; mobile + spatial inherit when implemented). Both subtypes obey the same rule: surfaces precede content; content embeds INTO the body's typed slots, never adjacent; empty states are READY, not absent. A body whose existence is contingent on content is two surfaces stitched together, not one body in two registers. The slab is alive the moment the user lands on /computer — `WebApp.bootstrapComputer` eagerly calls `registration.ensureDefaultSession()` at app boot, and within 1-2s the live_browser slab item mounts via `onSessionLive`. Content embeds via the slab item's typed slots — chrome inside `controlBandSlot`, screencast inside the screen mesh, ghost-ready affordance inside `stageEl` — never adjacent (no `setSlabControlBand` viewport-top fallback, no off-slab overlays). Empty register: sympathetic-breathing pulsing mark + caption (`type a URL · or ask motebit`) fills the body during cold-start and post-session-close, breathing at the same 0.3 Hz the identity body does, body-coherent. Compounds with `liquescentia-as-substrate` (the medium is always present) and `records-vs-acts` (acts pass through the body, records sit alongside; the body persists between acts). See [`docs/doctrine/always-already-body.md`](docs/doctrine/always-already-body.md).
